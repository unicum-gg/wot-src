package net.wg.white_tiger.gui.battle.views.wtBossWidget.components
{
   ...
   public class BossShield extends BattleUIComponent
   {
      ...
      {
         if(this._bossType != param1 && WT_VEHICLE_TYPE.isBossType(param1))
         {
            this._bossType = param1;
            this.eventBossTankIcon.bossType = this._bossType;
            this.eventBossHyperion.bossType = this._bossType;
            if(this._bossType == WT_VEHICLE_TYPE.BOSS_SPECIAL)
            {
               this.eventBossBackground.setSpecialTank();
            }
         }
      }
