...
from comp7.gui.impl.gen.view_models.views.lobby.summary_statistics_model import SummaryStatisticsType
...
class _SeasonStatistics(ViewImpl):
    ...
    def __updateSummaryStatistics(self, model):
        ...
        seasonStats = getComp7DossierStats(accDossier, season=self.__seasonNumber)
        viewModels = []
    ...
    def __updateVehicleStats(self, model):
        accDossier = self.__itemsCache.items.getAccountDossier()
        vehicles = getComp7DossierStats(accDossier, season=self.__seasonNumber).getVehicles()
        if not vehicles:
            return
        viewModels = []
