...
class ShowWTBattleResultsHandler(_ShowArenaResultHandler):
    ...
    def _showWindow(self, notification, arenaUniqueID):
        ...
        if self.prbEntity.isInQueue():
            self._showI18nMessage('#white_tiger.event.extend:notifications/battleResults/disableInQueue', SystemMessages.SM_TYPE.Warning)
            return
        if not self.gameEventCtrl.isEventPrbActive():
            self._showI18nMessage('#white_tiger.event.extend:notifications/battleResults/disableNotInPrebattle', SystemMessages.SM_TYPE.Warning)
            return
