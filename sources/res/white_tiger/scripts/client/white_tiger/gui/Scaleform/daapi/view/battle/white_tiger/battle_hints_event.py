...
_CONFIG_FILE = 'white_tiger/gui/battle_hints.xml'
...
class BattleHintData(namedtuple('_HintData', ('name', 'componentAlias', 'iconPath', 'duration', 'maxWaitTime',
                         ...
                         'rawMessage', 'backgroundColor', 'useCountdownTimer'))):
    ...
    def makeVO(self, data=None):
        ...
        if 'remainingTime' in data:
            timer = data['remainingTime']
        return {'message': message, 
           'iconSource': backport.image(R.images.white_tiger.gui.maps.icons.battleHints.dyn(self.iconPath)()) if hasIcon else None, 
           'timer': timer * 1000, 
