...
from gui.wt_event.wt_event_helpers import getFormattedTimeLeft
...
from skeletons.gui.game_control import IWhiteTigerController, IQuestsController
...
def clockSh(vspace=-6, hspace=-6):
    return icons._getIcon('clockSh', vspace=vspace, hspace=hspace)
...
class EventBattlesSelectorTooltip(BlocksTooltipData):
    ...
    def _packBlocks(self, *args):
        ...
        timeTableBlocks = [
         self._packTimeTableHeaderBlock()]
...
class EventBattlesServerPrimeTime(ToolTipBaseData):
    ...
    def getDisplayableData(self, peripheryID):
        ...
        timeLeftStr = '-'
        primeTimeStr = R.strings.white_tiger.primeTime.tooltips.server
        isNow = False
...
class EventBattlesCalendar(BlocksTooltipData):
    ...
    def _packBlocks(self, *args, **kwargs):
        ...
        if self.__gameEventController.isModeActive():
            blocks.append(self.__packHeader())
            blocks.append(formatters.packBuildUpBlockData([
             packTimeTableHeaderBlock(SELECTOR_BATTLE_TYPES.WHITE_TIGER)] + packCalendarBlock(self.__gameEventController, time_utils.getCurrentTimestamp(), SELECTOR_BATTLE_TYPES.WHITE_TIGER), 0, BLOCKS_TOOLTIP_TYPES.TOOLTIP_BUILDUP_BLOCK_WHITE_BG_LINKAGE, padding=formatters.packPadding(top=-8, bottom=-12)))
            blocks.append(self.__packFooter())
        return blocks

    def __packHeader(self):
        return formatters.packTextBlockData(text_styles.middleTitle(backport.text(_STR_PATH.selectorTooltip.header())), padding=formatters.packPadding(bottom=5))

    def __packFooter(self):
        currentSeason = self.__gameEventController.getCurrentSeason()
        endDate = currentSeason.getEndDate()
        if endDate is None:
            _logger.error("Current season doesn'n have end date")
            return ''
        else:
            timeLeft = getFormattedTimeLeft(getSecondsLeft())
            return formatters.packTextBlockData(text_styles.main(backport.text(_STR_PATH.selectorTooltip.tillEnd(), value=text_styles.highlightText(timeLeft))), padding=formatters.packPadding(top=-6, bottom=-10))
...
class EventQuestsTooltipData(BlocksTooltipData):
    ...
    def _packBlocks(self, *args, **kwargs):
        ...
        if vehicle is not None and self.__gameEventController.isInPrimeTime():
            quests = sorted([ q for q in self.__questController.getQuestForVehicle(vehicle) if q.isEventBattlesQuest()
                            ], key=questsSortFunc)
            if quests:
    ...
    def __packHeader(self, quests, vehicle):
        ...
        isBoss = g_wt_config.isBossVehicle(vehCD)
        allBossCompleted = total == completed and isBoss
        if diff > 0 or not isPrimeTimeLeft or isLastSeasonDay or allBossCompleted:
            ...
            desc = text_styles.concatStylesToSingleLine(clockSh(), text_styles.tutorial(backport.text(R.strings.event.tooltips.header.quests.header.description.till_upd(), time=getTillTimeByResource(EventInfoModel.getDailyProgressResetTimeDelta(), R.strings.menu.Time.timeLeftShort))))
        return formatters.packImageTextBlockData(title=text_styles.highTitle(title), desc=desc, img=backport.image(R.images.white_tiger.gui.maps.icons.quests.dyn(('{}_header').format(vehicle.eventType))()), txtPadding=formatters.packPadding(top=18), descPadding=formatters.packPadding(top=15, left=-4), txtOffset=20)
    ...
    def __packQuestReward(self, quest):
        ...
        if blocks:
            return formatters.packBuildUpBlockData(blocks, layout=BLOCKS_TOOLTIP_TYPES.LAYOUT_HORIZONTAL, padding=formatters.packPadding(left=20, right=20), align=BLOCKS_TOOLTIP_TYPES.ALIGN_CENTER)
        else:
            return
...
class EventLootBoxTooltipWindowData(WulfTooltipData):
    ...
    def getTooltipContent(self, boxType, *args, **kwargs):
        return WtEventLootBoxTooltipView(isHunterLootBox=boxType == WhiteTigerLootBoxes.WT_HUNTER)
...
class EventStampTooltipWindowData(WulfTooltipData):

    def __init__(self, context):
        super(EventStampTooltipWindowData, self).__init__(context, TOOLTIPS_CONSTANTS.EVENT_STAMP)

    def getTooltipContent(self, *args, **kwargs):
        return WtEventStampTooltipView()


class EventMainPrizeDiscountTooltipWindowData(WulfTooltipData):

    def __init__(self, context):
        super(EventMainPrizeDiscountTooltipWindowData, self).__init__(context, TOOLTIPS_CONSTANTS.EVENT_MAIN_PRIZE_DISCOUNT)

    def getTooltipContent(self, *args, **kwargs):
        return MainPrizeDiscountTooltipView()
...
class EventCarouselVehicleTooltipData(WulfTooltipData):
    ...
    def getTooltipContent(self, intCD, *args, **kwargs):
        ...
        return WtCarouselVehicleTooltipView(vehInvID=vehicle.invID)


class EventBattlesEndData(WulfTooltipData):

    def __init__(self, context):
        super(EventBattlesEndData, self).__init__(context, TOOLTIPS_CONSTANTS.EVENT_BATTLES_END)

    def getTooltipContent(self, *args, **kwargs):
        return WtEventBattlesEndTooltipView()


class WtGuaranteedRewardTooltipData(WulfTooltipData):

    def __init__(self, context):
        super(WtGuaranteedRewardTooltipData, self).__init__(context, TOOLTIPS_CONSTANTS.WT_GUARANTED_REWARD)
