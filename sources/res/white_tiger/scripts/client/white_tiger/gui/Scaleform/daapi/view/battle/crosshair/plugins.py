...
BOSS_SHIELD_DYN_COMPONENT_NAME = 'boss_shield'

def createPlugins():
    resultPlugins = {'plasmaBuffPlugin': PlasmaBuffPlugin, 
       'specialShotPlugin': SpecialShotPlugin, 
       'shotResultIndicator': WTShotResultIndicatorPlugin}
...
class PlasmaBuffPlugin(CrosshairPlugin):
    ...
    def __updatePlasmaCount(self, vInfo, value=None):
        ...
        if value:
            ...
            plasmaBonus = arenaInfo.dynamicComponents.get('wtPlasmaBonusComponent') if arenaInfo else None
            if plasmaBonus:
