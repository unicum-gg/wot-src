...
def getViewSettings():
    ...
    return (
     ViewSettings(WHITE_TIGER_HANGAR_ALIASES.WHITE_TIGER_HANGAR, HangarWindow, '', WindowLayer.SUB_VIEW, WHITE_TIGER_HANGAR_ALIASES.WHITE_TIGER_HANGAR, ScopeTemplates.LOBBY_SUB_SCOPE),
     ViewSettings(WHITE_TIGER_HANGAR_ALIASES.WHITE_TIGER_PRIME_TIME_ALIAS, WhiteTigerPrimeTimeView, getSwfExtensionUrl('white_tiger', WHITE_TIGER_HANGAR_ALIASES.WHITE_TIGER_PRIME_TIME), WindowLayer.TOP_SUB_VIEW, WHITE_TIGER_HANGAR_ALIASES.WHITE_TIGER_PRIME_TIME_ALIAS, ScopeTemplates.LOBBY_SUB_SCOPE, True),
...
def _getVehicleTypeImage(resImgId, vTypeName):
    ...
    if imgId != -1:
        return backport.image(imgId).replace('img://gui/', '../')
    return ''