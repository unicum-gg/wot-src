...
def whiteTigerSquadOptionBuilder(cm, options, userCMInfo, ctrl=None):
    ...
    if not ctrl.isAvailable():
        ...
        if not cm.isSquadAlreadyCreated(PREBATTLE_TYPE.WHITE_TIGER):
            wtSquadItem = cm.makeItem(CREATE_WHITE_TIGER_SQUAD, backport.text(R.strings.menu.contextMenu.createEventSquad()), optInitData={'enabled': not cm.prbEntity.isInQueue(), 'textColor': HIGHLIGHT_COLOR})
            if squadItem:
