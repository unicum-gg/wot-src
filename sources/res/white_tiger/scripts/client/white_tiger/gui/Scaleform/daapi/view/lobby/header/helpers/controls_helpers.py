...
class WhiteTigerLobbyHeaderHelper(DefaultLobbyHeaderHelper):
    ...
    def _getInSquadTooltipData(cls, _):
        ...
        return (
         wtSquadTooltip.header(), defaultSquadTooltip.body(), {})
    ...
    def _getDisabledFightTooltipData(cls, prbValidation, isInSquad):
        ...
        if state == PREBATTLE_RESTRICTION.VEHICLE_NOT_READY and 'noTickets' in ctx and ctx['noTickets']:
            ...
            rSubClass = _RESTRICTION_TO_STRING_SUBCLASS.get(state, rSubClass)
        if not rSubClass:
            return (makeTooltip(None, None), False)
        else:
            return (
             makeTooltip(backport.text(rSubClass.header()), backport.text(rSubClass.body())), False)