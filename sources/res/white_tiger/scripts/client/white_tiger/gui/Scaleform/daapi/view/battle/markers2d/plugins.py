...
import BigWorld
from gui.Scaleform.daapi.view.battle.shared.markers2d.plugins import AreaMarkerPlugin, ChatCommunicationComponent
from gui.Scaleform.daapi.view.battle.shared.markers2d.markers import BaseMarker, ReplyStateForMarker
from gui.battle_control import avatar_getter
from chat_commands_consts import INVALID_MARKER_SUBTYPE, INVALID_MARKER_ID, MarkerType, DefaultMarkerSubType, INVALID_COMMAND_ID
...
from white_tiger.cgf_components.wt_helpers import getPlasmaBonusComponent
...
class WhiteTigerBaseAreaMarkerPlugin(AreaMarkerPlugin, ChatCommunicationComponent):
    ...
    def _getMarkerFromTargetID(self, targetID, markerType):
        componentID = self.getMarkerIdFormEntityID(targetID)
        if componentID not in self.__markers or markerType != self.getMarkerType():
            return None
        return self.__markers[componentID]
    ...
    def __onActionAddedToMarkerReceived(self, senderID, commandID, markerType, generatorIndex):
        ...
        if not avatar_getter.isVehicleAlive() and marker.getBoundCheckEnabled() and _ACTIONS.battleChatCommandFromActionID(commandID).name not in AUTOCOMMIT_COMMAND_NAMES:
...
class WhiteTigerVehicleMarkerPlugin(RespawnableVehicleMarkerPlugin):
    ...
    def _onVehicleFeedbackReceived(self, eventID, vehicleID, value):
        ...
        elif eventID == FEEDBACK_EVENT_ID.WT_VEHICLE_DISCRETE_DAMAGE_RECEIVED:
            attackerID, plasmaDamage = value
            plasmaDamage = round_py2_style(plasmaDamage)
    ...
    def _setVehicleInfo(self, marker, vInfo, guiProps, nameParts):
        ...
        if WT_VEHICLE_TAGS.BOSS in vType.tags:
            classTag = 'wtboss'
        if WT_VEHICLE_TAGS.PRIORITY_BOSS in vType.tags:
            classTag = 'wtSpecialBoss'
        if WT_VEHICLE_TAGS.MINIBOSS in vType.tags:
            classTag = 'miniboss'
            customName = backport.text(R.strings.white_tiger_battle.botNames.Ermelinda())
        elif WT_VEHICLE_TAGS.BOT in vType.tags and WT_VEHICLE_TAGS.BOSS not in vType.tags:
            customName = backport.text(R.strings.white_tiger_battle.playersPanel.botName())
        if customName == '':
            customName = nameParts.playerName
        canShowPlasma = not isBossTeam(vInfo.team)
        self._invokeMarker(markerID, 'showPlasmaBuff', canShowPlasma)
        self._invokeMarker(markerID, 'setVehicleInfo', classTag, vType.iconPath, nameParts.vehicleName, vType.level, nameParts.playerFullName, customName, nameParts.clanAbbrev, nameParts.regionCode, vType.maxHealth, guiPropsName, hunting, squadIndex, backport.text(R.strings.ingame_gui.stun.seconds()))
        self._invokeMarker(markerID, 'update')

    def __onPlasmaChanged(self, plasmaDict):
        for vehId, plasmaBuffValue in viewitems(plasmaDict):
            if vehId not in self._markers:
                continue
            markerID = self._markers[vehId].getMarkerID()
