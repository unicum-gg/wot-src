...
class TankInfoView(ViewComponent[TankInfoViewModel]):
    ...
    def createToolTipContent(self, event, contentID):
        ...
        if parameter is not None:
            return TankInfoTooltipView(parameter=parameter)
        else:
            return super(TankInfoView, self).createToolTipContent(event, contentID)
    ...
    def _getEvents(self):
        return (
         (
          g_currentVehicle.onChanged, self.__fillModel),)
