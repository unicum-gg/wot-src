...
class ConsumablesPanelView(ViewComponent[HangarConsumablesPanelViewModel]):
    ...
    def createToolTipContent(self, event, contentID):
        ...
        return super(ConsumablesPanelView, self).createToolTipContent(event, contentID)
    ...
    def _getEvents(self):
        return (
         (
          g_currentVehicle.onChanged, self.__update),
         (
          self.viewModel.onBuyTicket, self.__onBuyTicketClick),
         (
          self.viewModel.onOpenTasks, self.__onOpenTasksClick))
    ...
    def __onTokensUpdate(self, diff):
        if self.__wtCtrl.isEventPrbActive() and any('wtevent:' in key for key in diff.keys()):
            self.__update()
    ...
    def __fillEquipment(self, array, item):
        ...
        for eq in item.consumables.layout:
            ...
            if 'repairkit' in eq.tags or 'medkit' in eq.tags:
                ...
                ultimates.append(eq)
            elif 'builtin' in eq.tags or 'trigger' in eq.tags:
                abilities.append(eq)
