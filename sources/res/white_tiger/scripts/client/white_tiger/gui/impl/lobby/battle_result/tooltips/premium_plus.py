...
class WtPremiumPlusTooltip(ViewImpl):
    __slots__ = ()
    __battleResults = dependency.descriptor(IBattleResultsService)

    def __init__(self):
        contentResID = R.views.white_tiger.lobby.postbattle.tooltips.PremiumPlus()
        settings = ViewSettings(contentResID)
        settings.model = PremiumPlusModel()
        super(WtPremiumPlusTooltip, self).__init__(settings)

    def _onLoading(self, *args, **kwargs):
        super(WtPremiumPlusTooltip, self)._initialize(*args, **kwargs)
