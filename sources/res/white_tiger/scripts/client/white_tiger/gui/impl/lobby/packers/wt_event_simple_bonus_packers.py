...
from gui.shared.missions.packers.bonus import BaseBonusUIPacker, SimpleBonusUIPacker, GoodiesBonusUIPacker, ItemBonusUIPacker, VehiclesBonusUIPacker, CustomizationBonusUIPacker
from gui.shared.utils.functions import makeTooltip
from white_tiger.gui.impl.lobby.packers.wt_event_bonuses_packers import WTCrewBookBonusUIPacker, WTCrewSkinBonusUIPacker
...
HUNTER_BONUSES_ORDER = [
 Currency.CREDITS, 'goodies', 'items', constants.PREMIUM_ENTITLEMENTS.PLUS, 'slots',
 'customizations', 'tmanToken', 'battleToken']
BOSS_BONUSES_ORDER = [Currency.GOLD, Currency.CREDITS, 'crewBooks', _RANDOM_CREWBOOK,
 'goodies', constants.PREMIUM_ENTITLEMENTS.PLUS, _RANDOM_STYLE_2D, _RANDOM_DECAL, 'items']
_ICON_PATH = R.images.gui.maps.icons
_EXT_ICON_PATH = R.images.white_tiger.gui.maps.icons
_BONUS_TYPE_TO_GROUP = {_RANDOM_STYLE_3D: BonusGroup.STYLE_3D, 
   'vehicles': BonusGroup.VEHICLES}
NO_SPLITTED_BONUSES = frozenset((
 _RANDOM_CREWBOOK, _RANDOM_DECAL, _RANDOM_STYLE_2D, _RANDOM_STYLE_3D, _EMBLEM, _CREWBOOK))

def getWtSplitBonusFunction(bonus):
    if bonus.getName() in NO_SPLITTED_BONUSES:
        return None
    else:
...
_AWARD_VIEW_BONUS_POSITIONS = {'vehicles': 2, 
   'mainPrizeDiscount': 1, 
   'ticket': 1, 
   'tmanToken': 1}
...
def mergeWtProgressionBonuses(bonuses):
    ...
    mergeBonuses(bonuses)
    if len(bonuses) > 1:
        newli = copy.copy(bonuses)
        for idx, x in enumerate(newli):
            name = x.getName()
            if name in _AWARD_VIEW_BONUS_POSITIONS:
                newIdx = _AWARD_VIEW_BONUS_POSITIONS[name]
                bonuses.insert(newIdx, bonuses.pop(idx))
...
def sortBonuses(bonus, orderList):
    bonusName = bonus.getName()
    if bonusName in orderList:
        return orderList.index(bonusName)
...
def _getBonusStatus(bonusType, bonusGroup, boxCtrl=None):
    rewards = boxCtrl.getLootBoxesRewards(WhiteTigerLootBoxes.WT_BOSS)
    if not rewards:
        ...
        if not bonus:
            ...
            return None
        if all([ bonus.getC11nItem(item).fullCount() > 0 for item in bonus.getValue() ]):
            return _Status.COLLECTED
        return _Status.NOT_COLLECTED


def _getStyles3dStatus():
    return _getBonusStatus(_RANDOM_STYLE_3D, BonusGroup.CUSTOMIZATION)
...
STATUS_GETTERS = {_RANDOM_STYLE_3D: _getStyles3dStatus}
...
def _composeCustomization(bonuses):
    ...
    for bonus in bonuses:
        ...
        for item in bonus.getCustomizations():
            ...
            elif itemType in ('decal', 'projectionDecal'):
                decals.append(bonus)
            elif itemType == 'inscription':
...
def _composeProgressionCustomizations(bonuses):
    ...
    for bonus in bonuses:
        bonusName = bonus.getName()
        if bonusName == 'battleToken':
            continue
        elif bonusName == 'customizations':
            progBonus.append(WtCustomizationsBonus(bonusName, bonus.getValue()))
        else:
...
_BONUS_COMPOSERS = (
 _composeCrewBooks, _composeCustomization)
...
class StylePacker(object):
    ...
    def pack(cls, model, bonus):
        ...
        item.setIsCollected(STATUS_GETTERS.get(cls._TOOLTIP_TYPE) == _Status.COLLECTED)
        model.addViewModel(item)


class Event3dStylePacker(StylePacker):
    _TOOLTIP_TYPE = _RANDOM_STYLE_3D


class Event2dStylePacker(StylePacker):
    _TOOLTIP_TYPE = _RANDOM_STYLE_2D


class EventDecalPacker(StylePacker):
    _TOOLTIP_TYPE = _RANDOM_DECAL
...
class SimpleEventPremiumPacker(SimpleBonusUIPacker):
    ...
    def _packSingleBonus(cls, bonus, label=''):
        model = WtPortalBonusModel()
        model.setIcon('premium_plus_universal')
        return model
...
class CustomizationEventItemPacker(CustomizationBonusUIPacker):
    ...
    def _packSingleBonus(cls, bonus, item, count):
        ...
        customizationItem = bonus.getC11nItem(item)
        if isStyle3D(customizationItem):
            model.setIcon(R.images.gui.maps.icons.quests.bonuses.small.style_3d())
        else:
...
def getDefaultWtSimpleBonusPackersMap():
    return {Currency.CREDITS: SimpleEventCountablePacker(), 
