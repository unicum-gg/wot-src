...
from white_tiger.gui.shared.event_dispatcher import showInfoPage, showFinalNarrativeView
...
class HangarView(ViewComponent[HangarViewModel], IRoutableView):
    ...
    def _getEvents(self):
        ...
        return [
         (
          model.onEscPressed, self.__onEscape),
         (
          model.onViewLoaded, self.__onViewLoaded),
         (
          model.onInfoClicked, self.__onInfoClicked),
         (
          model.onNarrativeClicked, self.__onNarrativeClicked),
         (
    ...
    def _onLoading(self, *args, **kwargs):
        ...
        self.viewModel.setIsNarrativeAvailable(stageFinished == maxProgression)
    ...
    def __onSyncCompleted(self, reason, diff):
        if self.__wtCtrl.isEventPrbActive():
            stageFinished = self.__economicsCtrl.getFinishedLevelsCount()
            maxProgression = self.__economicsCtrl.getProgressionMaxLevel()
            self.viewModel.setIsNarrativeAvailable(stageFinished == maxProgression)
