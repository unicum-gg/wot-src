...
def isWTBannerEntryPointAvailable(wtController=None):
    return wtController.isModeActive()
...
class WTEventEntryPoint(ViewImpl, Notifiable):
    ...
    def createToolTipContent(self, event, contentID):
        ...
        if contentID == R.views.white_tiger.lobby.tooltips.BattlesEndTooltipView():
    ...
    def __getState(self):
        if self.__wtController.isInPrimeTime() and not self.__wtController.isFrozen():
            return State.ACTIVE
        if self.__wtController.isBattlesEnd():
            return State.BATTLES_END
        return State.LOCKED