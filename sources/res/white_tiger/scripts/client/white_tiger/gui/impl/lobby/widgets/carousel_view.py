...
class CarouselView(ViewComponent[HangarCarouselViewModel]):
    ...
    def createToolTipContent(self, event, contentID):
        ...
        if contentID == R.views.white_tiger.mono.lobby.tooltips.carousel_vehicle_tooltip():
            return CarouselVehicleTooltipView(vehInvID=event.getArgument('id'))
    ...
    def _getCallbacks(self):
        return (
         (
          'cache.vehsLock', self.__onVehicleLockUpdated),
         (
    ...
    def __onTokensUpdate(self, diff):
        if self.__whiteTigerCtrl.isEventPrbActive() and any('wtevent:' in key for key in diff.keys()):
            self.__fillVehicles()
