...
class QuestsView(ViewComponent[QuestsViewModel]):
    ...
    def _getEvents(self):
        return (
         ...
         (
          self.wtController.onSettingsChanged, self._onSettingsChanged))
    ...
    def __populateModel(self):
        ...
        with self.viewModel.transaction() as (model):
            ...
            self._updateEventCountdown(model)
    ...
    def _updateEventCountdown(self, model):
        timeLeft = self.wtController.getTimeLeft()
        model.setEventCountdown(timeLeft if timeLeft > 0 else -1)
...
class QuestsContainer(object):
    ...
    def getQuests(self, groupID, allowCompleted=False, reverse=False):
        ...
        quests = ((qID, q) for qID, q in viewitems(self.eventsCache.getAllQuests(filterQuests)) if q.getGroupID() == groupID)
