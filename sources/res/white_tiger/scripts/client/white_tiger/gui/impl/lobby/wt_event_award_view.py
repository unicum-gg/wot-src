...
class WTEventAwardView(ViewImpl):
    ...
    VALUABLE_REWARDS_PREFIX = ('dossier', 'tmanToken')
    ...
    def createToolTipContent(self, event, contentID):
        ...
        return super(WTEventAwardView, self).createToolTipContent(event, contentID)
    ...
    def _onLoading(self, questId, *args, **kwargs):
        ...
        isValuableReward = self.__hasValuableReward(bonuses)
        ...
        with self.getViewModel().transaction() as (model):
            model.setIsValuableReward(isValuableReward)
            model.setIsPostBattle(isPostBattle)
            model.setTitle(backport.text(rAwardsView.title()))
            model.setStatus(statusText)
            packBonusModelAndTooltipData(bonuses, model.getMainRewards(), self._tooltipItems, getWtEventBonusPacker())
        self.__addListeners()
    ...
    def __hasValuableReward(self, bonuses):
        for bonus in bonuses:
            if bonus.getName() in self.VALUABLE_REWARDS_PREFIX:
                return True
