...
_GENERATOR_ID_LABEL_MAP = {1: 'A', 2: 'B', 3: 'C'}
...
class WhiteTigerHudView(ViewImpl, IArenaVehiclesController):
    ...
    def __init__(self):
        ...
        with self.viewModel.transaction() as (model):
            _modelsArray = Array()
            for k, v in viewitems(_GENERATOR_ID_LABEL_MAP):
    ...
    def addVehicleInfo(self, vo, _):
        if WT_VEHICLE_TAGS.MINIBOSS in vo.vehicleType.tags:
            self.__setupMiniBossInfo(vo)
        if WT_VEHICLE_TAGS.BOSS in vo.vehicleType.tags:
            self.__setupBoss(vo)
    ...
    def __setupBoss(self, vInfo):
        with self.viewModel.transaction() as (model):
            ...
            isCurrentPlayer = avatar_getter.getPlayerVehicleID() == vInfo.vehicleID
            model.setIsAlly(isCurrentPlayer)
    ...
    def __onGeneratorStopCapture(self, generatorID, _):
        if generatorID not in self._generatorModels:
            _logger.error('ID does not exist')
            return
