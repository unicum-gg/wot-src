...
from white_tiger_common.wt_constants import PREBATTLE_TYPE, QUEUE_TYPE, CLIENT_UNIT_CMD
import logging
...
class WhiteTigerBattleSquadEntity(SquadEntity):
    ...
    def getConfirmDialogMeta(self, ctx):
        ...
        if not ctrl.isModeActive() or not ctrl.isHangarAvailable() or ctrl.isFrozen():
            return None
        return super(WhiteTigerBattleSquadEntity, self).getConfirmDialogMeta(ctx=ctx)
    ...
    def _setVehicle(self, ctx, callback=None):
        ...
        if not vehTypeCD:
            _logger.error('Vehicle is not valid  %r', ctx)
            if callback:
                callback(False)
            return
        self._requestsProcessor.doRequest(ctx, 'doCustomUnitCmd', CLIENT_UNIT_CMD.WT_SQUAD_SELECT_VEHICLE, uint64Arg=vehTypeCD, callback=callback)