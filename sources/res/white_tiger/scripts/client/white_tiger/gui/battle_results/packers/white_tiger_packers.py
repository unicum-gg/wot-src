...
from white_tiger.gui.white_tiger_gui_constants import WT_QUEST_BOSS_GROUP_ID, MAX_VISIBLE_QUESTS, HUNTER_QUEST_CHAINS, SPECIAL_QUEST_GROUP_IDS
...
class WhiteTigerPersonalRewards(PersonalRewards):
    _REWARDS_ORDER = [
     ...
     'customizations',
     'wtevent_ticket']
    _AVAILABLE_REWARDS = _REWARDS_ORDER
    ...
    def packModel(cls, modelArray, battleResults, tooltipData):
        ...
        for model in sorted(viewvalues(rewards), key=cls._rewardSortKey)[:_MAX_VISIBLE_REWARDS]:
...
class WTBonusConditionPacker(BonusConditionPacker):
    ...
    def _traversCondition(self, ctx):
        ...
        if not preFormattedCondition:
            ...
            event = ctx['event']
            isSpecialMission = event.getGroupID() in SPECIAL_QUEST_GROUP_IDS
            decorationIcon = DailyQuestDecorationMap.get(event.getIconID(), '')
            return packWTBonus(preFmtCond, isSpecialMission, decorationIcon)
