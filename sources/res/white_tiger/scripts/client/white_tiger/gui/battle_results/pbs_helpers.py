...
def packAchievementTooltipData(achievement, tooltipData, tooltipIdx):
    tooltipData[str(tooltipIdx)] = TooltipData(tooltip=None, isSpecial=True, specialAlias=TOOLTIPS_CONSTANTS.BATTLE_STATS_ACHIEVS, specialArgs=[
...
def isWTAchievement(achievementData):
    if achievementData is None:
        ...
        return achievementData[0].getName() == WT_PROGRESSION_ACHIEVEMENT
...
def _getTokenCount(reusable, tokenName):
    ...
    if tokenName not in questTokens:
        return 0
    return questTokens[tokenName].get('diff', 0)
