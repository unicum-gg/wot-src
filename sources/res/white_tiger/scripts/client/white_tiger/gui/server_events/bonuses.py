...
class TicketTokensBonus(TokensBonus):
    ...
    def formatValue(self):
        ...
        amount = sum(data.get('count', 0) for tokenID, data in viewitems(self._value) if tokenID == ticketName)
        if bool(amount):
            return amount
        else:
            return
...
class StampTokensBonus(TokensBonus):
    ...
    def formatValue(self):
        ...
        amount = sum(data.get('count', 0) for tokenID, data in viewitems(self._value) if tokenID == stampName)
        if bool(amount):
            return amount
        else:
            return
...
class WTLootBoxBonus(LootBoxTokensBonus):
    ...
    def formatValue(self):
        amount = sum(data.get('count', 0) for data in viewvalues(self._value))
        return amount or 0
