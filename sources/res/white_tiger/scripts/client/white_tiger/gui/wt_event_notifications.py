    ...
    def __onServerSettingsChange(self, diff):
        ...
        if IS_LOOT_BOXES_ENABLED in diff:
            if self.__lootBoxesCtrl.isEnabled():
                self.__systemMessages.proto.serviceChannel.pushClientMessage({'text': backport.text(self._STR_RES.lootboxes.switchOn.body())}, SCH_CLIENT_MSG_TYPE.WT_SWITCH_ON_LOOTBOXES_NOTIFICATIONS)
            else:
