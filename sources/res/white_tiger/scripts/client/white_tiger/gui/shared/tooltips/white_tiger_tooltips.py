    ...
    def _packBlocks(self, *args, **kwargs):
        ...
        if self.__wtCtrl.isAvailable():
            blocks.append(formatters.packBuildUpBlockData([
             self.__packHeader(),
             packTimeTableHeaderBlock(SELECTOR_BATTLE_TYPES.WHITE_TIGER),
             formatters.packBuildUpBlockData(packCalendarBlock(self.__wtCtrl, time_utils.getCurrentTimestamp(), SELECTOR_BATTLE_TYPES.WHITE_TIGER))]))
        return blocks

    def __packHeader(self):
        currentSeason = self.__wtCtrl.getCurrentSeason()
        if not currentSeason:
            return self.__packNextSeasonHeader(self.__wtCtrl.getNextSeason())
        return self.__packCurrentSeasonHeader(currentSeason)
