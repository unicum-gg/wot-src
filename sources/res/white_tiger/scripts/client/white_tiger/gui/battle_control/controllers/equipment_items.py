...
class _WTItem(_TriggerItem):
    ...
    def getAnimationType(self):
        ...
        if self._stage == EQUIPMENT_STAGES.ACTIVE:
            return ANIMATION_TYPES.MOVE_GREEN_BAR_DOWN
        return super(_WTItem, self).getAnimationType()

    def canActivate(self, entityName=None, avatar=None):

        def isInCooldownStage():
            return self._timeRemaining > 0 and self._stage in (
        ...
        def isTeleportationInProgress():
            teleportCtrl = self._sessionProvider.dynamic.getControllerByID(BATTLE_CTRL_ID.WT_BATTLE_GUI_CTRL)
            return teleportCtrl and teleportCtrl.teleportationInProgress

        if self._isLocked:
            result = False
            error = _ActivationError('wtEquipmentRestriction', None)
            return (
             result, error)
        else:
            if isInCooldownStage() or isUnavailable() or isTeleportationInProgress():
                result = False
...
class _WTStrongholdItem(_WTItem):
    ...
    def canActivate(self, entityName=None, avatar=None):
        if self._isLocked:
            ...
            if self._stage == EQUIPMENT_STAGES.ACTIVE or self._stage == EQUIPMENT_STAGES.READY:
                return (True, None)
            return super(_WTStrongholdItem, self).canActivate(entityName, avatar)
...
class _TeleportItem(_WTItem):
    ...
    def __init__(self, descriptor, quantity, stage, timeRemaining, _, tags=None):
        ...
        self.teleportPoints = [ {'guid': udo.guid, 'position': (udo.position.x, udo.position.z), 'position3D': (udo.position.x, udo.position.y, udo.position.z), 'yaw': udo.yaw, 'index': udo.positionNumber} for udo in BigWorld.userDataObjects.values() if isinstance(udo, TeleportKeyPoint)
                              ]
    ...
    def update(self, quantity, stage, timeRemaining, totalTime):
        ...
        if stage != EQUIPMENT_STAGES.PREPARING and teleport is not None and teleport.isSpawnPointsVisible:
            teleport.closeSpawnPoints()
            teleport.updateTeleportationProgress()
        return
...
class _HyperionItem(_WTItem):
    ...
    def getAnimationType(self):
        if self._stage == EQUIPMENT_STAGES.COOLDOWN:
            return ANIMATION_TYPES.SHOW_COUNTER_GREEN
        return super(_HyperionItem, self).getAnimationType()
