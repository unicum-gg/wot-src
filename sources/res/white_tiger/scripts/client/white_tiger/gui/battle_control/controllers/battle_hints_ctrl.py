    ...
    def showHint(self, hintName, data=None):
        ...
        if hint:
            if 'param1' in data and hintName.startswith(self._OVERTIME_HINT_PREFIX):
                data['remainingTime'] = float(data['param1'])
            if hintName.startswith('wtBattleStart_hunters') or hintName.startswith('wtBossTeleport'):
                value = ''
                if 'param1' in data:
                    value = data['param1']
                data['bossName'] = backport.text(R.strings.white_tiger.teleportBossName.dyn(value)())
            if self.__currentComponent:
                ...
                for view in self.__iterComponentsByAlias(hint.componentAlias):
                    ...
                    hintDuration = None
                    if data.get('param1') and (hintName.startswith('wtOvertime_hunter') or hintName.startswith('wtOvertime_boss')):
                        hintDuration = float(data.get('param1'))
                        if not hintDuration:
                            LOG_WARNING('No remainTime for overtime hint! Use default value from white_tiger/gui/battle_hints.xml')
