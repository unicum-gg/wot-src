...
class _EconomicsConfig(namedtuple('_ProgressionConfig', ('ticketToken', 'quickBossTicketToken', 'quickHunterTicketToken',
                                  'ticketsToDraw', 'progression', 'stampsPerProgressionStage',
                                  'stamp', 'mainRewardTypeFilter'))):
    __slots__ = ()
...
class EconomicsController(IEconomicsController, IGlobalListener, SeasonProvider):
    ...
    def onLobbyInited(self, event):
        super(EconomicsController, self).onLobbyInited(event)
    ...
    def getLastProgressionStepID(self):
        ...
        if progression:
            return self.getEconomicsSettings().progression[(-1)]['quest']
        return ''
    ...
    def getFinishedLevelsCount(self):
        ...
        return min(stampsCount // stampsPerLevel, totalLevels)
    ...
    def __selectFirstVehicleByTag(self, tags, condition):
        ...
        wtVehicles = {vehicle.invID:vehicle for vehicle in self.__itemsCache.items.getVehicles(criteria=criteria).values()}
