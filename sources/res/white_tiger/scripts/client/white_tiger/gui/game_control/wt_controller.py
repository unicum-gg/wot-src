...
PERFORMANCE_GROUP_LIMITS = {WtPerfProblems.HIGH_RISK: [{WtLimitType.SYSTEM_DATA: {'clientBit': 1}}, {WtLimitType.SYSTEM_DATA: {'osBit': 1, 'graphicsEngine': 0}}, {WtLimitType.HARDWARE_PARAMS: {HARDWARE_SCORE_PARAMS.PARAM_GPU_MEMORY: 490}}, {WtLimitType.SYSTEM_DATA: {'graphicsEngine': 0}, WtLimitType.HARDWARE_PARAMS: {HARDWARE_SCORE_PARAMS.PARAM_RAM: 2900}}], WtPerfProblems.MEDIUM_RISK: [{WtLimitType.HARDWARE_PARAMS: {HARDWARE_SCORE_PARAMS.PARAM_GPU_SCORE: 150}}, {WtLimitType.HARDWARE_PARAMS: {HARDWARE_SCORE_PARAMS.PARAM_CPU_SCORE: 50000}}]}

@lru_cache()
def _getSpecialBossCD():
    return VehicleDescr(typeName='ussr:R232_IS-7G').type.compactDescr
...
class WhiteTigerController(IWhiteTigerController, Notifiable, SeasonProvider, IGlobalListener):
    ...
    def showIntroVideo(self, onVideoClosed=None):
        showVideo('wt_intro', onVideoClosed)
    ...
    def showOutroVideo(self):
        showVideo('wt_outro', partial(self.accountSettings.setOutroVideoViewed, status=True))
    ...
    def updateArenaBans(self, isOnlyInit=False):
        ...
        wtBans = tuple(b for b in arenaBans.itervalues() if wtSimple in b.get('bonusTypes', ()) or wtGold in b.get('bonusTypes', ()))
        if wtBans:
            ban = max(wtBans, key=lambda b: b.get('expiryTime', 0))
