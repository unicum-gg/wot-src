...
class SoundSystem(CGF.System):
    ...
    def onMappingUnloaded(self):
        for vehicle in self.reaction(self.VehicleIterate):
            if not vehicle.isDestroyed and hasattr(vehicle, 'appearance') and vehicle.appearance is not None:
    ...
    def onExitVehicleSound(self, sound):
        ...
        if not vehicle:
            ...
            soundObjIndex = sound.getSoundObjectIndex()
            if vehicle.isDestroyed or not hasattr(vehicle, 'appearance') or vehicle.appearance is None:
    ...
    def __onVehicleEnterWorld(self, vehicle):
        ...
        if vehicleId in self.__pendingVehicles:
            ...
            _logger.info('Play postponed sound for vehicleID=%s, callback=%s, arguments=%s', vehicleId, cb, args)
            cb(vehicle, *args)
