...
class ClientWhiteTigerBattleMode(wt_constants.WhiteTigerBattleMode):
    ...
    def _client_controlModes(self):
        return {CTRL_MODE_NAME.MAP_CASE: (
                                   HyperionMapCaseControlMode, 'strategicMode', CTRL_TYPE.USUAL), 
           CTRL_MODE_NAME.LOOK_AT_KILLER: (
                                         WTLookAtKillerMode, 'killCamMode', CTRL_TYPE.USUAL)}
...
def preInit():
    ...
    registerLobbyTooltipsBuilders([
     (
      'white_tiger.gui.Scaleform.daapi.view.tooltips.tooltip_builders', WHITE_TIGER_BATTLES_SET)])
...
def init():
    ...
    g_overrideScaleFormViewsConfig.initExtensionBattlePackages(__name__, [
     'white_tiger.gui.Scaleform.daapi.view.battle'], wt_constants.ARENA_GUI_TYPE.WHITE_TIGER)
    g_overrideScaleFormViewsConfig.initExtensionLobbyPackages(__name__, [
     'white_tiger.gui.Scaleform.daapi.view.lobby',
     'white_tiger.gui.impl.lobby'])
