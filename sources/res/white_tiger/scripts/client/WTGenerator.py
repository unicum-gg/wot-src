...
class WTGenerator(Capturable):
    ...
    def set_activationProgress(self, prev):
        ...
        if activation.progress > 0:
            ...
            gameObject.removeComponent(WTGeneratorProgressComponent)
        return
    ...
    def updateFirstDamaged(self, vIds):
        for veh in [ BigWorld.entities[vId] for vId in vIds if BigWorld.entities.has_key(vId) ]:
            activation = veh.entityGameObject.findRead(WTGeneratorActivationComponent)
    ...
    def activationAdd(self, added):
        ...
        for veh in [ BigWorld.entities[vId] for vId in added if BigWorld.entities.has_key(vId) ]:
            queue.createComponent(veh.entityGameObject, WTGeneratorActivationComponent, gameObject)

    def activationRemove(self, removed):
        for veh in [ BigWorld.entities[vId] for vId in removed if BigWorld.entities.has_key(vId) ]:
            veh.entityGameObject.removeComponent(WTGeneratorActivationComponent)
    ...
    def __createProgressComponent(self, gameObject):
        if gameObject.findRead(WTGeneratorProgressComponent) is not None:
            return
        else:
            queue = CGF.CommandQueue(gameObject.spaceID)
            queue.createComponent(gameObject, WTGeneratorProgressComponent, lambda : self.activationProgress.progress, lambda : self.activationProgress.timeLeft)
            return

    def __createCapturedComponent(self, gameObject):
        if gameObject.findRead(WTGeneratorCapturedComponent) is not None:
            return
        else:
            queue = CGF.CommandQueue(gameObject.spaceID)
            queue.createComponent(gameObject, WTGeneratorCapturedComponent, self.activationProgress.invadersVehicleIDs)
            return
