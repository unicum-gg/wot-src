...
class WTPlasmaBonusComponent(DynamicScriptComponent):
    ...
    def getPlasmaBonusForVehicle(self, vehicleID):
        return self.__plasmaVehicleBonuses.get(vehicleID, 0)

    def getPlasmaBonusMultiplier(self, plasmaBonus):
        if plasmaBonus in self.plasmaBonusLevelMultipliers:
    ...
    def __notifyPlayersPlugin(self):
        ...
        if vehicleID in self.__plasmaVehicleBonuses and ctrl:
            ctrl.notifyStateChanged(VEHICLE_VIEW_STATE.PLASMA, (
             self.__plasmaVehicleBonuses[vehicleID],
             self.plasmaBonusLevelMultipliers[self.__plasmaVehicleBonuses[vehicleID]]))
    ...
    def __isPlasmaBonusChanged(self, vehicleID, previousPlasmaCountDict):
        if not previousPlasmaCountDict or vehicleID not in previousPlasmaCountDict or not self.__plasmaVehicleBonuses:
            return True
        return previousPlasmaCountDict[vehicleID] != self.__plasmaVehicleBonuses[vehicleID]