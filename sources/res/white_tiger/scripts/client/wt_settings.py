...
class _WhiteTigerConfig(namedtuple('WhiteTigerConfig', (
 'isEnabled',
 'peripheryIDs',
 'primeTimes',
 'seasons',
...
_VehicleData = namedtuple('VehicleData', ('vehicle', 'equipments', 'type', 'subType', 'crew', 'tokenForPlay',
 'isBoss', 'isSpecialBoss', 'isHunter', 'canShowInHangar'))
...
class WTConfig(object):
    ...
    def getTokenDataByName(self, tokenName):
        ...
        return {'tokenDraw': tokenDraw, 'currentCount': self.__itemsCache.items.tokens.getTokenCount(tokenName)}
    ...
    def getTokensForBattle(self, vehicleCD):
        tokenForPlay = self.__vehiclesCachedData[vehicleCD].tokenForPlay
        if tokenForPlay:
            return self.__itemsCache.items.tokens.getTokenCount(tokenForPlay['name'])
        return -1
    ...
    def __getVehicleData(self, vehicleCD, data, isBoss=False, isSpecialBoss=False, isHunter=False):
        ...
        if not data['tankManData']:
            ...
            return _VehicleData(vehicle, data['equipments'], data['type'], data['subType'], Tankman.Tankman(tankmanCompDescr, vehicle=vehicle), data.get('tokenForPlay', {'name': '', 'drawForBattle': 0}), isBoss, isSpecialBoss, isHunter, canShowInHangar)
