import logging
from constants_utils import initCommonTypes, addRosterTypes, addInvitationTypes, addBattleEventTypesFromExtension
...
from white_tiger.gui.battle_results.composer import WhiteTigerStatsComposer
...
from white_tiger.gui.prb_control import registerWtOtherParams
...
class ClientWhiteTigerBattleMode(wt_constants.WhiteTigerBattleMode):
    ...
    def _client_gameControllers(self):
        from skeletons.gui.game_control import IWhiteTigerController, ILootBoxesController, IWhiteTigerSettingsController
        from white_tiger.gui.game_control.wt_controller import WhiteTigerController
        from white_tiger.gui.game_control.loot_boxes_controller import LootBoxesController
        from white_tiger.gui.game_control.wt_settings_controller import WtSettingsController
        return [
         (
          IWhiteTigerController, WhiteTigerController, False),
         (
          ILootBoxesController, LootBoxesController, False),
         (
          IWhiteTigerSettingsController, WtSettingsController, False)]
    ...
    def _client_messengerClientFormatters(self):
        ...
        cmt = gui_constants.SCH_CLIENT_MSG_TYPE
        return {cmt.WT_EVENT_STATE: formatters.WTEventStateMessageFormatter(), 
           cmt.WT_ARENA_BAN_NOTIFICATIONS: formatters.WTArenaBanSystemMessageFormatter(), 
           cmt.WT_ARENA_ATTENTION_NOTIFICATIONS: formatters.WTArenaWarningSystemMessageFormatter(), 
           cmt.WT_SWITCH_ON_LOOTBOXES_NOTIFICATIONS: formatters.WTLootBoxesNotificationsFormatter(), 
           cmt.WT_PROGRESSION: formatters.WTEventProgressionFormatter()}
    ...
    def _client_notificationActionHandlers(self):
        ...
        return (handlers.WTOpenPortalsHandler,
         ...
         handlers.WTFairPlayActionHandler,
         handlers.WTWarningFairPlayActionHandler)
    ...
    def _client_battleResultsComposerClass(self):
        return WhiteTigerStatsComposer
    ...
    def registerAdditionalSystemMessageTypes(self):
        ...
        SM_TYPE.inject(['WTLootBoxRewards', 'WTLootBoxRerollRewards'])
...
def preInit():
    ...
    addBattleEventTypesFromExtension(wt_constants.BATTLE_EVENT_TYPE, __name__)
    ...
    registerBattleResultsComposer(wt_constants.ARENA_BONUS_TYPE.WHITE_TIGER_2, WhiteTigerStatsComposer)
    ...
    arenaRanges = (
     wt_constants.ARENA_BONUS_TYPE.WHITE_TIGER, wt_constants.ARENA_BONUS_TYPE.WHITE_TIGER_2)
    ...
    _MODENAME_TO_PO_FILE.update({gui_constants.SELECTOR_BATTLE_TYPES.WHITE_TIGER: gui_constants.SELECTOR_BATTLE_TYPES.WHITE_TIGER})
    registerHangarPresetGetter(QUEUE_TYPE.WHITE_TIGER, WtEventPresetsGetter)
    registerWtOtherParams()
    from AvatarInputHandler import _CTRLS_DESC_MAP
    from aih_constants import CTRL_MODE_NAME
    from white_tiger.avatar_input_handler.wt_map_case_mode import HyperionMapCaseControlMode
    _CTRLS_DESC_MAP[CTRL_MODE_NAME.MAP_CASE_HYPERION] = (
     HyperionMapCaseControlMode,
     'strategicMode',
     aih_constants.CTRL_TYPE.USUAL)


def init():
    _logger.debug('init: %r', __name__)
    LOBBY_EXT_PACKAGES = [
     'white_tiger.gui.Scaleform.daapi.view.lobby',
     'white_tiger.gui.Scaleform.daapi.view.lobby.header']
    ...
    g_overrideScaleFormViewsConfig.initExtensionBattlePackages(__name__, [
     'white_tiger.gui.Scaleform.daapi.view.battle.white_tiger'], wt_constants.ARENA_GUI_TYPE.WHITE_TIGER)
