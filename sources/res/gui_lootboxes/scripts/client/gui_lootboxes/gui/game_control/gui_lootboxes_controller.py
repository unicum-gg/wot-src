...
class GuiLootBoxesController(IGuiLootBoxesController):
    __slots__ = ('__em', '__boxesCount', '__isLootBoxesAvailable', '__isActive', '__bonusesConfig',
                 '__hangarOptimizer', '__boxKeysCount')
    ...
    def openShop(self, lootboxID=None):
        if self.isBuyAvailable():
            ...
            if lootBox is not None:
                handler = self.__shopWindowHandler.get(lootBox.getType(), None) or self.__shopWindowHandler.get(lootBox.getCategory(), None)
            if callable(handler):
                handler()
            else:
    ...
    def __updateLastAddedLootBox(self):
        ...
        lootBoxIdToCountStore = {lootBox.getID():lootBox.getInventoryCount() for lootBox in lootBoxes if lootBox.getInventoryCount() > 0 if lootBox.getInventoryCount() > 0}
