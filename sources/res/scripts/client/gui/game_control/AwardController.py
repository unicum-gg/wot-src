        ...
        for qID in data.get('completedQuestIDs', set()):
            if qID in allQuests:
                if self.isShowCongrats(allQuests[qID]):
                    ...
                    windowCtx = {'eventsCache': self.eventsCache, 'bonusVehicles': vehiclesDict}
...
class SeniorityAwardsWindowHandler(ServiceChannelHandler):
    ...
    def _showAward(self, ctx=None):
        if self.__mergedRewards:
            vehicles = self.__mergedRewards.get('vehicles', [])
...
class CrewBooksQuestHandler(MultiTypeServiceChannelHandler):
    ...
    def _needToShowAward(self, ctx):
        ...
        if res:
            ...
            res = res and any(isCrewBook(intCD) for intCD in message.data['items'])
...
class CustomizationRewardHandler(MultiTypeServiceChannelHandler):
    ...
    def _getAttachments(self, ctx):
        ...
        items = messageData['data'].get('customizations', []) if 'data' in messageData else []
        detailedRewards = messageData.get('detailedRewards', {})
        for questID in messageData.get('completedQuestIDs', set()):
            if questID in detailedRewards and self._isAllowedByQuest(questID):
                questDetailedRewards = detailedRewards[questID]
                if 'customizations' in questDetailedRewards:
                    items.extend(questDetailedRewards['customizations'])
...
class PrestigeMilestonesAwardWindowHandler(ServiceChannelHandler):
    ...
    def __parseMessage(message):
        reward = message.data.get('reward')
        if not reward:
            return
        else:
            vehCD = reward.get('vehTypeCompDescr')
    ...
    def __parseReward(message):
        ...
        if not reward:
            ...
            styleID = reward.get('id')
            custType = reward.get('custType')
...
class BattleQuestsAutoWindowHandler(MultiTypeServiceChannelHandler):
    ...
    def _getSysMsgTypes(self):
        return (
         SYS_MESSAGE_TYPE.battleResults.index(), SYS_MESSAGE_TYPE.personalMissionRebalance.index())
        ...
        for uniqueQuestID in completedQuestUniqueIDs:
            ...
            if questID in allQuests:
                ...
                if self.isShowCongrats(quest):
                    ...
                    vehiclesDict = vehiclesList[0] if vehiclesList else {}
                    ctx.update({'eventsCache': self.eventsCache, 'bonusVehicles': vehiclesDict})
    ...
    def _getContext(uniqueQuestID, completedQuests, completedQuestUniqueIDs):
        if personal_missions.g_cache.isPersonalMission(uniqueQuestID):
            ...
            if pqType.id not in completedQuests:
                ctx = {'isMainReward': pqType.mainQuestID in completedQuestUniqueIDs, 'isAddReward': pqType.addQuestID in completedQuestUniqueIDs, 
...
class PersonalMissionByAwardListHandler(PersonalMissionAutoWindowHandler):
    ...
    def _getContext(uniqueQuestID, completedQuests, completedQuestUniqueIDs):
        if personal_missions.g_cache.isPersonalMission(uniqueQuestID):
            pqType = personal_missions.g_cache.questByUniqueQuestID(uniqueQuestID)
            if pqType.id not in completedQuests:
                ctx = {'isMainReward': True, 'isAddReward': False, 
...
class PersonalMissionOperationAwardHandler(BattleQuestsAutoWindowHandler):
    ...
    def _showAward(self, ctx):
        ...
        for operationId, prefix in self.__getFinalTokenQuestIdsByOperationId():
            ...
            if quests:
                ctx = {'operationID': operationId, 'branch': getBranchByOperationId(operationId), 
...
class RankedQuestsHandler(ServiceChannelHandler):
    ...
    def __showSeasonAward(self, quest, data):
        ...
        if season is not None:
            showRankedSeasonCompleteView({'quest': quest, 'awards': data}, True)
        else:
...
class MapboxProgressionRewardHandler(AwardHandler):
    ...
    def _showAward(self, ctx):
        _, message, __ = ctx
        bonuses = chain.from_iterable(getServiceBonuses(name, value) for name, value in iteritems(message['savedData'].get('rewards', {})))
...
class ClanSupplyPurchaseHandler(ServiceChannelHandler):
    __purchaseCache = dependency.descriptor(IPurchaseCache)
    REGULAR_TAG = 'clansupply.map_product'
    ELITE_TAG = 'clansupply.map_product_elite'
    CLAN_SUPPLY_TAGS = (
    ...
    def _showAward(self, ctx):
        _, message = ctx
        invoiceData = message.data
        for tag in invoiceData.get('tags', []):
    ...
    def _needToShowAward(self, ctx):
        ...
        invoiceData = message.data
        metaData = invoiceData.get('meta', {})
        return invoiceData.get('assetType', 0) == INVOICE_ASSET.PURCHASE and 'data' in invoiceData and 'tags' in invoiceData and metaData.get('type') == 'normal'
    ...
    def __tryToShowAwards(self, invoiceData, isElite):
        ...
        if not rewards:
            _logger.info('Reward list is empty, clan supply awards window will not be shown for purchase %s', productCode)
            return
...
class DailyQuestHandlerBase(MultiTypeServiceChannelHandler):
    ...
    def __fillCompletedQuests(self, message):
        ...
        if qIDs:
            self.quests.update(sorted([ (qID, data.get('detailedRewards', {}).get(qID)) for qID in qIDs ], key=lambda item: getIdxFromQuestID(item[0])))
            if message.type == SYS_MESSAGE_TYPE.battleResults.index():
...
class DailyQuestHandler(DailyQuestHandlerBase):
    ...
    def _showAward(self, ctx):
        for quest, questBonuses in self.quests.items():
            window = WinbackRewardWindow(ctx={'quests': [
                        quest], 
               'bonuses': questBonuses, 
               'isOnlyDaily': True})
...
class WinbackQuestHandler(DailyQuestHandlerBase):
    ...
    def _showAward(self, ctx):
        ...
        for bonusesIndex, bonuses in enumerate(splittedBonuses):
            ...
            window = WinbackRewardWindow(ctx={'quests': list(quests)[fromIdx:toIdx], 
               'bonuses': bonuses, 
               'isOnlyDaily': False, 
    ...
    def _getMainBonusesList(self, bonuses):
        ...
        for bonusName, bonusData in bonuses.items():
            ...
            elif bonusName == 'tokens':
                result += [ {'tokens': {tokenName: bonusData.get(tokenName)}} for tokenName in bonusData.keys() if self.winbackController.isWinbackOfferToken(tokenName)
                          ]
            elif bonusName == VehiclesBonus.VEHICLES_BONUS:
                result += [ {VehiclesBonus.VEHICLES_BONUS: {vehicleCD: vehicleData}} for vehicleBlock in bonusData for vehicleCD, vehicleData in iteritems(vehicleBlock) if vehicleData.get('compensatedNumber', 0) <= 0
                          ]
            elif bonusName == BlueprintBonusTypes.BLUEPRINTS:
                result += self._getDiscounts(bonuses)
            elif bonusName == 'slots':
...
class PMAdvancedOperationAwardHandler(MultiTypeServiceChannelHandler):
    ...
    def _showAward(self, ctx, clientCtx=None):
        _, message = ctx
        context = {'questID': None, 'rewards': {}, 'type': REWARDS_VIEW_TYPES['operationWithHonor']}
        completedQuests = message.data.get('completedQuestIDs', set())
...
class PMAdvancedVehicleDetailHandler(MultiTypeServiceChannelHandler):
    ...
    def _showAward(self, ctx, clientCtx=None):
        _, message = ctx
        context = {'questID': None, 'rewards': {}, 'type': REWARDS_VIEW_TYPES['vehicleDetail']}
        completedQuests = message.data.get('completedQuestIDs', set())
...
class ChallengesFailHandler(ServiceChannelHandler):
    ...
    def _showAward(self, ctx):
        _, message = ctx
        completedQuest = message.data.get('completedQuestID')
        challengeID, missionID = parseChallengeQuestId(completedQuest)
        challenge = self.__challenges.getChallenge(challengeID)
        if challenge is None:
            return
        else:
            attempts = message.data.get('attemptsLeft') or 0
            if attempts == 0:
                gfTemplate = GFNotificationTemplates.CHALLENGES_FAIL_NOTIFICATION
                messageData = {'challengeName': challenge.name, 'challengeID': challengeID}
            else:
                messageData = {'missionID': str(missionID), 
                   'attempts': attempts, 
...
class ChallengesWinHandler(ServiceChannelHandler):
    ...
    def _showAward(self, ctx):
        _, message = ctx
        quests = message.data.get('completedQuestIDs', set())
        ...
        challenge = self.__challenges.getChallenge(challengeID)
        rewards = message.data.get('detailedRewards', {}).get(completedQuest) or {}
        if challenge is None:
            return
        else:
            if len(challenge.questsIDs) == missionIndex and challenge.getTokenID(ChallengeTokenType.WIN) in rewards.get('tokens', {}):
                if challenge.mainRewardType != ChallengeMainRewardTypes.EMPTY:
                    ...
                    SystemMessages.pushMessage(text=formatted, type=SystemMessages.SM_TYPE.ChallengesKingReward, messageData={'header': header})
                else:
                    messageData = {'challenge': challenge, 'rewards': rewards}
                    gfTemplate = GFNotificationTemplates.CHALLENGES_CHALLENGE_COMPLETED_NOTIFICATION
                    pushGFNotification(gfTemplate, data=messageData)
            else:
                messageData = {'missionID': str(missionIndex), 'rewards': rewards, 'challenge': challenge}
