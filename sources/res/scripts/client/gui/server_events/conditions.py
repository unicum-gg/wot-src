...
class DailyXPFactor(_Requirement):
    ...
    def __init__(self, path, data):
        super(DailyXPFactor, self).__init__('dailyXPFactor', dict(data), path)
        self._relation = _findRelation(self._data.keys())
        self._relationValue = float(_getNodeValue(self._data, self._relation))
        ...
        if bonus is None:
            ...
            if groupBy is None:
                ...
                if isCurrentProgress:
                    ...
                    result[None] = (current, total, diff, self.__isProgressCompleted(curProg))
                else:
                    diff = self.__getProgDiff(curProg, prevProg)
                    result[None] = (min(curProg.get(key, 0), total), total, diff, self.__isProgressCompleted(curProg))
            else:
