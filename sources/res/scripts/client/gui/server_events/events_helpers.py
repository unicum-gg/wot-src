...
def isSuitableForPM(diff):
    ...
    filteredPMTokenQuests = {qID for qID in diff.get('quests', {}).iterkeys() if any([ qID.startswith(prefix) for prefix in NO_AWARD_LIST_QUEST_PREFIXES ]) and not isPMNoAwardListMilestone(qID) or re.match(isPMQuestRegExp, qID) if any([ qID.startswith(prefix) for prefix in NO_AWARD_LIST_QUEST_PREFIXES ]) and not isPMNoAwardListMilestone(qID) or re.match(isPMQuestRegExp, qID)}
    ...
    if 'tokens' in diff:
        ...
        pmTokens = {token for token in tokens if token.startswith(PT_TOKEN_PREFIX) or token.startswith(FINAL_PT_TOKEN_PREFIX) or isPMPoints(token) if token.startswith(PT_TOKEN_PREFIX) or token.startswith(FINAL_PT_TOKEN_PREFIX) or isPMPoints(token)}
