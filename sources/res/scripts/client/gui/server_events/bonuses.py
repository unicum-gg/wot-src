...
class CurrenciesBonus(IntegralBonus):
    ...
    def formattedList(self):
        return [
         i18n.makeString('#quests:bonuses/%s/description' % self._code, value=self.formatValue())]
...
class _PremiumDaysBonus(IntegralBonus):
    ...
    def getIconBySize(self, size):
        ...
        if iconName is None:
            iconName = RES_ICONS.getBonusIcon(size, ('{}_{}').format(self.getName(), 'universal'))
        return iconName
...
class LootBoxTokensBonus(TokensBonus):
    ...
    @property
    def lootBoxID(self):
        return int(self.tokenID.split(':')[1])
        ...
        for tokenID, tokenVal in self._value.iteritems():
            ...
            if lootBox is not None:
                if lootBox.getType() in self.lootBoxSystem.getActiveEvents():
                    ...
                    result.append(makeHtmlString('html_templates:lobby/quests/bonuses', 'lootBox', {'lootBoxType': lootBox.getType(), 'value': tokenVal['count']}))
...
class PersonalMissionsPointsTokensBonus(TokensBonus):
    ...
    def formatValue(self):
        return self.getValue().get(findFirst(isPMPoints, self.getValue(), '')).get('count', 0)
...
class PersonalMissionsDailyPointsTokensBonus(PersonalMissionsPointsTokensBonus):
    ...
    def isShowInGUI(self):
        ...
        if not all(serverSettings.isPersonalMissionsEnabled(branchName) for branchName in branches):
            return False
        else:
            isWithoutAwardListCampaignSelected = personalMissions.isBranchWithoutAwardListActive()
            if not isWithoutAwardListCampaignSelected:
                return False
            withoutAwardListActiveOperations = personalMissions.getActiveOperations(branches)
            if not withoutAwardListActiveOperations:
                return False
            currentOperation = first(operation for operation in withoutAwardListActiveOperations if operation.getBranchName() in personalMissions.getActiveCampaigns())
            ...
            isDisabled = currentOperation.isDisabled() if currentOperation is not None else False
            shouldBeVisible = isWithoutAwardListCampaignSelected and withoutAwardListActiveOperations and not isOperationCompleted and not isProgressionCompleted and not isDisabled
...
def personalMissionsTokensFactory(name, value, isCompensation=False, ctx=None):
    ...
    withoutAwardListBranchesOperations = list(chain.from_iterable(PM_BRANCH.BRANCH_TO_OPERATION_IDS[branch] for branch in PM_BRANCH.convertNameToType(PM_BRANCH.WITHOUT_AWARD_LIST_BRANCHES)))
    completionTokenID = PERSONAL_MISSION_TOKEN % (ctx['campaignID'], ctx['operationID']) if ctx['operationID'] not in withoutAwardListBranchesOperations else ''
    result = []
    operationsIds = set(PM_BRANCH.BRANCH_TO_OPERATION_IDS[PM_BRANCH.PERSONAL_MISSION_3] + PM_BRANCH.BRANCH_TO_OPERATION_IDS[PM_BRANCH.PERSONAL_MISSION_4])
    for tID, tValue in value.iteritems():
        ...
        elif tID.startswith(RECRUIT_TMAN_TOKEN_PREFIX) and ctx['operationID'] in operationsIds:
        ...
        for item, count in self.getItems().iteritems():
            if item is not None and count:
                ...
                if item.itemTypeID == GUI_ITEM_TYPE.OPTIONALDEVICE and not item.isRegular:
                    itemInfo['overlayIcon'] = {AWARDS_SIZES.SMALL: item.getBonusOverlay(AWARDS_SIZES.SMALL), AWARDS_SIZES.BIG: item.getBonusOverlay(AWARDS_SIZES.BIG)}
        ...
        for item, count in self.getMentoringLicenses().iteritems():
            if item is not None:
                result.append({'id': item.intCD, 
                   ...
                   'value': count, 
                   'icon': {AWARDS_SIZES.SMALL: item.iconInfo, AWARDS_SIZES.BIG: item.bigIcon}, 
                   'name': item.userName, 
...
class LootBoxRandomNationalBonus(SimpleBonus):
    __ICON_NAMES = {LOOTBOX_RANDOM_NATIONAL_BLUEPRINT: 'random_national', 
       LOOTBOX_RANDOM_NATIONAL_BROCHURE: 'booklet_random', 
       LOOTBOX_RANDOM_NATIONAL_GUIDE: 'guide_random', 
       LOOTBOX_RANDOM_NATIONAL_CREW_BOOK: 'crewBook_random'}
    ...
    def getIconName(self):
        return self.__ICON_NAMES.get(self._name, '')
    ...
    def _getWrappedBonusList(self):
        ...
        return [
         {'id': 0, 
            'type': self.getName(), 
            'value': self.getCount(), 
            'icon': {AWARDS_SIZES.SMALL: backport.image(imageRes.small.dyn(self.getIconName())()), AWARDS_SIZES.BIG: backport.image(imageRes.big.dyn(self.getIconName())())}, 
            'name': backport.text(awardItem.header()) if awardItem else '', 
            'description': backport.text(awardItem.body()) if awardItem else ''}]
...
def getSplitBonusFunction(bonus):
    if not getattr(bonus, 'isSplittable', True):
        return
    else:
