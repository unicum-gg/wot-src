...
from debug_utils import LOG_CURRENT_EXCEPTION, LOG_ERROR, LOG_CODEPOINT_WARNING
...
from optional_bonuses import BONUS_MERGERS, getProbableBonuses
...
class SimpleBonus(object):
    ...
    def isReceived(self):
        LOG_CODEPOINT_WARNING('SimpleBonus::isReceived')
        return False
...
class _PremiumDaysBonus(IntegralBonus):
    ...
    def getIconResource(self, size):
        iconR = R.images.gui.maps.icons.quests.bonuses.dyn(size).dyn(('{}_{}').format(self.getName(), self.getValue()))
        if iconR:
            return iconR()
        return R.invalid()
...
class BasicPremiumDaysBonus(_PremiumDaysBonus):
    ...
    def getList(self):
        return [
         {'itemSource': backport.image(R.images.gui.maps.icons.quests.bonuses.small.premium_1()), 
...
class TokensBonus(SimpleBonus):
    TOKENS = 'tokens'
    _TOKEN_RECORD = namedtuple('_TOKEN_RECORD', [
     'id',
     'expires',
     'count',
     'limit'])
...
class VersusAIProgressionsTokenBonus(TokensBonus):
    ...
    def _format(self, styleSubset):
        ...
        text = makeHtmlString(('html_templates:lobby/quests/{}').format(styleSubset), self.__TEMPLATE_NAME, {'count': formatedValue, 'valueName': preformattedBonus.userName, 'progressionName': progressionName})
        if text != self.__TEMPLATE_NAME:
            return text
...
class TicketTokensBonus(TokensBonus):
    ...
    def formatValue(self):
        ticketNameList = self.__gameEventCtrl.getBossTokenIDList()
        amount = sum([ data.get('count', 0) for tokenID, data in self._value.iteritems() if tokenID in ticketNameList ])
        if bool(amount):
            return amount
        else:
            return
    ...
    def _format(self, styleSubset):
        tokenName = None
        if self._value.keys() and self._value.keys()[0].startswith('wtevent:'):
            tokenName = self._value.keys()[0].split(':')[1]
        formattedValue = self.formatValue()
        if self._name is not None and formattedValue is not None:
            text = makeHtmlString(('html_templates:lobby/quests/{}').format(styleSubset), tokenName if tokenName else self._name, {'value': formattedValue})
            if text != self._name:
                return text
        return formattedValue
...
class MainPrizeDiscountTokensBonus(TokensBonus):
    __gameEventCtrl = dependency.descriptor(IWhiteTigerController)

    def __init__(self, name, value, isCompensation=False, ctx=None):
        super(MainPrizeDiscountTokensBonus, self).__init__(name, value, isCompensation, ctx)
        self._name = 'mainPrizeDiscount'
    ...
    def formatValue(self):
        ...
        amount = sum([ data.get('count', 0) for tokenID, data in self._value.iteritems() if tokenID == mainPrizeDiscountName
                     ])
    ...
    def formattedList(self):
        ...
        for tokenID, tokenVal in self._value.iteritems():
            ...
            if lootBox is not None:
                ...
                wtLootboxesTypes = (
                 WhiteTigerLootBoxes.WT_HUNTER, WhiteTigerLootBoxes.WT_BOSS)
                lootBoxType = lootBox.getType()
                if lootBoxType in wtLootboxesTypes:
                    templateStr = 'engineer' if lootBoxType == WhiteTigerLootBoxes.WT_BOSS else 'hunter'
                    result.append(makeHtmlString('html_templates:lobby/quests/bonuses/wtlootBox', templateStr, {'lootBoxType': lootBox.getType(), 
                       'count': tokenVal['count']}))
                else:
                    result.append(makeHtmlString('html_templates:lobby/quests/bonuses', 'lootBox', {'lootBoxType': lootBox.getType(), 
                       'name': lootBox.getUserName(), 'count': tokenVal['count']}))
...
class LootBoxKeyTokensBonus(TokensBonus):
    ...
    def _getWrappedBonusList(self):
        return [
         {'id': self.__getLootBoxKeyTokenID(), 
...
class TmanTemplateTokensBonus(TokensBonus):
    TANKMAN_RECEVIED_TOKEN_PREFIX = 'got_'
    ...
    def formattedList(self):
        return [ makeHtmlString('html_templates:lobby/quests/bonuses', self._name, {'value': getRecruitInfo(tID).getFullUserNameByNation(0)}) for tID in self.getTokens()
               ]

    def isReceived(self):
        result = []
        for tID in self.getTokens().iterkeys():
            result.append(self.itemsCache.items.tokens.getTokenCount(self.TANKMAN_RECEVIED_TOKEN_PREFIX + tID) > 0)

        return all(result)


class WtTmanTemplateTokensBonus(TmanTemplateTokensBonus):
    WT_TANKMAN_TOKEN_PREFIX = 'tankmen_WT2025_'
    ...
    def formattedList(self):
        result = []
        for tID, _ in self.getTokens().iteritems():
            recruitInfo = getRecruitInfo(tID)
            result.append(makeHtmlString('html_templates:lobby/quests/bonuses', 'tankman', {'crewName': recruitInfo.getFullUserNameByNation(0)}))
...
class X3CrewTokensBonus(TokensBonus):
    ...
    def _format(self, styleSubset):
        return makeHtmlString('html_templates:lobby/quests/bonuses', CREW_BONUS_X3_TOKEN, {'value': self.formatValue()})

    def formatValue(self):
        return self.getValue()[CREW_BONUS_X3_TOKEN]['count']
    ...
    def getWrappedEventLootBoxesBonusList(self):
        ...
        for _, tokenRecord in self.getTokens().iteritems():
            result.append({'id': 0, 
               'type': ItemPackType.CUSTOM_X3_CREW_BONUS, 
               'value': tokenRecord.count, 
               'icon': {AWARDS_SIZES.SMALL: self.getIconBySize(AWARDS_SIZES.SMALL), 
                        AWARDS_SIZES.BIG: self.getIconBySize(AWARDS_SIZES.BIG)}, 
               'name': self.getUserName(), 
...
class CollectionTokenBonus(TokensBonus, ICollectionItemBonus):
    ...
    def _getWrappedBonusList(self):
        return [
         {'id': cllcTokenToEntitlement(first(self.getTokens().keys(), '')), 
            'value': 1, 
            'type': ('custom/{}').format(self.getName()), 
            'name': '', 
            'description': '', 
...
class CollectionEntitlementBonus(EntitlementBonus, ICollectionItemBonus):
    ...
    def __init__(self, name, value, isCompensation=False, ctx=None):
        ...
        value = self.getValue()
        _, _, collectionId, itemId = value.id.split('_')
        self.__collectionId = int(collectionId)
        self.__itemId = int(itemId)
...
class AnyCollectionItemBonus(SimpleBonus):
    ...
    def getIconBySize(self, size):
        if size == AWARDS_SIZES.SMALL:
            return backport.image(R.images.gui.maps.icons.collectionItems.c_48x48.dyn(('any_{}').format(self.getCollectionId()))())
        return backport.image(R.images.gui.maps.icons.collectionItems.c_80x80.dyn(('any_{}').format(self.getCollectionId()))())

    def _getWrappedBonusList(self):
        collRes = R.strings.dyn(COLLECTION_RES_PREFIX + self.__collectionsSystem.getCollection(self.getCollectionId()).name)
        return [
         {'id': self.getCollectionId(), 
...
def tokensFactory(name, value, isCompensation=False, ctx=None):
    ...
    for tID, tValue in value.iteritems():
        if tID.startswith(LOOTBOX_TOKEN_PREFIX):
            ...
            tmanInfo = getRecruitInfo(tID)
            if tmanInfo is not None and tmanInfo.getGroupName().startswith(WtTmanTemplateTokensBonus.WT_TANKMAN_TOKEN_PREFIX):
                result.append(WtTmanTemplateTokensBonus({tID: tValue}, isCompensation, ctx))
            else:
        ...
        elif tID == dependency.instance(IWhiteTigerController).getConfig().ticketToken:
            result.append(TicketTokensBonus(name, {tID: tValue}, isCompensation, ctx))
        elif tID == dependency.instance(IWhiteTigerController).getConfig().vipTicketToken:
            result.append(TicketTokensBonus(name, {tID: tValue}, isCompensation, ctx))
        elif tID == dependency.instance(IWhiteTigerController).getConfig().stamp:
            result.append(StampTokensBonus(name, {tID: tValue}, isCompensation, ctx))
        elif tID == dependency.instance(IWhiteTigerController).getConfig().mainPrizeDiscountToken:
            result.append(MainPrizeDiscountTokensBonus(name, {tID: tValue}, isCompensation, ctx))
        else:
...
def entitlementsFactory(name, value, isCompensation=False, ctx=None):
    ...
    for eID, eValue in value.iteritems():
        if eID.startswith(COLLECTION_ITEM_PREFIX_NAME):
            result.append(CollectionEntitlementBonus(COLLECTION_ITEM_BONUS_NAME, (eID, eValue.get('count', 0)), isCompensation, ctx))
        else:
            result.append(EntitlementBonus(name, (eID, eValue.get('count', 0)), isCompensation, ctx))
...
class LbC11nProgressTokenBonus(C11nProgressTokenBonus):
    BONUS_NAME = 'lbStyleProgress'

    @staticmethod
    def _parseC11nProgressToken(token):
        try:
            _, style, progressLevel = token.id.split(':')
            styleID, branchID = style.split('_')
        ...
        for item, count in self.getItems().iteritems():
            if item is not None and count:
                ...
                result.append({'id': item.intCD, 
                   ...
                   'value': count, 
                   'icon': {AWARDS_SIZES.SMALL: item.getBonusIcon(AWARDS_SIZES.SMALL), AWARDS_SIZES.BIG: item.getBonusIcon(AWARDS_SIZES.BIG)}, 
                   'name': item.userName, 
        ...
        for discount, count in self.getDiscounts().iteritems():
            if discount is not None:
                result.append({'id': discount.discountID, 
                   ...
                   'icon': {AWARDS_SIZES.SMALL: discount.icon, AWARDS_SIZES.BIG: discount.bigIcon}, 
        ...
        for form, count in self.getRecertificationForms().iteritems():
            if form is not None:
                result.append({'id': form.intCD, 
                   ...
                   'value': count, 
                   'icon': {AWARDS_SIZES.SMALL: form.iconInfo, AWARDS_SIZES.BIG: form.bigIcon}, 
                   'name': form.userName, 
    ...
    def getWrappedLootBoxesBonusList(self):
        ...
        for demountKit, count in self.getDemountKits().iteritems():
            if demountKit is not None:
                result.append({'id': demountKit.intCD, 
                   'type': ('demountKit/{}').format(demountKit.demountKitGuiType), 
                   'value': count, 
                   'icon': {AWARDS_SIZES.SMALL: demountKit.getIcon(ICONS_SIZES.X48), AWARDS_SIZES.BIG: demountKit.getIcon(ICONS_SIZES.X80)}, 
                   'name': demountKit.userName, 
                   'description': demountKit.shortDescription})
...
class VehiclesBonus(SimpleBonus):
    ...
    def getCompensation(self):
        compensations = []
        for _, vehInfo in self.getVehicles():
            compensation = vehInfo.get('customCompensation')
            if compensation is not None:
                money = Money(*compensation)
                compensations.append(money.gold)
            else:
                compensations.append(0)
        ...
        if 'noCrew' not in vehInfo:
            ...
            if 'tankmen' in vehInfo:
                for tman in vehInfo['tankmen']:
                    ...
                    elif tman['role'] == Tankman.ROLES.COMMANDER:
...
class TankmenBonus(SimpleBonus):
    _TankmanInfoRecord = namedtuple('_TankmanInfoRecord', [
     'nationID',
     'role',
     'vehicleTypeID',
     'firstNameID',
...
class CustomizationsBonus(SimpleBonus):
    ...
    def getCount(self):
        return sum(v.get('value', 1) for v in self.getCustomizations())
    ...
    def isReceived(self):
        ...
        for customizationItem in self._value:
            c11nItem = self.getC11nItem(customizationItem)
            result.append(bool(c11nItem.fullCount()))
...
class BoxBonus(SimpleBonus):
    ...
    def getOptionalBonusesWithProbability(self):
        name = self._name
        probability, bonuses = getProbableBonuses(name, self._value)
        result = []
...
class RandomCrewbook(SimpleBonus):

    def getIcon(self):
        return R.images.gui.maps.icons.crewBooks.books.small.random_brochure()

    def getXP(self):
        if not self._value:
            return 0
        else:
            crewBookParams = first(self._value)
            for crewBookCD in crewBookParams:
                crewBook = self.itemsCache.items.getItemByCD(crewBookCD)
                if crewBook is not None:
                    return crewBook.getXP()
    ...
    def getBookType(self):
        if not self._value:
            ...
            for crewBookCD in crewBookParams:
                crewBook = self.itemsCache.items.getItemByCD(crewBookCD)
                if crewBook is not None:
                    return crewBook.getBookType()

            return ''


class RandomEventStyle3d(CustomizationsBonus):

    def getIcon(self):
        return R.images.gui.maps.icons.quests.bonuses.small.style_3d()


class RandomEventInscription(CustomizationsBonus):
    _TOOLTIP_PATH = R.strings.event.WtEventPortals.collectionTooltip.randomInscription

    def getIcon(self):
        return R.images.gui.maps.icons.quests.bonuses.small.decal()

    def getTooltip(self):
        return makeTooltip(backport.text(self._TOOLTIP_PATH.title()))


class RandomEventEmblem(CustomizationsBonus):
    _TOOLTIP_PATH = R.strings.event.WtEventPortals.collectionTooltip.randomEmblem

    def getIcon(self):
        return R.images.gui.maps.icons.quests.bonuses.small.emblem()

    def getTooltip(self):
        return makeTooltip(backport.text(self._TOOLTIP_PATH.title()))


class RandomEventCamouflage(CustomizationsBonus):
    _TOOLTIP_PATH = R.strings.event.WtEventPortals.collectionTooltip.randomCamouflage

    def getIcon(self):
        return R.images.gui.maps.icons.quests.bonuses.small.camouflage()
...
class WtCustomizationsBonus(CustomizationsBonus):
    ...
    def _format(self, styleSubset):
        ...
        if self._name is not None and item is not None:
            c11nItem = self.getC11nItem(item)
            itemType = item.get('custType') if item.get('custType') == c11nItem.itemTypeName else c11nItem.itemTypeName
            value = item.get('value')
            text = makeHtmlString(('html_templates:lobby/quests/bonuses/{}/wtEvent').format(self._name), itemType, {'name': c11nItem.userName, 'value': value})
            if text != self._name:
                return text
        return item


class RandomEventStyle2d(CustomizationsBonus):
    _TOOLTIP_PATH = R.strings.event.WtEventPortals.collectionTooltip.randomStyle2d

    def getIcon(self):
        return R.images.gui.maps.icons.quests.bonuses.small.style()

    def getTooltip(self):
        return makeTooltip(backport.text(self._TOOLTIP_PATH.title()))


class RandomEventDecal(CustomizationsBonus):
    _TOOLTIP_PATH = R.strings.event.WtEventPortals.collectionTooltip.randomDecal

    def getIcon(self):
        return R.images.gui.maps.icons.quests.bonuses.small.wt_decal()
    ...
    def getWrappedLootBoxesBonusList(self):
        return [
         {'id': 0, 
            ...
            'value': self.getCount(), 
            'icon': {AWARDS_SIZES.SMALL: self.getImage(size='small'), AWARDS_SIZES.BIG: self.getImage()}, 
            'name': self.getBlueprintTooltipName(), 
            'description': backport.text(R.strings.tooltips.blueprint.BlueprintFragmentTooltip.random.description())}]
...
class VehicleBlueprintBonus(SimpleBonus):
    ...
    def _getWrappedBonusList(self):
        ...
        result.append({'id': self._getFragmentCD(), 
           'type': ('custom/{}').format(self.getName()), 
           'value': self.getCount(), 
           'icon': {AWARDS_SIZES.SMALL: self.getImage(), AWARDS_SIZES.BIG: self.getImage()}, 
           'name': self.getBlueprintTooltipName(), 
           'description': self._getDescription()})
        return result
...
class NationalBlueprintBonus(VehicleBlueprintBonus):
    ...
    def _getFormattedMessage(self, styleSubset, formattedValue):
        return makeHtmlString(('html_templates:lobby/quests/{}').format(styleSubset), self._HTML_TEMPLATE, {'nationName': self._localizedNationName(), 'value': formattedValue})
...
class CrewSkinsBonus(SimpleBonus):
    ...
    def formattedList(self):
        ...
        for rarity, count in sortedByRarity.iteritems():
            result.append(makeHtmlString('html_templates:lobby/quests/bonuses', 'crewSkin', {'value': count, 'rarity': str(rarity)}))
...
class CrewBooksBonus(SimpleBonus):
    ...
    def formattedList(self):
        ...
        for item, count in self.getItems():
            result.append(makeHtmlString('html_templates:lobby/quests/bonuses', 'crewBook', {'type': item.getBookType(), 'nation': item.getNation(), 'value': count, 'name': item.userName}))
    ...
    def getXP(self):
        if not self._value:
            ...
            crewBookParam = first(self._value)
            if isinstance(crewBookParam, list):
                crewBookParam = first(crewBookParam)
            crewBook = self.itemsCache.items.getItemByCD(crewBookParam)
            if crewBook is None:
                return 0
            return crewBook.getXP()
        ...
        for item, count in self.getItems():
            if item is not None:
                result.append({'id': item.intCD, 
                   ...
                   'value': count, 
                   'icon': {AWARDS_SIZES.SMALL: item.getShopIcon(AWARDS_SIZES.SMALL), AWARDS_SIZES.BIG: item.getShopIcon(AWARDS_SIZES.BIG)}, 
                   'name': item.userName, 
                   'description': item.fullDescription})
...
class DogTagComponentBonus(SimpleBonus):
    _DogTagComponentRecord = namedtuple('_DogTagComponentRecord', [
     'componentId', 'unlock', 'value', 'grade'])
...
def _getPreferredMapSlotAwardTexts(slotName):
    ...
    if not awardItem:
        return ('', '')
    name = backport.text(awardItem.header()) if hasattr(awardItem, 'header') else ''
    if hasattr(awardItem, 'body'):
        description = backport.text(awardItem.body())
    elif hasattr(awardItem, 'body1'):
...
class PreferredMapSlotsBonus(IntegralBonus):
    ...
    def getWrappedLootBoxesBonusList(self):
        ...
        return [
         {'id': 0, 
            'type': ('custom/{}').format(self._slotName), 
            'value': self.getValue(), 
            'icon': {AWARDS_SIZES.SMALL: self.getIconBySize(AWARDS_SIZES.SMALL), AWARDS_SIZES.BIG: self.getIconBySize(AWARDS_SIZES.BIG)}, 
            'name': name, 
...
_BONUSES = {Currency.CREDITS: CreditsBonus, 
   ...
   'preferredMapSlots': PreferredMapSlotsBonus}
HIDDEN_BONUSES = (
 MetaBonus,)
_BONUSES_PRIORITY = (
 'tokens',
...
def getNonQuestBonuses(name, value, ctx=None):
    if name == 'preferredMapSlots' and isinstance(value, dict) and len(value) > 1:
        bonuses = []
        for slotID, slotDurationDays in value.iteritems():
            bonuses.extend(_initFromTree((name, 'default'), name, {slotID: slotDurationDays}, ctx=ctx))
...
def mergeCustomizationBonuses(lhv, rhv):
    ...
    for rhvItem in rhv.getValue():
        mergedItem = findFirst(lambda i, ri=rhvItem: i['id'] == ri['id'] and i['custType'] == ri['custType'], mergedValue)
        if mergedItem is not None:
            mergedItem['value'] += rhvItem['value']
        else:
...
def splitBonuses(bonuses, splitFunc=None):
    split = []
    splitFunction = getSplitBonusFunction if splitFunc is None else splitFunc
    for bonus in bonuses:
        splitFunc = splitFunction(bonus)
        if splitFunc:
