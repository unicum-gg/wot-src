...
class CommonStatsBlockConstructor(VehicleTooltipBlockConstructor):
 ...
 'switchTime'), 
    ...
    __CONDITIONAL_PARAMS = (
     (
      ROCKET_ACCELERATION_SPEED_LIMITS, ('speedLimits', ROCKET_ACCELERATION_SPEED_LIMITS)),)
    ...
    def construct(self):
        if self.configuration.params and not self.configuration.simplifiedOnly:
            ...
            if currentCrewSize < len(self.vehicle.descriptor.type.crewRoles):
                return [
                 formatters.packImageTextBlockData(title='', desc=text_styles.standard(TOOLTIPS.VEHICLE_STATS_FOOTNOTE), img=RES_ICONS.MAPS_ICONS_LIBRARY_STORE_CONDITION_OFF, imgPadding=formatters.packPadding(top=4), txtGap=-4, txtOffset=20, padding=formatters.packPadding(left=59, right=20))]
        return []
...
class StatusBlockConstructor(VehicleTooltipBlockConstructor):
    ...
    def construct(self):
        ...
        if isClanLock or isDisabledInRoaming:
            ...
            return (
             block, result and result.get('operationError') is not None, result)
