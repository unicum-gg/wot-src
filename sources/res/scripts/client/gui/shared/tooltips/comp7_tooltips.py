...
class RoleSkillLobbyTooltipData(BlocksTooltipData):
    ...
    def __packHeaderBlock(cls, equipment):
        blocks = [
         formatters.packTitleDescBlock(title=text_styles.highTitle(equipment.userString), desc=cls.__getCooldown(equipment), gap=-3),
...
def getRoleEquipmentTooltipParts(equipment):
    paramsTemplate = g_htmlTemplates['html_templates:comp7/tooltips/']['roleSkill']
    active, passive = getRoleSkillDescription(equipment)
