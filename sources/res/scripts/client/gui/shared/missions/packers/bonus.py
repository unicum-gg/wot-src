...
from gui.shared.system_factory import collectBonusPackers
...
EXCHANGE_RATE_TOKEN = 'exchangeRateToken'
...
if typing.TYPE_CHECKING:
    ...
    from gui.server_events.bonuses import CustomizationsBonus, CrewSkinsBonus, TokensBonus, SimpleBonus, ItemsBonus, DossierBonus, VehicleBlueprintBonus, CrewBooksBonus, GoodiesBonus, TankmenBonus, VehiclesBonus, DogTagComponentBonus, BattlePassPointsBonus, CurrenciesBonus, PersonalExchangeRateTokensBonus, UndefinedAmountBonus
...
def _bonusSortKey(sequence, bonus):
    ...
    if bonusName in sequence:
        return sequence.index(bonusName)
    return len(sequence)
...
def getDefaultBonusPackersMap():
    ...
    bonusPackers = {'battlePassPoints': BattlePassPointsBonusPacker(), 
    ...
    bonusPackers.update(collectBonusPackers())
    return bonusPackers
...
class TokenBonusUIPacker(BaseBonusUIPacker):
    ...
    def _getTokenBonusPackers(cls):
        return {BATTLE_BONUS_X5_TOKEN: cls.__packBonusQuestToken(BATTLE_BONUS_X5_TOKEN), 
           ...
           EXCHANGE_RATE_TOKEN: cls.__packExchangeRateToken}
    ...
    def _getTokenBonusType(cls, tokenID, complexToken):
        ...
        if isPMPoints(tokenID):
            return PM_POINTS_TOKEN
        if tokenID.split(':')[0].endswith(GOLD_MISSION):
            return GOLD_MISSION
        if tokenID.startswith(EXCHANGE_RATE_FREE_XP_NAME) or tokenID.startswith(EXCHANGE_RATE_GOLD_NAME):
            return EXCHANGE_RATE_TOKEN
        return ''
    ...
    def _getTooltipsPackers(cls):
        return {BATTLE_BONUS_X5_TOKEN: cls.__getBonusFactorTooltip(BATTLE_BONUS_X5_TOKEN), 
           ...
           EXCHANGE_RATE_TOKEN: cls.__getExchangeRateTooltip}
    ...
    @classmethod
    def __packExchangeRateToken(cls, model, bonus, _, token):
        if token.count <= 0:
            return None
        else:
            model.setUserName(backport.text(R.strings.tooltips.quests.bonuses.token.exchangeRate.dyn(bonus.resourceName).header()))
            model.setIconSmall(backport.image(R.images.gui.maps.icons.quests.bonuses.dyn(AWARDS_SIZES.SMALL).dyn('rate_' + bonus.resourceName)()))
            model.setIconBig(backport.image(R.images.gui.maps.icons.quests.bonuses.dyn(AWARDS_SIZES.BIG).dyn('rate_' + bonus.resourceName)()))
            model.setLabel(formatCountLabel(bonus.getCount()))
            return model
    ...
    @classmethod
    def __getExchangeRateTooltip(cls, _, token):
        header = backport.text(R.strings.tooltips.quests.bonuses.token.exchangeRate.dyn(token.id).header())
        return createTooltipData(makeTooltip(header))
...
class ItemBonusUIPacker(BaseBonusUIPacker):
    ...
    def _getToolTip(cls, bonus):
        ...
        for item in sorted(viewkeys(bonus.getItems())):
            tooltipData.append(TooltipData(tooltip=None, isSpecial=True, specialAlias=ItemsBonusFormatter.getTooltip(item), specialArgs=[
             item.intCD]))
...
class GoodiesBonusUIPacker(BaseBonusUIPacker):
    ...
    def _getToolTip(cls, bonus):
        ...
        for booster in sorted(iterkeys(bonus.getBoosters()), key=lambda b: b.boosterID):
            tooltipData.append(TooltipData(tooltip=TOOLTIPS_CONSTANTS.SHOP_BOOSTER, isSpecial=False, specialAlias=None, specialArgs=[
             booster.boosterID], isWulfTooltip=True))
        ...
        for item in sorted(viewkeys(bonus.getMentoringLicenses())):
            tooltipData.append(TooltipData(tooltip=TOOLTIPS_CONSTANTS.MENTOR_LICENSE, isSpecial=False, specialAlias=None, specialArgs=[
             item.intCD], isWulfTooltip=True))
...
class BlueprintBonusUIPacker(BaseBonusUIPacker):
    ...
    def _getToolTip(cls, bonus):
        return [
         TooltipData(tooltip=None, isSpecial=True, specialAlias=bonus.getBlueprintSpecialAlias(), specialArgs=[
...
class CrewSkinBonusUIPacker(BaseBonusUIPacker):
    ...
    def _getToolTip(cls, bonus):
        ...
        for item, _, _, _ in sorted(bonus.getItems()):
            tooltipData.append(TooltipData(tooltip=None, isSpecial=True, specialAlias=TOOLTIPS_CONSTANTS.CREW_SKIN, specialArgs=[
             item.getID()]))
...
class DossierBonusUIPacker(BaseBonusUIPacker):
    ...
    def _getAchievementTooltip(cls, bonus):
        ...
        for achievement in bonus.getAchievements():
            tooltipData.append(TooltipData(tooltip=None, isSpecial=True, specialAlias=TOOLTIPS_CONSTANTS.BATTLE_STATS_ACHIEVS, specialArgs=[
             achievement.getBlock(), achievement.getName(), achievement.getValue()]))
...
class DailyMissionsVehiclesBonusUIPacker(VehiclesBonusUIPacker):
    ...
    def _packTooltip(cls, bonus, vehicle, vehInfo):
        ...
        tooltipData.specialArgs.extend([
         bonus.getTmanRoleLevel(vehInfo) > 0,
         False,
         False])
        return tooltipData
...
class GroupsBonusUIPacker(BaseBonusUIPacker):
    ...
    def _getToolTip(cls, bonus):
        return [
         createTooltipData(makeTooltip(TOOLTIPS.getAwardHeader(bonus.getName()), TOOLTIPS.getAwardBody(bonus.getName())))]
...
class BonusUIPacker(object):
    ...
    def pack(self, bonus):
        ...
        _logger.error('BonusUIPacker.pack - Packer not implemented yet, name=%s', bonus.getName())
        return []
    ...
    def getToolTip(self, bonus):
        ...
        _logger.error('BonusUIPacker.getToolTip - Packer not implemented yet, name=%s', bonus.getName())
        return []
    ...
    def getContentId(self, bonus):
        ...
        _logger.error('BonusUIPacker.getContentId - Packer not implemented yet, name=%s', bonus.getName())
        return []
