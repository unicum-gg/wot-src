...
VEHICLE_TYPES_ORDER_INDICES = {n:i for i, n in enumerate(VEHICLE_TYPES_ORDER)}
...
VEHICLE_TABLE_TYPES_ORDER = (
 VEHICLE_CLASS_NAME.HEAVY_TANK,
 VEHICLE_CLASS_NAME.MEDIUM_TANK,
 VEHICLE_CLASS_NAME.LIGHT_TANK,
 VEHICLE_CLASS_NAME.AT_SPG,
 VEHICLE_CLASS_NAME.SPG)
VEHICLE_TABLE_TYPES_ORDER_INDICES = {n:i for i, n in enumerate(VEHICLE_TABLE_TYPES_ORDER)}
VEHICLE_TABLE_TYPES_ORDER_INDICES_REVERSED = {n:i for i, n in enumerate(reversed(VEHICLE_TABLE_TYPES_ORDER))}
VEHICLE_BATTLE_TYPES_ORDER = (
 VEHICLE_CLASS_NAME.HEAVY_TANK,
 VEHICLE_CLASS_NAME.MEDIUM_TANK,
 VEHICLE_CLASS_NAME.AT_SPG,
 VEHICLE_CLASS_NAME.LIGHT_TANK,
...
VEHICLE_ROLES_LABELS_BY_CLASS = {VEHICLE_CLASS_NAME.LIGHT_TANK: [ constants.ROLE_TYPE_TO_LABEL.get(group) for group in _LIGHT_GROUPS ], VEHICLE_CLASS_NAME.MEDIUM_TANK: [ constants.ROLE_TYPE_TO_LABEL.get(group) for group in _MEDIUM_GROUPS ], VEHICLE_CLASS_NAME.HEAVY_TANK: [ constants.ROLE_TYPE_TO_LABEL.get(group) for group in _HEAVY_GROUPS ], VEHICLE_CLASS_NAME.AT_SPG: [ constants.ROLE_TYPE_TO_LABEL.get(group) for group in _AT_SPG_GROUPS ], VEHICLE_CLASS_NAME.SPG: [ constants.ROLE_TYPE_TO_LABEL.get(group) for group in _SPG_GROUPS ]}
...
EPIC_ACTION_VEHICLE_CDS = (
 44033,
 63265)
NOT_FULL_AMMO_MULTIPLIER = 0.2
_MAX_RENT_MULTIPLIER = 2
RentPackagesInfo = namedtuple('RentPackagesInfo', (
 'hasAvailableRentPackages',
 'mainRentType',
 'seasonType'))
CrystalsEarnedInfo = namedtuple('CrystalsEarnedInfo', (
 'current',
 'max'))
EliteStatusProgress = typing.NamedTuple('EliteStatusProgress', (
 (
  'unlocked', typing.Set[int]),
 (
  'toUnlock', typing.Set[int]),
 (
...
class Vehicle(FittingItem):
    __slots__ = ('__customState', '__weakref__', '_inventoryID', '_xp', '_dailyXPFactor',
                 ...
                 '_rentPackagesInfo', '_isDisabledForBuy', '_isSelected', '_restorePrice',
                 '_searchableUserName', '_personalDiscountPrice', '_rotationGroupNum',
                 '_rotationBattlesLeft', '_isRotationGroupLocked', '_isInfiniteRotationGroup',
                 '_settings', '_lock', '_repairCost', '_health', '_gun', '_turret',
                 '_engine', '_chassis', '_radio', '_fuelTank', '_equipment', '_bonuses',
                 '_crewIndices', '_crew', '_lastCrew', '_hasModulesToSelect', '_outfitComponents',
                 '_slotsAnchors', '_unlockedBy', '_maxRentDuration', '_minRentDuration',
                 '_slotsAnchorsById', '_hasNationGroup', '_extraSettings', '_groupIDs',
                 '_postProgression', '_invData', '_proxy')
    ...
    def __init__(self, strCompactDescr=None, inventoryID=-1, typeCompDescr=None, proxy=None, extData=None, invData=None):
        ...
        self._proxy = proxy
        self._invData = invData or {}
        postProgressionFeatures = None
        if self._proxy is not None and self._proxy.inventory.isSynced() and self._proxy.stats.isSynced() and self._proxy.shop.isSynced() and self._proxy.vehicleRotation.isSynced() and self._proxy.recycleBin.isSynced():
            invDataTmp = self._proxy.inventory.getItems(GUI_ITEM_TYPE.VEHICLE, inventoryID)
            if invDataTmp is not None:
                self._invData = invDataTmp
            ...
            self._isDisabledForBuy = self.intCD in self._proxy.shop.getNotToBuyVehicles()
            invRentData = self._invData.get('rent')
            if invRentData is not None:
                self._rentInfo = RentalInfoProvider(isRented=True, *invRentData)
            hasAvailableRentPackages, mainRentType, seasonType = self.rentalsController.getRentPackagesInfo(self._proxy.shop.getVehicleRentPrices().get(self.intCD, {}), self._rentInfo)
            self._rentPackagesInfo = RentPackagesInfo(hasAvailableRentPackages, mainRentType, seasonType)
            self._isSelected = bool(self.invID in self._proxy.stats.oldVehInvIDs)
            self.createAppliedOutfits(self._proxy)
            restoreConfig = self._proxy.shop.vehiclesRestoreConfig
            self._restorePrice = calcVehicleRestorePrice(self.buyPrices.itemPrice.defPrice, self._proxy.shop)
            self._restoreInfo = self._proxy.recycleBin.getVehicleRestoreInfo(self.intCD, restoreConfig.restoreDuration, restoreConfig.restoreCooldown)
            self._personalDiscountPrice = self._proxy.shop.getPersonalVehicleDiscountPrice(self.intCD)
            self._rotationGroupNum = self._proxy.vehicleRotation.getGroupNum(self.intCD)
            self._rotationBattlesLeft = self._proxy.vehicleRotation.getBattlesCount(self.rotationGroupNum)
            self._isRotationGroupLocked = self._proxy.vehicleRotation.isGroupLocked(self.rotationGroupNum)
            self._isInfiniteRotationGroup = self._proxy.vehicleRotation.isInfinite(self.rotationGroupNum)
            self._unlockedBy = self._proxy.vehicleRotation.unlockedBy(self.rotationGroupNum)
            postProgressionFeatures = self._proxy.inventory.getVehPostProgressionFeaturesListByCD(self.intCD)
        ...
        self._inventoryCount = 1 if self._invData else 0
        self._settings = self._invData.get('settings', 0)
        ...
        self._equipment = VehicleEquipment(self._proxy, vehDescr, self._invData, postProgressionFeatures)
        self.initCrew()
        self._lastCrew = self._invData.get('lastCrew')
        if self.canTradeIn:
            self._rentPackagesInfo = RentPackagesInfo(False, 0, 0)
            self._rentPackages = []
            self._maxRentDuration, self._minRentDuration = (0, 0)
        else:
            self._rentPackages = calcRentPackages(self, self._proxy, self.rentalsController)
    ...
    def __repr__(self):
        return 'Vehicle<id:%d, intCD:%d, nation:%d, lock:%s>' % (
         self.invID, self.intCD, self.nationID, self.lock)
    ...
    def initCrew(self):
        defaultCrew = [
         None] * len(self._descriptor.type.crewRoles)
        crewList = self._invData.get('crew', defaultCrew)
        battleCrewList = self._invData.get('battleCrewCDs')
        self.calcCrewBonuses(battleCrewList or crewList, self._proxy, fromBattle=battleCrewList is not None)
        self._crewIndices = {invID:idx for idx, invID in enumerate(crewList)}
        self._crew = self._buildCrew(battleCrewList or crewList, self._proxy, fromBattle=battleCrewList is not None)
        return
    ...
    def __initAnchors(self):
        ...
        for slotHelper in (chassisCustomizationSlots, hullCustomizationSlots, turretCustomizationSlots,
    ...
    def getAnchors(self, slotType, areaId):
        return viewitems(self._slotsAnchors.get(slotType, {}).get(areaId, {}))
    ...
    def outfits(self):
        ...
        outfits = {season:self.__getOutfit(component, vehicleCD) for season, component in viewitems(self._outfitComponents)}
        return outfits
    ...
    def calcCrewBonuses(self, crew, proxy, fromBattle):
        ...
        for tID in crew:
            ...
            brotherhoodSum += tdescr.skillLevel('brotherhood') or 0
            if tdescr.role == Tankman.ROLES.COMMANDER:
                commanderEffRoleLevel = float(tdescr.roleLevel)

        brotherhoodLevel = brotherhoodSum / (len(crew) * tankmen.MAX_SKILL_LEVEL)
        bonuses['brotherhood'] = brotherhoodLevel * tankmen.getSkillsConfig().getSkill('brotherhood').crewLevelIncrease
        if commanderEffRoleLevel > 0:
            commanderEffRoleLevel += bonuses['brotherhood'] + bonuses['equipment'] + bonuses['optDevices']
            bonuses['commander'] += round_py2_style(old_div(commanderEffRoleLevel, tankmen.COMMANDER_ADDITION_RATIO))
        return

    def getTankmanIDBySlotIdx(self, slotIdx):
        for slot, tman in self.crew:
            if slot == slotIdx:
                if tman:
                    return tman.invID
                return NO_TANKMAN
    ...
    def _buildCrew(self, crew, proxy, fromBattle):
        ...
        for idx, tankmanID in enumerate(crew):
            ...
            if tankmanID is not None:
                if fromBattle:
                    tmanInvData = {'compDescr': tankmanID, 'invID': -1}
                else:
    ...
    def __getCustomOutfitComponent(self, proxy, season):
        ...
        if customOutfitData is not None:
            return customizations.parseC11sComponentDescr(customOutfitData)
        else:
    ...
    def getEliteStatusProgress(self):
        ...
        installable = {intCD for intCD in vehType.installableComponents if getTypeOfCompactDescr(intCD) in GUI_ITEM_TYPE.VEHICLE_MODULES if getTypeOfCompactDescr(intCD) in GUI_ITEM_TYPE.VEHICLE_MODULES}
        ...
        unlocks = set(self.itemsCache.items.stats.unlocks)
    ...
    def isRentAvailable(self):
        if self._rentPackagesInfo.mainRentType == RentType.TIME_RENT:
            return self.maxRentDuration - self.rentLeftTime >= self.minRentDuration
        return self._rentPackagesInfo.hasAvailableRentPackages and self._rentPackagesInfo.mainRentType in (RentType.SEASON_RENT, RentType.SEASON_CYCLE_RENT)
    ...
    def getC11nItemsNoveltyCounter(self, proxy, itemTypes=None, season=None, itemFilter=None):
        count = 0
        newItems = proxy.inventory.getC11nItemsNoveltyCounters(self._descriptor.type)
        for itemCD, qtyItems in viewitems(newItems):
    ...
    def getNewC11nItems(self, proxy):
        newItems = []
        for itemCD in proxy.inventory.getC11nItemsNoveltyCounters(self._descriptor.type):
            item = proxy.getItemByCD(itemCD)
    ...
    def __getStateLevel(self, state):
        if state in (
         Vehicle.VEHICLE_STATE.CREW_NOT_FULL,
        ...
        if state in (Vehicle.VEHICLE_STATE.WILL_BE_UNLOCKED_IN_BATTLE,):
            return Vehicle.VEHICLE_STATE_LEVEL.ATTENTION
    ...
    @property
    def isWotPlus(self):
        return checkForTags(self.tags, VEHICLE_TAGS.WOT_PLUS)
    ...
    def shortDescriptionSpecial(self):
        ...
        if description.find('_short_special') == -1:
            return description
        return ''
    ...
    def longDescriptionSpecial(self):
        ...
        if description.find('_long_special') == -1:
            return description
        return ''
    ...
    def canSell(self):
        ...
        if self.isRented:
            if not self.rentalIsOver:
                return False
            if st in (self.VEHICLE_STATE.RENTAL_IS_OVER,
             self.VEHICLE_STATE.IGR_RENTAL_IS_OVER,
             self.VEHICLE_STATE.RENTABLE_AGAIN):
    ...
    def mayRent(self, money):
        ...
        return (
         False, GUI_ITEM_ECONOMY_CODE.NO_RENT_PRICE)
    ...
    def getUnlocksDescrs(self):
        for unlockIdx, data in enumerate(self.descriptor.type.unlocksDescrs):
            yield (
             unlockIdx, data[0], data[1], set(data[2:]))
    ...
    def __getCrewSkills(self):
        ...
        for _, roles in enumerate(self.descriptor.type.crewRoles):
            for tankmanIdx, vehTankman in self.crew:
                if vehTankman and tankmanIdx not in usedTankmans and roles == vehTankman.combinedRoles:
                    ...
                    for forRole, bonusSkils in iteritems(vehTankman.bonusSkills):
        ...
        return (
         skills, skillLevels, rolesBonusSkills, bonusSkillsLevels)
    ...
    def getCrewWithSkills(self, skillNames):
        ...
        if not skillNames:
            ...
            skillNames = lfilter(None, skillNames)
            crewItems = []
            for slotIdx, tman in self.crew:
                ...
                rolesBonusSkills = {}
                for role, bonusSkills in iteritems(tman.bonusSkills):
                ...
                for skillName in skillNames:
                    ...
                    if roleType in (tman.role, tankmen.COMMON_SKILL_ROLE_TYPE):
                ...
                if not skillsAdded:
                    ...
                    bonusSkillsLevels = [
                     tmanDescr.bonusSkillsLevels, skillNamesMaxLvl]
    ...
    def getCrewWithoutSelectedSkills(self, skillNames=None):
        ...
        for slotIdx, tman in self.crew:
            ...
            bonusSkillsLevels = [
             lastBonusSkillLevel] * NPS.MAX_BONUS_SKILLS_PER_ROLE
            ...
            unskilledTman = self.itemsFactory.createTankman(unskilledTmanCD, vehicle=self, vehicleSlotIdx=tman.vehicleSlotIdx, bonusSkillsLevels=[
             bonusSkillsLevels, maxLevelSkills])
    ...
    def getCrewBySkillLevels(self, defRoleLevel, skillsByIdxs=None, levelByIdxs=None, nativeVehsByIdxs=None, activateBrotherhood=False, skillLevelsByIdxs=None, skillsEfficiencyByIdxs=None, rolesBonusSkills=None, bonusSkillsLevels=None):
        ...
        crewItems = []
        crewRoles = self.descriptor.type.crewRoles
        brotherhoodActive = skillsByIdxs and any(BROTHERHOOD_SKILL_NAME in skills for skills in viewvalues(skillsByIdxs))
        for idx, _ in enumerate(crewRoles):
            ...
            if defRoleLevel is not None:
                ...
                tankman = self.itemsFactory.createTankman(tankmen.generateCompactDescr(tankmen.generatePassport(nationID), vehicleTypeID, role, defRoleLevel, skillsByIdxs.get(idx, []), lastSkillLevel=skillLevelsByIdxs.get(idx, tankmen.MAX_SKILL_LEVEL), skillsEfficiencyXP=skillsEfficiencyByIdxs.get(idx, MAX_SKILLS_EFFICIENCY_XP), rolesBonusSkills=rolesBonusSkills.get(idx, {})), vehicle=self, vehicleSlotIdx=idx, bonusSkillsLevels=[
                 bonusSkillsLevels.get(idx, [tankmen.MAX_SKILL_LEVEL] * NPS.MAX_BONUS_SKILLS_PER_ROLE)])
    ...
    def setCustomOutfit(self, season, outfit):
        for s in SeasonType.REGULAR:
            ...
            elif s in self._outfitComponents and self._outfitComponents[s].styleId:
    ...
    def getAnyOutfitSeason(self):
        ...
        if activeSeasons:
            return random.choice(activeSeasons)
    ...
    def _mayPurchase(self, price, money):
        if getattr(BigWorld.player(), 'isLongDisconnectedFromCenter', False):
            return (False, GUI_ITEM_ECONOMY_CODE.CENTER_UNAVAILABLE)
        return super(Vehicle, self)._mayPurchase(price, money)
    ...
    def _compare(self, other):
        ...
        return super(Vehicle, self)._compare(other)
    ...
    def __calcSellPrice(self, proxy, price):
        ...
        for defCompDescr, instCompDescr in zip(defaultDevices, installedDevices):
...
def getWotPlusExclusiveVehicleTypeUserName(vehType):
    return i18n.makeString('#menu:header/wotPlusExclusiveVehicleType/%s' % vehType)
...
def getTypeSmallIconPath(vehicleType, isElite=False):
    if isElite:
        return RES_ICONS.maps_icons_vehicletypes_elite_all_png(vehicleType)
    return RES_ICONS.maps_icons_vehicletypes_all_png(vehicleType)
...
def getType44x44IconResource(vehicleType, isElite=False):
    return R.images.gui.maps.icons.vehicleTypes.c_44x44.dyn(replaceHyphenToUnderscore(vehicleType + '_elite' if isElite else vehicleType))
...
def sortCrew(crewItems, crewRoles):
    ...
    if len(crewItems) > 1:
        return sorted(crewItems, key=lambda k: RO[crewRoles[k[0]][0]])
    return crewItems
