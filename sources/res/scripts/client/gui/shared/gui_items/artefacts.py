...
class Equipment(VehicleArtefact):
    ...
    def getConflictedLayoutEquimpents(self, vehicle):
        conflictEqs = list()
        if self in vehicle.consumables.layout:
            return conflictEqs
        for e in vehicle.consumables.layout.getItems():
            compatibility = e.descriptor.checkCompatibilityWithActiveEquipment(self.descriptor)
            if compatibility:
                compatibility = self.descriptor.checkCompatibilityWithEquipment(e.descriptor)
            if not compatibility:
...
class BattleBooster(Equipment):
    ...
    def getAffectedSkillUserName(self):
        if self.isCrewBooster():
            return tankmen.getSkillsConfig().getSkill(self.getAffectedSkillName()).userString
        return ''
    ...
    def _getShortInfo(self, vehicle=None, expanded=False):
        if self.isCrewBooster():
            ...
            if self.isEconomicBooster():
                return self.getEconomicDirectivesDescription()
...
def getAbilityVO(vehicle):
    abilityId = vehicle.descriptor.type.ability
    if abilityId:
        from items.vehicles import g_cache
        return {'abilityName': g_cache.getEquipmentByID(abilityId).name, 
           'tooltip': TOOLTIPS_CONSTANTS.ABILITY_LOBBY_TOOLTIP, 
           'tooltipArgs': [
                         abilityId, vehicle.intCD]}
    else:
        return