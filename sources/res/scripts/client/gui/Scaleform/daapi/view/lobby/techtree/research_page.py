...
def _getPreferentialBenefits(benefits, root, _=None):
    if root.isPreferential:
        benefits.append((
         backport.image(R.images.gui.maps.shop.kpi.preferential()),
         backport.text(R.strings.vehicle_preview.infoPanel.preferentialTitle()),
         backport.text(R.strings.vehicle_preview.infoPanel.preferentialText())))
...
def _getRestoreBannerStr(param):
    if param != float('inf'):
        return text_styles.concatStylesToMultiLine(text_styles.mainBig(backport.text(R.strings.menu.research.restore.commonInfo())), text_styles.mainBig(backport.text(R.strings.menu.research.restore.dueDate(), date=param)))
    return text_styles.mainBig(backport.text(R.strings.menu.research.restore.commonInfoUnlim()))
    ...
    def _getRootData(self):
        ...
        result = {'vehicleTitle': {'intCD': self._data.getRootCD(), 
           ...
           'vehicleButton': {'shopIconPath': getShopVehicleIconPath(STORE_CONSTANTS.ICON_SIZE_MEDIUM, root.name.split(':')[1]), 
                             ...
                             'goToVehicleViewBtnVisible': not (root.isTelecom and root.rentalIsOver), 
