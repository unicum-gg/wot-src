...
def _getPersonalMissionsTooltip(branch, key):
    ...
    if branch == PM_BRANCH.PERSONAL_MISSION_2:
        return TOOLTIPS_HANGAR_HEADER_PM2.get(key, '')
    if branch == PM_BRANCH.PERSONAL_MISSION_3:
        return TOOLTIPS_HANGAR_HEADER_PM3.get(key, '')
    return ''
...
class HangarHeader(HangarHeaderMeta, IGlobalListener, IEventBoardsListener):
    __slots__ = ('_currentVehicle', '__screenWidth', '__isShowPersonalMission', '__activeWidgets',
                 '__widgets')
    ...
    def __init__(self):
        ...
        self.__widgets = {wrapper.alias:wrapper for _, wrapper in inspect.getmembers(self.__class__, inspect.ismethod) if getattr(wrapper, 'alias', None) if getattr(wrapper, 'alias', None)}
        return
    ...
    def __getDisabledPersonalMissionsVO(self, modeName):
        ...
        for branch in reversed(PM_BRANCH.ACTIVE_BRANCHES):
            ...
            result.append(self._headerQuestFormatterVo(enable=False, icon='', label=_ms(MENU.hangarHeaderPersonalMissionsLabel(LABEL_STATE.EMPTY)), questType=questType, tooltip=makeTooltip(header=backport.text(strPath.header()), body=backport.text(strPath.body(), modeName=backport.text(strPath.mode.dyn(modeName)())))))
    ...
    def __getEarlyAccessQuestsVO(self, wrap=True):
        ...
        if wrap:
            return self._wrapQuestGroup(HANGAR_HEADER_QUESTS.QUEST_TYPE_EARLY_ACCESS, '', quests)
        return quests
    ...
    def __getArmoryYardQuestsVO(self, wrap=True):
        ...
        if wrap:
            return self._wrapQuestGroup(HANGAR_HEADER_QUESTS.QUEST_TYPE_ARMORY_YARD, '', quests)
        return quests
    ...
    def __getMarathonQuestsVO(self, vehicle, isGroupped=False):
        ...
        if marathons:
            ...
            for index, marathonEvent in enumerate(marathons):
                ...
                if flagVO['visible']:
                    quest = self._headerQuestFormatterVo(flagVO['enable'], flagVO['flagHeaderIcon'], '', ('').join((HANGAR_HEADER_QUESTS.QUEST_TYPE_MARATHON,
                     str(index))), flag=flagVO['flagMain'], stateIcon=flagVO['flagStateIcon'], questID=marathonEvent.prefix, tooltip=flagVO['tooltip'], isTooltipSpecial=flagVO['enable'])
                    if not isGroupped:
                        wrappedGroup = self._wrapQuestGroup(('').join((HANGAR_HEADER_QUESTS.QUEST_GROUP_MARATHON,
                         str(index))), '', [quest])
    ...
    def __getWinbackQuestsToHeaderVO(self):
        ...
        if flagVO['visible']:
            quest = self._headerQuestFormatterVo(flagVO['enable'], flagVO['flagIcon'], flagVO['flagLabel'], HANGAR_HEADER_QUESTS.QUEST_TYPE_COMMON, flag=flagVO['flagMain'], tooltip=flagVO['tooltip'], isTooltipSpecial=flagVO['enable'])
            return [
             self._wrapQuestGroup(HANGAR_HEADER_QUESTS.QUEST_TYPE_COMMON, '', [quest])]
        return []
    ...
    @staticmethod
    def _headerQuestFormatterVo(enable, icon, label, questType, flag=None, flagDisabled=None, stateIcon=None, questID=None, isReward=False, tooltip='', isTooltipSpecial=False, isTooltipWulf=False):
    ...
    def addExternalWidgetHandler(cls, alias, handler):
        if not callable(handler):
            raise SoftException('[HangarHeader] handler is not callable.')
        if alias in cls.__externalWidgets:
            _logger.error('[HangarHeader] handler already in __externalWidgets')
    ...
    def removeExternalWidgetHandler(cls, alias):
        ...
        if handler is None:
            _logger.error('__externalWidgets not have handler with %s alias', alias)
        return