...
class Hangar(LobbySelectableView, HangarMeta, IGlobalListener):
    ...
    def paramsPanel(self):
        if self.__currentVehicleParamsAlias is not None:
            return self.getComponent(self.__currentVehicleParamsAlias)
        else:
            return
    ...
    def hasUiEffects(self):
        return self._settingsCore.getSetting(GRAPHICS.UI_EFFECTS)
    ...
    def _populate(self):
        ...
        self.addListener(events.StrongholdEvent.STRONGHOLD_UPDATED, self.__updateBattleModifiersWidget, scope=EVENT_BUS_SCOPE.LOBBY)
    ...
    def _dispose(self):
        ...
        self.removeListener(events.StrongholdEvent.STRONGHOLD_UPDATED, self.__updateBattleModifiersWidget, scope=EVENT_BUS_SCOPE.LOBBY)
        self.itemsCache.onSyncCompleted -= self.onCacheResync
        self.__guiLootBoxes.onStatusChange -= self.__onLootBoxesEventStatusChange
        self.__armoryYardCtrl.onStatusChange -= self.__onArmoryYardStatusChange
        g_currentVehicle.onChanged -= self.__onCurrentVehicleChanged
    ...
    def __updateUiEffectsState(self):
        self.as_updateUiEffectsStateS(self.hasUiEffects)
    ...
    def __showArmoryYardVehiclePreview(self, vehicleCD):
        if self.__armoryYardCtrl.isActive() and self.__armoryYardCtrl.getFinalRewardVehicle().intCD == vehicleCD:
            shared_events.showHeroTankPreview(vehicleCD, bottomPanelTextData={'uniqueVehicleTitle': text_styles.tutorial(backport.text(R.strings.vehicle_preview.buyingPanel.uniqueVehicleLabel()))})
            return True
        return False
