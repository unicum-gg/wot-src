...
class _SelectionTask(_Task):
    selectionWithCardinality = (
     _cm.SUMSEQN, _cm.SUMN, _cm.SUMMSEQN)
    ...
    def getInfo(self):
        ...
        if method in self.selectionWithCardinality:
            ...
            result = result % {'cardinality': str(cardinality), 'distance': str(distance)}
...
class _BattleTypeCondition(_Condition):
    ...
    def getTooltip(self):
        if self._event.getBattleType() == ARENA_GUI_TYPE.RANDOM:
            return makeTooltip(body=TOOLTIPS.ELEN_CONDITION_BATTLETYPE_RANDOM)
        return makeTooltip(body=TOOLTIPS.ELEN_CONDITION_BATTLETYPE_NOTRANDOM)
...
class _TopLeaderboard(object):
    ...
    def __getTitle(self):
        ...
        if self.__eventType == _et.LEVEL:
            return _ms(EVENT_BOARDS.VEHICLES_LEVEL, level=int2roman(self.__value))
        return 'ERROR'
    ...
    def __getDecoration(self):
        if self.__eventType == _et.NATION:
            ...
            if self.__eventType in (_et.LEVEL, _et.CLASS):
                return getLevelBackgroundIcon()
            return
    ...
    def __getAmountTooltip(self):
        ...
        if parameter in parametersWithTooltip:
            return makeTooltip(body=_ms(EVENT_BOARDS.tooltip_top_amount(parameter)))
        else:
            return
    ...
    def __getRibbon(self):
        ...
        if ribbon is None and rank <= self._event.getLeaderboardViewSize():
            return WOODEN_RIBBON
        else:
            return ribbon
    ...
    def __getMyPosititon(self):
        ...
        if self.__notInTop:
            return text_styles.neutral(_ms(EVENT_BOARDS.TOP_PARTICIPATION_NOTINTOP))
        return text_styles.main(('{} {}').format(_ms(EVENT_BOARDS.TOP_POSITION), self._top.getMyPosition()))
...
class EventInfo(object):
    ...
    def getServerData(self):
        ...
        if not isAvailableServer and isAvailableAnyServer and self._playerState.getPlayerState() == _es.JOINED:
            buttons = [ {'label': str(self._lobbyContext.getPeripheryName(int(pt.getServer()), False)), 'server': pt.getServer(), 'tooltip': makeTooltip(self._lobbyContext.getPeripheryName(int(pt.getServer()), False), TOOLTIPS.ELEN_BUTTON_SERVER_BODY)} for pt in availableServers
                      ]
            result = {'serverBlock': {'title': title, 'buttons': buttons}, 'isUnsuitableServer': True}
            ...
            result = {'reloginBlock': {'title': EVENT_BOARDS.SERVER_NOSUITABLE_HEADER, 'description': description, 
    ...
    def getStatusData(self):
        ...
        return {'title': title, 'description1': description1, 
    ...
    def getPopoverAlias(self):
        if self._event.getType() == _et.VEHICLE:
            return EVENTBOARDS_ALIASES.RESULT_FILTER_POPOVER_VEHICLES_ALIAS
        return EVENTBOARDS_ALIASES.RESULT_FILTER_POPOVER_ALIAS
...
class EventHeader(object):
    ...
    def getParticipateInfo(self):
        ...
        if event.isFinished():
            ...
            result = {'participate': participate, 'participateTooltip': participateTooltip, 
...
BONUS_PRIORITY = ('gold', 'crystal', 'tokens', 'battleToken', 'vehicles', 'badgesGroup',
                  'dossier', 'lootbox', 'credits', 'items', 'goodies', 'customizations',
                  'tankmen', 'premium', 'slots', 'berths')

def _readEventBoardsRewards(rewards):
    badgesGroupData = rewards.pop('badgesGroup', None)
    if badgesGroupData is not None:
        return {'badgesGroup': int(badgesGroupData['value'])}
    else:
        return {}
