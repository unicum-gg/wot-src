...
from gui.Scaleform.daapi.view.lobby.vehicle_preview.items_kit_helper import BOX_TYPE, OFFER_CHANGED_EVENT, getActiveOffer, lookupItem, mayObtainForMoney, mayObtainWithMoneyExchange, showItemTooltip
...
from gui.shared.event_dispatcher import showVehicleRentDialog, showLootBoxEntry, showLootBoxBuyWindow
...
class VehiclePreviewBottomPanel(VehiclePreviewBottomPanelMeta):
    ...
    def __updateBtnState(self, *_):
        ...
        if item is None:
            ...
            hasExternalLink = (yield self.__isHeroTankFromBoxes() or self.__hasExternalLink()) if 1 else self._lootBoxesCtrl.getBoxesCount(self._lootBoxesCtrl.mainEntryPoint) == 0
            if hasExternalLink:
    ...
    def __purchaseHeroTank(self):
        if self.__isHeroTankFromBoxes():
            ...
            if self._lootBoxesCtrl.getBoxesCount(eventName):
                showLootBoxEntry()
            else:
                showLootBoxBuyWindow(eventName)
            return
    ...
    def __isHeroTankFromBoxes(self):
        isLootboxAvailable = self._lootBoxesCtrl.isAvailable(self._lootBoxesCtrl.mainEntryPoint)
        return isLootboxAvailable and self.__isHeroTank and self._heroTanks.getCurrentFromBoxes()