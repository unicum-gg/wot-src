...
def getBonusPacker(isRewardScreen=False, isOperationCompleted=False):
    ...
    mapping.update({PREMIUM_ENTITLEMENTS.BASIC: premiumPacker, 
       ...
       'completionTokens': pawnedBonusPacker, 
       'tankwomanBonus': pawnedBonusPacker, 
       'freeTokens': pawnedBonusPacker})
...
class PawnedBonusPacker(BaseBonusUIPacker):
    ...
    def _pack(cls, bonus):
        ...
        quest = cls.eventsCache.getPersonalMissions().getQuestsForBranch(branchID)[ctx['questID']]
        ...
        for frmtBonus in formattedBonuses:
            ...
            packedBonus.setIcon(frmtBonus.get('imgSource', ''))
            if frmtBonus.get('specialAlias', '') == TOOLTIPS_CONSTANTS.FREE_SHEET_USED:
