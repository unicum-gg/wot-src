...
class SelectableRewardBase(ViewImpl):
    ...
    def __selectTab(self, tabName, initial=False):
        if self.__selectedTab != tabName:
            ...
            self._fillRewards(self.__selectedTab, initial=initial)
    ...
    def __getValidTabType(self, tabType):
        if tabType.endswith(self.__REWARD_EXTRA_ENDING):
            return tabType[:-len(self.__REWARD_EXTRA_ENDING)]
        return tabType
    ...
    def __processRewards(self):
        for selectableReward in self.__selectableRewards:
            ...
            for giftID, gift in offer.iteritems():
                ...
                rewardOp = self._createReward if currentTab['rewards'].get(rewardName) is None else self.__updateReward
