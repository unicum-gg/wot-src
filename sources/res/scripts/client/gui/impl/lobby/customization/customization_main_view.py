...
class CustomizationMainView(ViewImpl, EventSystemEntity, CustomizationSettingsSerializable):
    __slots__ = ('__ctx', '__carouselDP', '__slotSelector', '__selectedItem', '__isHistoric',
                 '__isNonHistoric', '__isFantastical', '__initAnchorsPositionsCallback',
    ...
    def __getInitialTab(self):
        if self.__service.isStyleInstalled() and self.__ctx.mode.currentOutfit.style is not None:
            ...
            hasAnyCustomization = bool({item for item in (self.__service.getItemByCD(sd.intCD) for sd in self.__ctx.mode.currentOutfit.slotsData()) if ItemTags.NATIONAL_EMBLEM not in item.descriptor.parentGroup.itemPrototype.tags if ItemTags.NATIONAL_EMBLEM not in item.descriptor.parentGroup.itemPrototype.tags})
    ...
    def __fillCarouselModel(self, carouselModel, carouselData):
        ...
        displayedBookmarks = carouselData.bookmarks if useBookmarks else [{'bookmarkName': '', 'bookmarkIndex': 0, 'isProgressive': hasProgressiveItems}]
    ...
    def __onMoveSpace(self, args=None):
        if args is None:
            ...
            self.__toolbarProvider.lobbyViewMouseEvent({'dx': args.get('dx'), 'dy': args.get('dy'), 'dz': args.get('dz')})
            self.fireEvent(CameraRelatedEvents(CameraRelatedEvents.LOBBY_VIEW_MOUSE_MOVE, ctx={'dx': args.get('dx'), 'dy': args.get('dy'), 'dz': args.get('dz')}), EVENT_BUS_SCOPE.GLOBAL)
            self.fireEvent(events.LobbySimpleEvent(events.LobbySimpleEvent.NOTIFY_SPACE_MOVED, ctx={'dx': args.get('dx'), 'dy': args.get('dy'), 'dz': args.get('dz')}), EVENT_BUS_SCOPE.GLOBAL)
            return
