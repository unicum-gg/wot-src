...
def checkIsQualificationBattle(personalRecord):
    if 'avatar' in personalRecord:
        return personalRecord['avatar'].get('comp7QualActive', False)
    return False
...
class Comp7VehicleStatValuesBlock(RegularVehicleStatValuesBlock):
    __slots__ = ('damageDealtBySkills', 'healed', 'healedAlly', 'capturedPointsOfInterest',
                 'roleSkillUsed')
