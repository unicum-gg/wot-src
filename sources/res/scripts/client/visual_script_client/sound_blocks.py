...
class ScheduleDelayedSound2D(Block, SoundMeta):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._start = self._makeEventInputSlot('start', self.__start)
        self._stop = self._makeEventInputSlot('stop', self.__stop)
        self._remainingTime = self._makeDataInputSlot('remainingTime', SLOT_TYPE.FLOAT)
        self._timeBeforeEnd = self._makeDataInputSlot('timeBeforeEnd', SLOT_TYPE.FLOAT)
        self._threshold = self._makeDataInputSlot('threshold', SLOT_TYPE.FLOAT)
        self._sound = self._makeDataInputSlot('sound', SLOT_TYPE.SOUND)
        self._startOut = self._makeEventOutputSlot('startOut')
        self._stopOut = self._makeEventOutputSlot('stopOut')
        self.__timerCallbackID = None
        self.__initDefaults()
        return

    def captionText(self):
        return 'Schedule Delayed Sound2D'
