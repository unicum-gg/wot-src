...
AdvancedChatCommandData = namedtuple('AdvancedChatCommandData', ('command', 'commandCreatorVehID',
                                                                 'callbackID', 'owners'))
...
class AdvancedChatComponent(ClientArenaComponent):
        ...
        if typeOfUpdate == ChatCommandChange.CHAT_CMD_WAS_REMOVED:
            if playerVehID in commandData.owners or playerVehID == cmdTargetID or self.__isLastPrebattleMarkerOwner(cmdMarkerType, cmdID, commandData) or not self.__isAliveVehicle(senderVehID):
                chatStats = {senderVehID: (
                               EMPTY_STATE, EMPTY_CHAT_CMD_FLAG)}
    ...
    def __removeActualTargetIfDestroyed(self, commands, playerVehID, targetID, markerType):
        ...
        if self.__markerInFocus and self.__markerInFocus.isFocused(targetID, markerType):
            ...
            if not listOfCommands:
                _logger.warning('Focused chat command target is missing: targetID=%s; markerType=%s; commandID=%s', targetID, markerType, self.__markerInFocus.commandID)
    ...
    def __getTargetIDForCommandName(self, commandName, cmd, allowDeadVehInfo=False):
        if commandName in (BATTLE_CHAT_COMMAND_NAMES.SOS, BATTLE_CHAT_COMMAND_NAMES.HELPME,
         BATTLE_CHAT_COMMAND_NAMES.RELOADINGGUN, BATTLE_CHAT_COMMAND_NAMES.TURNBACK,
         BATTLE_CHAT_COMMAND_NAMES.THANKS):
            return self.__getCommandCreatorVehID(cmd, allowDeadVehInfo)
        return cmd.getFirstTargetID()
    ...
    def __addCommandToList(self, commandID, commandName, commandCreatorID, commandTargetID, command, activeTime=_DEFAULT_ACTIVE_COMMAND_TIME):
        ...
        if isTemporarySticky:
            self.__temporaryStickyCommands[commandID][commandTargetID] = (
             commandTargetID, markerType)
    ...
    def __handleVehicleCommand(self, targetID, senderID, commandID, cmd):
        ...
        isPermanent = commandName not in (BATTLE_CHAT_COMMAND_NAMES.TURNBACK, BATTLE_CHAT_COMMAND_NAMES.THANKS,
         BATTLE_CHAT_COMMAND_NAMES.RELOADINGGUN, BATTLE_CHAT_COMMAND_NAMES.CONFIRM,
         BATTLE_CHAT_COMMAND_NAMES.POSITIVE, BATTLE_CHAT_COMMAND_NAMES.SOS,
    ...
    def __handleObjectiveCommand(self, commandName, cmdCreatorID, cmd):
        ...
        if not feedbackCtrl or feedbackCtrl is None:
            return
        if commandName in (BATTLE_CHAT_COMMAND_NAMES.ATTACKING_OBJECTIVE,
    ...
    def __setFocusedOnMarker(self, targetID, markerType, repliedToCommandID):
        if markerType != MarkerType.INVALID_MARKER_TYPE:
            ...
            if repliedToCommandID not in commands:
                _logger.warning('Unable to focus missing chat command: targetID=%s; markerType=%s; commandID=%s', targetID, markerType, repliedToCommandID)
                return
    ...
    def __handleReply(self, cmd):
        ...
        if not isAlreadyAdded and wasMuted:
            if replyToActionName in _CHAT_CMD_CREATE_IF_NO_ORIGINAL_COMMAND_VEHICLES:
                protoData = {'int32Arg1': targetID, 'int64Arg1': replierVehicleID, 'floatArg1': 0.0, 'strArg1': '', 'strArg2': ''}
