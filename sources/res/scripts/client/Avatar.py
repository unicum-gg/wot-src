...
import BigWorld, Keys, Math, ResMgr, WWISE, Account, AccountCommands, AreaDestructibles, AvatarInputHandler, AvatarPositionControl, BattleReplay, ClientArena, CommandMapping, Event, FlockManager, MusicControllerWWISE, ProjectileMover, SoundGroups, TriggersManager, Vehicle, VehicleGunRotator, VehicleObserverGunRotator, Weather, constants
...
class PlayerAvatar(BigWorld.Entity, ClientChat, CombatEquipmentManager, AvatarObserver, TeamHealthbarMechanic, AvatarEpicData, AvatarRecoveryMechanic, AvatarRespawnMechanic, VehiclesSpawnListStorage, VehicleRemovalController, VehicleHealthBroadcastListenerComponent, AvatarChatKeyHandling, TriggersController, VisualScriptController):
        ...
        if not self.userSeesWorld():
            ...
            try:
                ...
                if cmdMap.isFired(CommandMapping.CMD_QUEST_PROGRESS_SHOW, key) and (mods != 2 or not isDown):
                    if isEpicBattle or not isBR and self.lobbyContext.getServerSettings().isPersonalMissionsEnabled():
    ...
    def __couldToggleGUIVisibility(self):
        ...
        if app is None:
            ...
            if self.arenaGuiType in ARENA_GUI_TYPE.COMP7_RANGE and self.arena.period <= ARENA_PERIOD.PREBATTLE:
    ...
    def updateVehicleHealth(self, vehicleID, health, deathReasonID, isCrewActive, isRespawn):
        ...
        if not self.userSeesWorld():
            ...
            if not wasAlive and isAlive:
                ...
                MusicControllerWWISE.onPostmortemState(False)
                if self.vehicle:
            ...
            if not isAlive and wasAlive or not isAlive and wasRespawnAvailable and not isRespawn:
                ...
                postmortemMusicState = noRespawnPossible or not isRespawn
                MusicControllerWWISE.onPostmortemState(postmortemMusicState)
            return
    ...
    def updateSiegeStateStatus(self, vehicleID, status, timeLeft):
        ...
        if not typeDescr:
            return
        if status in (constants.VEHICLE_SIEGE_STATE.SWITCHING_ON,
         constants.VEHICLE_SIEGE_STATE.SWITCHING_OFF):
    ...
    def updateTargetingInfo(self, entityId, turretYaw, gunPitch, maxTurretRotationSpeed, maxGunRotationSpeed, shotDispMultiplierFactor, gunShotDispersionFactorsAfterShot, gunShotDispersionFactorsTurretRotation, chassisShotDispersionFactorsMovement, chassisShotDispersionFactorsRotation, aimingTime):
        LOG_DEBUG_DEV('updateTargetingInfo', entityId, turretYaw, gunPitch, maxTurretRotationSpeed, maxGunRotationSpeed, shotDispMultiplierFactor, gunShotDispersionFactorsAfterShot, gunShotDispersionFactorsTurretRotation, chassisShotDispersionFactorsMovement, chassisShotDispersionFactorsRotation, aimingTime)
        ...
        dispersionInfo[5] = gunShotDispersionFactorsAfterShot
        if self.gunRotator:
    ...
    def showShotResults(self, results):
        ...
        enemies = {}
        damagedAllies = defaultdict(int)
        burningEnemies = []
        hasKill = False
        for r in results:
            vehicleID = r & 4294967295
            flags = r >> 32 & 4294967295
        ...
        for allyVehID, hitsCount in damagedAllies.iteritems():
            showMessage(allyVehID, hitsCount > 1)
        ...
        if hasKill:
            ...
            if bestSound is not None:
                ...
                canPlaySound = True
                spamCtrl = self.guiSessionProvider.shared.battleSpamCtrl
                if spamCtrl is not None:
                    canPlaySound = spamCtrl.filterShotResultSound(bestSound[1])
                if canPlaySound:
    ...
    def explodeProjectile(self, shotID, effectsIndex, effectMaterialIndex, endPoint, velocityDir, damagedDestructibles):
        if self.userSeesWorld() and self.__projectileMover is not None:
            ...
            explInfo = (
             endPoint, velocityDir, physParams['shellVelocity'], physParams['shellMass'],
             physParams['splashRadius'], physParams['splashStrength'])
            g_playerEvents.onProjectileExplosion(*explInfo)
    ...
    def getOwnVehicleShotDispersionAngle(self, turretRotationSpeed, withShot=0):
        ...
        multFactor, gunShotDispersionFactorsTurretRotation, chassisShotDispersionFactorsMovement, chassisShotDispersionFactorsRotation, aimingTime, gunShotDispersionFactorsAfterShot = self.__dispersionInfo
        ...
        if withShot == 0 or shouldSkip:
            ...
            shotFactor = descr.gun.shotDispersionFactors['afterShotInBurst'] * gunShotDispersionFactorsAfterShot
        autoShootGunCtrl = getPlayerVehicleAutoShootGunController()
        if autoShootGunCtrl is not None:
            shotFactor = autoShootGunCtrl.getShootDispersionFactor()
    ...
    def __getVehicleMovementState(self):
        ...
        if isRightSideDestroyed or isLeftSideDestroyed:
    ...
    def __getSoundTypeForMultiTrackRepaired(self, prevState, extra):
        if 'functionalCanMove' in extra.sounds and prevState != self.__getVehicleMovementState():
            return 'functionalCanMove'
        else:
            return
    ...
    def __startWaitingForShot(self, makePrediction, simplifiedPrediction=False, shotArgs=None):
        if self.__shotWaitingTimerID is not None:
            BigWorld.cancelCallback(self.__shotWaitingTimerID)
            self.__shotWaitingTimerID = None
        if makePrediction and not simplifiedPrediction:
    ...
    def __onTimedOutCharge(self):
        self.__chargeWaitingTimerID = None
        self.__startWaitingForShot(self.__canMakeDualShot, shotArgs=(0, True))
        return
        ...
        try:
            ...
            if vehicle is not None and vehicle.isStarted:
                ...
                if shotArgs is not None:
                    ...
                    vehicle.showShooting(burstCount, -1 if isDual else gunIndexDelayed, True)
                    return
