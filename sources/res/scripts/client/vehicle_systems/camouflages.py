...
RepaintParams = namedtuple('PaintParams', ('enabled', 'baseColor', 'color', 'metallic',
                                           'gloss', 'fading', 'strength'))
RepaintParams.__new__.__defaults__ = (
 False, 0, (0, 0, 0), Math.Vector4(0.0), Math.Vector4(0.0), 0.0, 0.0)
CamoParams = namedtuple('CamoParams', ('mask', 'excludeMap', 'exclusionImpact', 'forceUv0',
                                       'tiling', 'rotation', 'weights', 'c0', 'c1',
                                       'c2', 'c3', 'gloss', 'metallic', 'useGMTexture',
                                       'glossMetallicMap', 'useEmission', 'forwardEmissionBrightness',
                                       'deferredEmissionBrightness', 'emissionMap',
                                       'emissionAnimationSpeed', 'emissionPatternMap',
                                       'useNormalMap', 'normalMap', 'normalMapFactor',
                                       'normalMaxLod'))
CamoParams.__new__.__defaults__ = (
 '', '', 1.0, Math.Vector4(0.0), 0, Math.Vector4(0.0), 0, 0, 0, 0,
 Math.Vector4(DEFAULT_GLOSS), Math.Vector4(DEFAULT_METALLIC), False, '',
 False, DEFAULT_FORWARD_EMISSION, DEFAULT_DEFERRED_EMISSION, '',
 DEFAULT_EMISSION_ANIMATION_SPEED, '',
 False, '', DEFAULT_NORMAL_MAP_FACTOR, DEFAULT_NORMAL_MAX_LOD)
ProjectionDecalGenericParams = namedtuple('ProjectionDecalGenericParams', ('tintColor',
                                                                           ...
                                                                           'scaleBySlotSize',
                                                                           'useEmission',
                                                                           'forwardEmissionBrightness',
                                                                           'deferredEmissionBrightness',
                                                                           'emissionMap',
                                                                           'emissionAnimationSpeed',
                                                                           'emissionPatternMap'))
...
def getRepaint(outfit, containerId, vDesc):
    ...
    if enabled:
        return RepaintParams(enabled, defaultColor, colors, metallics, glosses, fading, quality)
    else:
    ...
    for idx in range(capacity):
        ...
        if not slotData.isEmpty():
            if slotData.component.slotId in slotsByIdMap:
                ...
                _logger.warning('SlotId mismatch (slotId=%(slotId)d component=%(component)s)', {'slotId': slotData.component.slotId, 'component': slotData.component})
                continue
...
def __getProjectionDecalTextures(item, component, vehDesc):
    ...
    if IS_EDITOR:
        return (texture, glossTexture, emissionTexture, patternTexture)
    else:
        ...
        return (
         texture, glossTexture, emissionTexture, patternTexture)
