...
class CompoundAppearance(CommonTankAppearance, CallbackDelayer):
        ...
        if vehicle is None:
            ...
            if not vehicle.isEnteringWorld and self.engineAudition:
                if wheelsIdx == -1:
                    ...
                    if not self.wheelsAnimator.valid():
                        LOG_ERROR('Wheels animator not valid for vehicle. Vehicle id and CD: ', vehicle.id, vehicle.typeDescriptor.makeCompactDescr())
                        return
    ...
    def getGunMatrix(self):
        return (
         self.turretMatrix.target, self.gunMatrix.target)
