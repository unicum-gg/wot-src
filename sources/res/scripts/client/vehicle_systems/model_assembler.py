...
def createSwingingAnimator(gameObject, vehicleDesc, basisMatrix, worldMProv=None, lodLink=None):
    ...
    pp = tuple(p * m for p, m in zip(swingingCfg.pitchParams, (0.9, 1.88, 0.3, 4.0,
                                                               1.0, 1.0)))
...
def createTrackNodesAnimator(appearance, typeDescriptor, lodStateLink=None):
    ...
    try:
        ...
        for trackNode in trackNodesConfig.nodes:
            ...
            trackNodesAnimator.addTrackNode(trackNode.name, trackNode.isLeft, trackNode.initialOffset, leftSibling, rightSibling, trackNode.trackPairIndex, (
...
def assembleCustomLogicComponents(appearance, typeDescriptor, attachments, modelAnimators):
    assemblers = [
     (
      'flagAnimation', __assembleAnimationFlagComponent),
     (
      'prefab', __assemblePrefabComponent)]
    if IS_EDITOR:
