...
class VehicleGunRotator(object):
    ...
    def start(self):
        ...
        def multiGunCurrentShotPosition():
            ...
            if player is None:
                ...
                if multiGun is None or gunIdx >= len(multiGun) or gunIdx < 0:
                    return
                return multiGun[gunIdx].shotPosition
    ...
    def setShotPosition(self, vehicleID, shotPos, shotVec, dispersionAngle, forceValueRefresh=False):
        if self.__clientMode and not self.__showServerMarker and not forceValueRefresh:
            ...
            if not self.__clientMode or forceValueRefresh:
                ...
                self.__turretYaw, self.__gunPitch = getShotAngles(self._avatar.getVehicleDescriptor(), self._avatar.getOwnVehicleStabilisedMatrix(), (
                 self.__turretYaw, self.__gunPitch), mPos, adjust=True, overrideGunPosition=self.__gunPosition)
    ...
    def getShotParams(self, targetPoint, ignoreYawLimits=False, overrideShotIdx=None):
        ...
        shotTurretYaw, shotGunPitch = getShotAngles(descr, self._avatar.getOwnVehicleStabilisedMatrix(), (
         self.__turretYaw, self.__gunPitch), targetPoint, overrideGunPosition=self.__gunPosition, overrideShotIdx=overrideShotIdx)
    ...
    def __getGunMarkerPosition(self, shotPos, shotVec, dispersionAngles):
        ...
        return (endPos, direction, diameter, idealDiameter, dualAccDiameter, dualAccIdealDiameter, collData)
    ...
    def _updateMultiGunCollisionData(self):
        ...
        if multiGun is None:
            ...
            if BigWorld.player().vehicle is None:
                return
