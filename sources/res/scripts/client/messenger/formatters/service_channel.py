...
class ServiceChannelFormatter(object):
    ...
    def _getGuiSettings(self, data, key=None, priorityLevel=None, messageType=None, messageSubtype=None, decorator=None, auxData=None, isSoundable=False):
        try:
        ...
        return NotificationGuiSettings(self.isNotify(), priorityLevel, isAlert, messageType=messageType, messageSubtype=messageSubtype, decorator=decorator, lifeTime=lifeTime, auxData=auxData, isSoundable=isSoundable)
...
class BattleResultsFormatter(WaitItemsSyncFormatter):
    ...
    def _prepareFormatData(self, message):
        ...
        if guiType == ARENA_GUI_TYPE.BATTLE_ROYALE:
            ...
            battleResultKeys = self.__COMP7SeasonResultsKeys
            ctx = self.__makeTournamentComp7SeasonMsgCtx(battleResults, ctx)
        else:
    ...
    def __makeTournamentComp7SeasonMsgCtx(self, battleResults, ctx):
        ctx[b'ratingPointsStr'] = g_settings.htmlTemplates.format(b'battleResultRatingPoints', {b'ratingPoints': str(battleResults[b'comp7RatingDelta'])})
        return ctx
    ...
    def __makeVehicleLockString(self, vehicleNames, battleResults):
        ...
        for vehIntCD, battleResult in battleResults.get(b'playerVehicles', {}).iteritems():
            ...
            if vehicleName is None:
                continue
            locks.append(g_settings.htmlTemplates.format(b'battleResultLocks', ctx={b'vehicleName': vehicleName.userName, 
    ...
    def __makeRankedFlowStrings(self, battleResults):
        ...
        if battleResults.get(b'guiType', 0) == ARENA_GUI_TYPE.RANKED:
            ...
            if stateChange == RankChangeStates.DIVISION_EARNED:
                stateChangeStr = backport.text(shortcut.rankedState.dyn(RankChangeStates.RANK_EARNED)())
...
class CurrencyUpdateFormatter(ServiceChannelFormatter):
    REFERRAL_PROGRAM_EMITTER_ID = 1033
    ...
    def _getFormatter(self, emitterID):
        formatters = {self.REFERRAL_PROGRAM_EMITTER_ID: self._referralProgramFormatter}
    ...
    def _referralProgramFormatter(self, message, currencyCode, amountDelta, transactionTime, emitterID):
        if currencyCode == Currency.GOLD:
            formatted = g_settings.msgTemplates.format(b'ReferralProgramCashbackInvoiceReceived', ctx={b'amount': amountDelta})
            return [
             MessageData(formatted, self._getGuiSettings(message, b'ReferralProgramCashbackInvoiceReceived'))]
...
class InvoiceReceivedFormatter(WaitItemsSyncFormatter):
    ...
    def dataSubformatters(self):
        return collectServiceChannelSubformatter((InvoiceReceivedFormatter, IInvoiceDataSubFormatter))
...
class ConverterFormatter(ServiceChannelFormatter):
    ...
    def format(self, message, *args):
        ...
        extData = data.get(b'extData', {})
        if extData:
            for k, value in extData.iteritems():
                form = collectConvertersSubFormatter(k)
                if form is not None:
                    messagesListData.extend(form().format(message))
...
class QuestAchievesFormatter(object):
    ...
    def questAchievesSubFormatters(self):
        return collectServiceChannelSubformatter((QuestAchievesFormatter, IQuestAchievesSubformatter))
    ...
    def getFormattedAchieves(cls, data, asBattleFormatter, processCustomizations=True, processTokens=True):
        result = []
        for formatter in cls.questAchievesSubFormatters:
            formatter.format(data, asBattleFormatter, result, processCustomizations, processTokens)
        ...
        if not asBattleFormatter:
            ...
            if crystal:
                formatter = getBWFormatter(Currency.CRYSTAL)
                result.append(cls.__makeQuestsAchieve(b'battleQuestsCrystal', crystal=formatter(crystal)))
            eventCoin = data.get(Currency.EVENT_COIN, 0)
            if eventCoin:
                formatter = getBWFormatter(Currency.EVENT_COIN)
                result.append(cls.__makeQuestsAchieve(b'battleQuestsEventCoin', eventCoin=formatter(eventCoin)))
            equipCoin = data.get(Currency.EQUIP_COIN, 0)
            if equipCoin:
                formatter = getBWFormatter(Currency.EQUIP_COIN)
                result.append(cls.__makeQuestsAchieve(b'battleQuestsEquipCoin', equipCoin=formatter(equipCoin)))
            gold = data.get(Currency.GOLD, 0)
            if gold:
                formatter = getBWFormatter(Currency.GOLD)
                result.append(cls.__makeQuestsAchieve(b'battleQuestsGold', gold=formatter(gold)))
            bpcoin = data.get(Currency.BPCOIN, 0)
            if bpcoin:
                formatter = getBWFormatter(Currency.BPCOIN)
                result.append(cls.__makeQuestsAchieve(b'battleQuestsBpcoin', bpcoin=formatter(bpcoin)))
            vehiclesBattlePassPoints = data.get(Currency.BATTLE_PASS_POINTS, {}).get(b'vehicles')
            if vehiclesBattlePassPoints:
                questBattlePassPoints = sum(vehiclesBattlePassPoints.itervalues())
        ...
        if tokens:
            for tokenID, tokenData in tokens.iteritems():
                intCount = tokenData.get(b'count', 1)
                count = backport.getIntegralFormat(intCount)
                ...
                elif tokenID == CLAN_SEASON_PROGRESS_PREFIX:
                ...
                elif tokenID.startswith(constants.LOOTBOX_CUSTOMIZATION_PROGRESS_PREFIX):
                    _, style, progressLevel = tokenID.split(b':')
        ...
        if not asBattleFormatter:
            ...
            preferredMapSlots = data.get(b'preferredMapSlots', {})
            if preferredMapSlots:
                ...
                if slotNames:
                    result.append(cls.__makeQuestsAchieve(b'battleQuestsItems', names=(b', ').join(slotNames)))
        return result
...
class IQuestAchievesSubformatter(object):
...
class PersonalMissionsQuestAchievesFormatter(QuestAchievesFormatter):
    ...
    @classmethod
    def _processTokens(cls, data):
        quest = cls.__eventsCache.getPersonalMissions().getAllQuests().get(data.get(b'pmQuestID'))
        if quest:
            result = []
            ...
            offersTokenCount = sum(tokenData.get(b'count', 1) for tokenID, tokenData in data.get(b'tokens', {}).iteritems() if tokenID.startswith(OFFER_TOKEN_PREFIX))
            if offersTokenCount > 0:
                text = backport.text(R.strings.messenger.serviceChannelMessages.offerTokenBonus.alternativeTitle())
                ...
                offerName = g_settings.htmlTemplates.format(b'offerTokenText', {b'text': text})
                fmt = g_settings.htmlTemplates.format(b'offersAccruedInvoiceReceived', {b'offer': offerName})
...
class BattleMattersQuestAchievesFormatter(QuestAchievesFormatter):
    ...
    def _processTokens(cls, tokens):
        if cls.isWithSelectableReward(tokens):
            token = tokens.get(b'tokens').keys()[0]
            level = cls.__battleMattersController.getDelayedRewardVehiclesLevel(token)
            return text_styles.stats(backport.text(R.strings.messenger.serviceChannelMessages.battleMatters.token(), level=int2roman(int(level))))
        return b''
    ...
    def isWithSelectableReward(cls, awardsDict):
        if b'tokens' not in awardsDict:
            return False
        return any(cls.__battleMattersController.isDelayedRewardToken(v) for v in awardsDict[b'tokens'].iterkeys())
...
class ClanSeasonProgressRewardsFormatter(QuestAchievesFormatter):
    _BULLET = b'• '
    _SEPARATOR = b'<br/>' + _BULLET
...
class ClanSeasonQuestRewardsFormatter(QuestAchievesFormatter):
    ...
    @classmethod
    def formatQuestAchieves(cls, data, asBattleFormatter, processCustomizations=True, processTokens=True):
        result = super(ClanSeasonQuestRewardsFormatter, cls).formatQuestAchieves(data, asBattleFormatter, processCustomizations, processTokens=False)
        if result:
            return cls._BULLET + result
        return result
...
class PrbVehicleMaxFlamethrowerKickFormatter(ServiceChannelFormatter):
    __itemsCache = dependency.descriptor(IItemsCache)
    ...
    def format(self, message, *args):
        ...
        if vehInvID:
            vehicle = self.__itemsCache.items.getVehicle(vehInvID)
            if vehicle:
                formatted = g_settings.msgTemplates.format(b'prbVehicleMaxFlamethrowerKick', ctx={b'vehName': vehicle.userName})
        return [MessageData(formatted, self._getGuiSettings(message, b'prbVehicleMaxFlamethrowerKick'))]
...
class PersonalMissionFailedFormatter(WaitItemsSyncFormatter):
    ...
    def format(self, message, callback=None):
        ...
        if message.data and isSynced:
            data = message.data
            pmQuestsIDs = data.get(b'questIDs')
            if pmQuestsIDs:
                questsDict = {}
                for questID in pmQuestsIDs:
...
class ExcludedMapSlotKillSwitchFormatter(ServiceChannelFormatter):
    __TEMPLATES = {True: b'RewardsExcludedMapSlotEnabledMessage', 
       False: b'RewardsExcludedMapSlotDisabledMessage'}

    def format(self, message, *args):
        if not message or not message.data:
            return [MessageData(None, None)]
        else:
            slotType = message.data.get(b'slotType')
            isEnabled = message.data.get(b'enabled')
...
class BattlePassRewardFormatter(WaitItemsSyncFormatter):
    ...
    def __makeAfterBattle(self, ctx):
        ...
        chapterID = ctx.get(b'chapter')
        savedData = None
        if not (self.__battlePass.isCompleted() and self.__battlePass.isResourceCompleted()) or self.__battlePass.isMarathonChapter(chapterID):
            chapterName = backport.text(R.strings.battle_pass.chapter.fullName.quoted.num(chapterID)())
    ...
    def __makeAfterLevelsPurchase(self, ctx):
        ...
        if self.__battlePass.isFinalLevel(chapterID, currentLevel):
            if self.__battlePass.isCompleted() and self.__battlePass.isResourceCompleted() and not self.__battlePass.isSingleChapter():
                description = backport.text(R.strings.messenger.serviceChannelMessages.battlePassReward.battle.final.text(), season=self.__battlePass.getSeasonNum())
            elif self.__battlePass.isChapterExists(chapterID):
...
class ResourceWellNoVehiclesFormatter(WaitItemsSyncFormatter):
    ...
    def format(self, message, callback):
        ...
        if isSynced and isRuleCompleted:
            from tutorial.control.game_vars import getVehicleByIntCD
...
class AchievementsSMFormatter(ClientSysMessageFormatter):
    ...
    def format(self, message, *args):
        ...
        if messageType is None:
            ...
            ctx = self.__getCtx(messageType, rank, subRank)
            formatted = g_settings.msgTemplates.format(template, ctx, data={b'linkageData': self.__getSavedDate(messageType, rank, subRank)})
...
class PM3CampaignCompletedPerfect(ServiceChannelFormatter):
    __PM3_EARLY_PERFECT_COMPLETE_TEMPLATE = b'PM3EarlyPerfectCompletion'

    def __init__(self):
        self.__achievesFormatter = PersonalMissionsQuestAchievesFormatter()

    def format(self, message, *args):
        messageDataList = []
        pm3ChampionTokenQuestID = args[0]
        rewards = getRewardsForQuests(message, {pm3ChampionTokenQuestID})
        if b'vehicles' in rewards:
            return []
        else:
            fmt = self.__achievesFormatter.formatQuestAchieves(rewards, asBattleFormatter=False)
            if fmt is not None:
                templateParams = {b'achieves': fmt}
                settings = self._getGuiSettings(message, self.__PM3_EARLY_PERFECT_COMPLETE_TEMPLATE)
                formatted = g_settings.msgTemplates.format(self.__PM3_EARLY_PERFECT_COMPLETE_TEMPLATE, templateParams)
                messageDataList.append(MessageData(formatted, settings))
            return messageDataList


STRING_CONSTANT_CREDITS = b'credits'
STRING_CONSTANT_EQUIPMENT = b'equipments'
STRING_CONSTANT_INSTRUCTION = b'instructions'
STRING_CONSTANT_AMMUNITION = b'ammunitions'
STRING_CONSTANT_APPEARANCE = b'appearances'
STRING_CONSTANT_KIT = b'kits'
STRING_CONSTANT_CREW = b'crews'

def commaConcatinate(string, extra, doInclude=True):
    if string and doInclude:
        string = string + b', '
    if doInclude:
        string = string + extra
    return string
...
class ParagonsFormatter(ServiceChannelFormatter):
    _auxDataStrings = {STRING_CONSTANT_CREDITS: R.strings.paragons.branch.reseted.refund.credits(), 
       STRING_CONSTANT_EQUIPMENT: R.strings.paragons.branch.reseted.refund.equipment(), 
       STRING_CONSTANT_INSTRUCTION: R.strings.paragons.branch.reseted.refund.instruction(), 
       STRING_CONSTANT_AMMUNITION: R.strings.paragons.branch.reseted.refund.ammunition(), 
       STRING_CONSTANT_APPEARANCE: R.strings.paragons.branch.reseted.refund.appearance(), 
       STRING_CONSTANT_KIT: R.strings.paragons.branch.reseted.refund.kit(), 
       STRING_CONSTANT_CREW: R.strings.paragons.branch.reseted.refund.crew()}
    ...
    def format(self, message, *args):
        ...
        if messageType is None:
            ...
            parameters = message.get(b'parameters')
            from notification.decorators import ParagonsMessageDecorator
            auxData = {}
            if messageType == ParagonsSystemMessages.BRANCH_RESETED:
                notificationText = b''
                for key, textId in self._auxDataStrings.items():
                    value = parameters.get(key)
                    if int(value) > 0:
                        if key == STRING_CONSTANT_CREDITS:
                            amounts = text_styles.brcoin(value)
                        else:
                            count = (b'x{}').format(value)
                            amounts = text_styles.bpcoin(count)
                        if notificationText:
                            notificationText = notificationText + b'<br/>'
                        notificationText = notificationText + backport.text(textId, amounts=amounts)

                if notificationText:
                    auxData[b'texts'] = (b'<br/>').join((
                     backport.text(R.strings.paragons.branch.reseted.body.second()), notificationText))
                else:
                    auxData[b'texts'] = b''
            else:
                auxData = parameters
            formatted = g_settings.msgTemplates.format(messageType, ctx=auxData)
            return [
             MessageData(formatted, self._getGuiSettings(message, messageType, decorator=ParagonsMessageDecorator, auxData=auxData, isSoundable=True))]


class ParagonsLevelCompletedFormatter(ServiceChannelFormatter):
    __ACHIEVEMENT_NAMES = {
     b'ParagonsFirstSeason', b'Paragons_S2'}
    ...
    def format(self, message, *args):
        ...
        if not data:
            ...
            rewardInfo = data[b'levelRewardsInfo']
            coins = data[b'coinsGranted']
            chapter = data[b'chapterID']
            sourceID = data.get(b'sourceID')
            if sourceID is not None:
                sourceID = first(sourceID.split(b':'))
            messages = []
            for level, levelRewards in sorted(rewardInfo.items()):
                ...
                if b'dossier' in levelRewards:
                    for _, dossier in levelRewards[b'dossier'].items():
                        for blockName, recordName in dossier.keys():
                            if blockName == b'singleAchievements' and recordName in self.__ACHIEVEMENT_NAMES:
    ...
    def __pushSingleAchievementMessage(self, data, chapterID):
        ...
        chapterName = backport.text(R.strings.paragons.chapterName.short.dyn((b'id_{}').format(str(chapterID)))())
        text = backport.text(R.strings.system_messages.paragons.chapterCompleted.text(), chapter_name=chapterName)
        formatted = g_settings.msgTemplates.format(ParagonsSystemMessages.CHAPTER_COMPLETED, ctx={b'text': text})
    ...
    def _makeRewards(self, vehicles, styles, crews, showSelector, showBranch):
        rewards = b''
        rewards = commaConcatinate(rewards, backport.text(R.strings.paragons.reward.vehicle(), vehicles=vehicles), len(vehicles) > 0)
        rewards = commaConcatinate(rewards, backport.text(R.strings.paragons.reward.style(), styles=styles), len(styles) > 0)
        rewards = commaConcatinate(rewards, backport.text(R.strings.paragons.reward.crew(), crews=crews), len(crews) > 0)
        if showSelector:
            rewards = commaConcatinate(rewards, backport.text(R.strings.paragons.reward.selector.level.eleven()))
        if showBranch:
            rewards = commaConcatinate(rewards, backport.text(R.strings.paragons.reward.branch()))
        return rewards

    def _getShowSelectorAndRewards(self, rewards):
        vehicleName = b''
        styleName = b''
        crewName = b''
        showSelector = False
        showBranch = False
        from gui.impl.lobby.paragons.paragons_helpers.paragons_model_helpers import getParagonsBonuses
        for bonus in getParagonsBonuses(rewards):
            if bonus.getName() == b'vehicles':
                for vehicle, _ in bonus.getVehicles():
                    vehicleName = vehicle.userName

                continue
            if bonus.getName() == b'styleProgress':
                from gui.shared.gui_items.customization.c11n_helpers import getProgressionStyle
                style = getProgressionStyle(bonus.getStyleID(), None, None)
                styleName = style.userName
                continue
            if bonus.getName() == b'tmanToken':
                for tankman in bonus.getWrappedBonusList():
                    crewName = tankman.get(b'name')

                continue
            if bonus.getName() == b'vehicleSelector':
                showSelector = True
                continue
            if bonus.getName() == b'paragonsUnlocks':
                showBranch = True
                continue

        return (
         self._makeRewards(vehicleName, styleName, crewName, showSelector, showBranch), showSelector)


class ParagonsCoinsGrantedFormatter(ServiceChannelFormatter):
    ...
    def format(self, message, *args):
        ...
        if not data:
            ...
            if not data.get(b'sourceID'):
                return [MessageData(None, None)]
            sourceID = first(data[b'sourceID'].split(b':'))
            parameters = {b'coins': data[b'coinsGranted'], 
               b'source': backport.text(R.strings.paragons.notifications.source.dyn(sourceID)())}
            formatted = g_settings.msgTemplates.format(ParagonsSystemMessages.BATTLE_REWARD, ctx=parameters)
            return [
             MessageData(formatted, self._getGuiSettings(message, None, isSoundable=True))]
