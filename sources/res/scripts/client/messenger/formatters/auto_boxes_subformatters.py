...
from gui.shared.gui_items.loot_box import ALL_LUNAR_NY_LOOT_BOX_TYPES, WTLootBoxes
...
class LootBoxSystemAutoOpenFormatter(AsyncAutoLootBoxSubFormatter):
    ...
    @classmethod
    def getBoxesOfThisGroup(cls, data):
        return {boxId for boxId, info in data.iteritems() if info.get('relatedFeature') == DEFAULT_EVENT_NAME if info.get('relatedFeature') == DEFAULT_EVENT_NAME}
    ...
    def __getMessage(self, message, openedBoxes, eventName):
        textPathParts = [
         NotificationPathPart.MAIN, NotificationPathPart.AUTOOPEN]
        ...
        formatted = g_settings.msgTemplates.format(self.__MESSAGE_TEMPLATE, ctx={'header': header, 'description': description}, data={'savedData': {'eventName': eventName, 'rewards': rewards, 'boxIDs': boxesCounts}})
