...
VERSUS_AI_CAROUSEL_FILTER_CLIENT_1 = 'VERSUS_AI_CAROUSEL_FILTER_CLIENT_1'
...
SHOW_ABILITY_ADVANCE_ANIM = 'showAbilityAdvanceAnim'
...
COMP7_FLAGS_VERSION = 'comp7FlagsVersion'
...
class TankAcademy(object):
    TANK_ACADEMY_SETTINGS = 'tankAcademySettings'
    CURRENT_QUEST_ORDER = 'currentQuestOrder'


class Winback(object):
    WINBACK_SETTINGS = 'winbackSettings'
    NEED_SHOW_INTRO = 'needShowIntro'
    INTRO_LAST_TIME_SHOWN = 'introLastTimeShown'
    HAS_LEFT_VERSUS_AI_FROM_WINBACK = 'hasLeftVersusAIFromWinback'
    WINBACK_PROGRESSION_POINTS_SEEN = 'winbackProgressionsPointsSeen'
    WINBACK_BATTLES_COUNT = 'winbackBattlesCount'
...
DEFAULT_VALUES = {KEY_FILTERS: {STORE_TAB: 0, 
                 ...
                 COMP7_CAROUSEL_FILTER_CLIENT_1: {'searchNameVehicle': '', 
                                                  'clanRented': False}, 
                 VERSUS_AI_CAROUSEL_FILTER_1: {'ussr': False, 
                                               'germany': False, 
                                               'usa': False, 
                                               'china': False, 
                                               'france': False, 
                                               ...
                                               'level_10': False, 
                                               'level_11': False}, 
                 VERSUS_AI_CAROUSEL_FILTER_2: {'premium': False, 
                                               'elite': False, 
                                               'igr': False, 
                                               'rented': True, 
                                               'event': True, 
   ...
   KEY_SETTINGS: {'unitWindow': {SELECTED_INTRO_VEHICLES_FIELD: []}, 'vehicleSellDialog': {'isOpened': False}, 
                  ...
                  'cursor': False, 
                  'arcade': {'mixing': {'alpha': 100, 'type': 3}, 'gunTag': {'alpha': 100, 'type': 9}, 'centralTag': {'alpha': 100, 'type': 4}, 'net': {'alpha': 100, 'type': 0}, 'reloader': {'alpha': 100, 'type': 0}, 'condition': {'alpha': 100, 'type': 0}, 'cassette': {'alpha': 100, 'type': 0}, 'reloaderTimer': {'alpha': 100, 'type': 0}, 'zoomIndicator': {'alpha': 100, 'type': 0}}, 'sniper': {'mixing': {'alpha': 90, 'type': 3}, 'gunTag': {'alpha': 90, 'type': 9}, 'centralTag': {'alpha': 90, 'type': 4}, 'net': {'alpha': 90, 'type': 0}, 'reloader': {'alpha': 90, 'type': 0}, 'condition': {'alpha': 90, 'type': 0}, 'cassette': {'alpha': 90, 'type': 0}, 'reloaderTimer': {'alpha': 100, 'type': 0}, 'zoomIndicator': {'alpha': 100, 'type': 0}}, 'spgAim': {SPGAim.SHOTS_RESULT_INDICATOR: True, 
                             SPGAim.SPG_SCALE_WIDGET: True, 
                             SPGAim.AUTO_CHANGE_AIM_MODE: True, 
                             SPGAim.AIM_ENTRANCE_MODE: 0}, 
                  'contour': {CONTOUR.ENHANCED_CONTOUR: True, 
...
class AccountSettings(object):
    ...
    def convert():
        ...
        if currVersion != AccountSettings.version:
            ...
            if currVersion < 63:
                pass
            if currVersion < 64:
                obsoleteKeys = ['isEntryPointsEnabled', 'isTankRentalEnabled', 'isFreeDirectivesEnabled']
                for _, section in _filterAccountSection(ads):
                    accSettings = AccountSettings._readSection(section, KEY_SETTINGS)
            ...
            if currVersion < 81:
                for key, section in _filterAccountSection(ads):
                    accSettings = AccountSettings._readSection(section, KEY_SETTINGS)
                    if 'commanderCamHintSection' in accSettings.keys():
                        accSettings.deleteSection('commanderCamHintSection')
                    if PRE_BATTLE_HINT_SECTION in accSettings.keys():
                        preBattleSection = _unpack(accSettings[PRE_BATTLE_HINT_SECTION].asString)
                        for hintKey in ('battleCommunicationHint', 'reservesHintSection'):
    ...
    def setTankAcademySetting(cls, name, value):
        ...
        if name in taSection:
            ...
            _logger.error("Cann't set value in %s section for %s.", TankAcademy.TANK_ACADEMY_SETTINGS, name)
