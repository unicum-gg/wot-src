...
from AvatarInputHandler.DynamicCameras import ArcadeCamera, SniperCamera, StrategicCamera, ArtyCamera, DualGunCamera, AssaultCamera
...
class MouseSetting(ControlSetting):
    CAMERAS = {CTRL_MODE_NAME.POSTMORTEM: (
       ...
       CTRL_MODE_NAME.ASSAULT_SPG: (
                                  AssaultCamera.getCameraAsSettingsHolder, 'assaultMode/camera')}
...
class InterfaceScaleSetting(UserPrefsFloatSetting):
    ...
    def _getOptions(self):
        return [
         self.__getScales(graphics.getSuitableWindowSizes(), BigWorld.getCurrentResolution(BigWorld.WindowModeWindowed)),
    ...
    def __getScale(self):
        if self.__userData == self.AUTO_SCALE:
            return self.settingsCore.interfaceScale.getScaleOptions()[(-1)]
        return self.__userData

    def __indexToUserData(self, index):
        scaleOptions = self.settingsCore.interfaceScale.getScaleOptions()
        if index >= len(scaleOptions):
            return self.AUTO_SCALE
        return scaleOptions[int(index)]
...
class SniperModeByShiftSetting(StorageAccountSetting, MouseAffectedSetting):
    ...
    def _getCameras(self):
        return (
         CTRL_MODE_NAME.ARCADE, CTRL_MODE_NAME.SNIPER, CTRL_MODE_NAME.DUAL_GUN)
