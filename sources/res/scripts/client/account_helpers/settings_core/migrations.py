...
def _migrateTo103(core, data, initialized):
    ...
    limitedUiSpamOffRules = (
     (
      'uiSpamVisited_store', LuiRules.LOBBY_HEADER_COUNTERS_STORE),
     (
...
def _migrateTo120(core, data, initialized):
    data['feedbackSixthSense']['indicatorAlpha'] = 100
...
def _migrateTo148(core, data, initialized):
    ...
    data['gameExtData2'][GAME.ENABLE_BATTLE_CONTEXT_HINTS] = True
    config = getConfig()
    hintConf = {conf.hintId:conf for conf in config}
