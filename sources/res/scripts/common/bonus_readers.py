...
import items, calendar, constants
...
__all__ = [
 'readBonusSection', 'readUTC', 'SUPPORTED_BONUSES']
...
def __readBonus_vehicle(bonus, _name, section, eventType, checkLimit):
    ...
    if section.has_key('withSlot'):
        extra['withSlot'] = True
    if section.has_key('checkRecycleBin'):
        extra['checkRecycleBin'] = True
    if section.has_key('force'):
        extra['force'] = True
...
_RESERVED_NAMES = frozenset(['config', 'properties', 'limitID', 'probability', 'compensation', 'name', 'surprise',
 ...
 'depthLevel', 'mainRotationBranch', 'userProbability'])
