...
from unit_roster_config import SquadRoster, UnitRoster, SpecRoster, EventRoster, EpicRoster, BattleRoyaleRoster, MapBoxRoster
if TYPE_CHECKING:
...
ProfileVehicle = namedtuple('ProfileVehicle', ('vehCompDescr', 'vehOutfitCD', 'seasonType',
                                               'marksOnGun', 'prestigeLevel', 'stFrags'))
...
CMD_NAMES = {v:k for k, v in CLIENT_UNIT_CMD.__dict__.items() if not k.startswith('__') if not k.startswith('__')}
...
UNIT_ASSEMBLER_IMPL_NAMES = {k.lower():v for k, v in viewitems(UnitAssemblerImplType.__dict__) if isinstance(v, int) and not k.startswith('_') if isinstance(v, int) and not k.startswith('_')}
...
VEHICLE_TAGS_GROUP_BY_UNIT_MGR_FLAGS = {UNIT_MGR_FLAGS.EVENT: (
                        ('event_battles', ), tuple()), 
   UNIT_MGR_FLAGS.EPIC: (
                       tuple(), BATTLE_MODE_VEHICLE_TAGS - {'epic_battles'}), 
   UNIT_MGR_FLAGS.BATTLE_ROYALE: (
                                ('battle_royale', ), tuple()), 
...
class UnitBase(OpsUnpacker):
    ...
    def _setEstimatedTimeInQueue(self, estimatedTimeInQueue):
        LOG_DEBUG_DEV('_setEstimatedTimeInQueue ', estimatedTimeInQueue)
    ...
    def checkVehicleTypesRange(self, vehicleTypes):
        ...
        if any(value < vehicleTypes[key][0] for key, value in viewitems(vehicleCount)):
            ...
            if any(value > vehicleTypes[key][1] for key, value in viewitems(vehicleCount)):
