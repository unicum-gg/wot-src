...
from typing import TYPE_CHECKING, Tuple, Sequence, Dict, NamedTuple, Optional, Union, Type, List
if TYPE_CHECKING:
...
class InstallableItem(with_metaclass(ReflectionMetaclass, VehicleItem)):
...
@add_shallow_copy()
class Chassis(with_metaclass(ReflectionMetaclass, InstallableItem)):
        ...
        if self._chassisType is not None:
            ...
            if self.generalWheelsAnimatorConfig is not None and self.generalWheelsAnimatorConfig.getHasTrackWithinTrack():
                ...
                if self.tracks is not None:
                    for trackPair in viewvalues(self.tracks.trackPairs):
...
@add_shallow_copy()
class Turret(with_metaclass(ReflectionMetaclass, InstallableItem)):
...
class Gun(with_metaclass(ReflectionMetaclass, InstallableItem)):
...
class Hull(with_metaclass(ReflectionMetaclass, BasicItem)):
...
class Shell(BasicItem):
    ...
    def __init__(self, typeID, componentID, componentName, compactDescr):
        ...
        self.type = None
        self.effectsCaliber = component_constants.ZERO_FLOAT
        self.effectsIndex = component_constants.INVALID_EFFECT_INDEX
        self.prefabEffectsIndex = component_constants.INVALID_EFFECT_INDEX
    ...
    @property
    def kindIdx(self):
        return SHELL_TYPES_INDICES[self.kind]
