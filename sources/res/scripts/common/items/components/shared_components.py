...
__all__ = ('MaterialInfo', 'DEFAULT_MATERIAL_INFO', 'EmblemSlot', 'LodSettings', 'NodesAndGroups',
           ...
           'MechanicsParams', 'RechargeableNitroParams', 'ConcentrationModeParams',
           'ImprovedRammingParams', 'PowerModeParams', 'BattleFuryParams', 'PillboxSiegeModeParams',
           'StationaryReloadParams', 'ExtraShotClipParams', 'AccuracyStacksParams',
           'SecondaryGunParams', 'SupportWeaponParams', 'ChargeableBurstParams',
           'ChargeShotParams', 'OverheatStacksParams', 'TargetDesignatorParams',
           'StanceDanceParams', 'TemperatureGunThermalState', 'TemperatureGunThermalStates',
           'TemperatureGunParams', 'OverheatGunParams', 'HeatingZonesGunParams',
           'StagedJetBoostersParams', 'LowChargeShotParams', 'PropellantGunParams',
           'WheeledDashParams', 'AuxiliaryRocketLauncherParams', 'ShellSwitcherParams',
           'ShellCalibrationParams', 'AutoreloaderSurgeParams', 'BustleFeedParams',
           'SightPointerParams')
MaterialInfo = reflectedNamedTuple('MaterialInfo', ('kind', 'armor', 'extra', 'multipleExtra',
                                                    ...
                                                    'mayRicochet', 'collideOnceOnly',
                                                    'checkCaliberForRicochet', 'checkCaliberForHitAngleNorm',
                                                    'damageKind', 'chanceToHitByProjectile',
...
class AttachmentSlotDescription(with_metaclass(ReflectionMetaclass, object)):
    __slots__ = ('type', 'slotId', 'position', 'rotation', 'scale', 'attachNode', 'hiddenForUser',
                 'enableVisTunnel', 'applyType', 'size', 'hangerId', 'hangerRotation',
                 'compatibleModels')

    def __init__(self, slotType='', slotId=0, position=None, rotation=None, scale=None, attachNode=None, hiddenForUser=False, enableVisTunnel=False, applyType='', size='', hangerId=0, hangerRotation=None, compatibleModels=(
...
NodesAndGroups = reflectedNamedTuple('NodesAndGroups', ('nodes', 'groups', 'activePostmortem',
                                                        'lodSettings'))
...
class I18nComponent(object):
    __slots__ = ('__userString', '__shortString', '__description', '__converted', '__shortDescriptionSpecial',
                 '__longDescriptionSpecial', '__shortFilterAlertSpecial', '__longFilterAlertSpecial')
...
class DeviceHealth(object):
    __slots__ = ('maxHealth', 'repairCost', 'maxRegenHealth', 'healthRegenPerSec',
                 'healthBurnPerSec', 'chanceToHit', 'hysteresisHealth', 'invulnerable',
                 'repairSpeedLimiter', 'repairTime')
...
SpeedTriggerData = namedtuple('SpeedTriggerData', ('enableSpeed', 'hysteresisSpeed'))
TickedImpulseData = namedtuple('TickedImpulseData', ('cooldown', 'magnitudePerTick',
                                                     'applyPoint'))

def readTickedImpulseData(ctx, section, subsection='tickedImpulse'):
    tickedImpulseCtx, tickedImpulseSection = _xml.getSubSectionWithContext(ctx, section, subsection)
    tickedImpulse = TickedImpulseData(cooldown=_xml.readNonNegativeFloat(tickedImpulseCtx, tickedImpulseSection, 'cooldown'), magnitudePerTick=_xml.readTupleOfPositiveFloats(tickedImpulseCtx, tickedImpulseSection, 'magnitudePerTick'), applyPoint=_xml.readVector3(tickedImpulseCtx, tickedImpulseSection, 'applyPoint'))
    return tickedImpulse


def readShellCDsByName(ctx, section, subsection='shells'):
    ...
    for shellData in shellsList:
        shellNation, shellName = shellData.split(':')
        shellDescr = getShellByName(shellName, shellNation)
        if shellDescr is None:
            _xml.raiseWrongXml(ctx, '', ("Unknown shell's name: {}!").format(shellData))
...
class RechargeableNitroParams(MechanicsParams):
    ...
    def __applyMulModifier(self, modifierType, modifierName, bonusValue):
        ...
        newModifier = ('mul', modifierType, modifierName, mulValue, 'rechargeableNitro')
        foundIndex = None
...
class ImprovedRammingParams(MechanicsParams):
    __slots__ = ('damageBonusStageSize', 'damageBonusBasicFactor', 'damageBonusChangeFactor',
                 'trackDamageBonusStageSize', 'trackDamageBonusBasicFactor', 'trackDamageBonusChangeFactor',
                 'reductionDamageBonusStageSize', 'reductionDamageBonusBasicFactor',
                 'reductionDamageBonusChangeFactor', 'damageValueToShowAnimation',
...
class PowerModeParams(MechanicsParams):
    __slots__ = ('modeThreshold', 'modeDuration', 'accelerationFactor', 'attenuationFactor',
                 'speedThreshold', 'gracePeriod', 'vehicleParams')
...
class PillboxSiegeModeParams(MechanicsParams):
    __slots__ = ('switchDriveToPillboxTime', 'switchSiegeToPillboxTime', 'switchPillboxToSiegeTime',
                 'switchPillboxToDriveTime')
...
class StationaryReloadParams(GunMechanicsParams):
    __slots__ = ('preparingSpeedFactor', 'finishingSpeedFactor', 'preparingDelay',
                 'finishingDelay', 'fixAngles')
...
class AccuracyStacksParams(MechanicsParams):
    __slots__ = ('levelMax', 'levelInitial', 'levelAfterShot', 'aimLevelBonus', 'aimBonusCap',
                 'gainMaxSpd', 'gainTime', 'stabilizeBonus')
    ...
    @classmethod
    def _readMechanicsParams(cls, ctx, section, readModifiers):
        levelMax = _xml.readPositiveInt(ctx, section, 'levelMax')
        levelInitial = _xml.readNonNegativeInt(ctx, section, 'levelInitial')
        levelAfterShot = _xml.readNonNegativeInt(ctx, section, 'levelAfterShot')
        aimLevelBonus = _xml.readNonNegativeFloat(ctx, section, 'aimLevelBonus')
        aimBonusCap = _xml.readNonNegativeFloat(ctx, section, 'aimBonusCap', 0.99)
        gainMaxSpd = _xml.readNonNegativeFloat(ctx, section, 'gainMaxSpd')
        gainTime = _xml.readPositiveFloat(ctx, section, 'gainTime')
        stabilizeBonus = _xml.readNonNegativeFloat(ctx, section, 'stabilizeBonus')
...
class LowChargeShotParams(GunMechanicsParams):
    __slots__ = ('almostFinishedTime', 'reloadTimeCoefficient')
    MECHANICS_NAME = 'lowChargeShot'
    ...
    @classmethod
    def _readMechanicsParams(cls, ctx, section, readModifiers):
        almostFinishedTime = _xml.readNonNegativeFloat(ctx, section, 'almostFinishedTime')
        reloadTimeCoefficient = _xml.readNonNegativeFloat(ctx, section, 'reloadTimeCoefficient')
        modifiers = readModifiers(ctx, _xml.getSubsection(ctx, section, 'modifiers'))
        return cls(almostFinishedTime=almostFinishedTime, reloadTimeCoefficient=reloadTimeCoefficient, modifiers=modifiers)

    @classmethod
    def getDefaultMechanicsMiscAttributes(cls):
        return {'lowChargeShot/reloadTimeCoefficient': 1.0}

    def _applyMechanicsAttrs(self, attr, value):
        if attr == 'lowChargeShot/reloadTimeCoefficient':
...
class ChargeShotParams(MechanicsParams):
    ...
    @classmethod
    def _readMechanicsParams(cls, ctx, section, readModifiers):
        timePerLevel = list(_xml.readTupleOfNonNegativeFloats(ctx, section, 'timePerLevel'))
        damageFactorsPerLevel = list(_xml.readTupleOfNonNegativeFloats(ctx, section, 'damageFactorsPerLevel'))
        shotBlockTime = _xml.readNonNegativeFloat(ctx, section, 'shotBlockTime')
...
class OverheatStacksParams(MechanicsParams):
    __slots__ = ('levelMax', 'levelInc', 'levelDec', 'aimLevelBonus', 'dmgLevelBonus',
                 'gainMaxSpd', 'gainTime', 'delayTimerDuration', 'heatingTime', 'coolingTime',
                 'dmgBonus', 'aimBonus')
    ...
    def _readMechanicsParams(cls, ctx, section, _):
        heatingTime = _xml.readNonNegativeInt(ctx, section, 'heatingTime', 0)
        coolingTime = _xml.readNonNegativeInt(ctx, section, 'coolingTime', 0)
        dmgBonus = _xml.readNonNegativeFloat(ctx, section, 'dmgBonus', 0.0)
        aimBonus = _xml.readNonNegativeFloat(ctx, section, 'aimBonus', 0.0)
        gainMaxSpd = _xml.readNonNegativeFloat(ctx, section, 'gainMaxSpd', 0.0)
        delayTimerDuration = _xml.readNonNegativeFloat(ctx, section, 'delayTimerDuration', 0.0)
...
class TargetDesignatorParams(MechanicsParams):
    __slots__ = MechanicsParams.__slots__ + ('damageIncomeFactor', 'cooldownTime',
                                             'deployTime', 'spottedMarkedTime', 'unspottedMarkedTime')
...
class AutoreloaderSurgeParams(MechanicsParams):
    __slots__ = ('maxCharges', 'startCharges', 'chargeTimeSRegular', 'chargeTimeSFullClip',
                 'reloadTime')
    MECHANICS_NAME = 'autoreloaderSurge'
    ...
    @classmethod
    def _readMechanicsParams(cls, ctx, section, readModifiers):
        maxCharges = _xml.readNonNegativeInt(ctx, section, 'maxCharges')
        if not cls._MIN_CHARGES <= maxCharges <= cls._MAX_CHARGES:
            _xml.raiseWrongXml(ctx, '', ('[{}] maxCharges must be in range [{}, {}], got {}').format(cls.MECHANICS_NAME, cls._MIN_CHARGES, cls._MAX_CHARGES, maxCharges))
        startCharges = _xml.readNonNegativeInt(ctx, section, 'startCharges')
        chargeTimeSRegular = _xml.readNonNegativeFloat(ctx, section, 'chargeTimeSRegular')
        chargeTimeSFullClip = _xml.readNonNegativeFloat(ctx, section, 'chargeTimeSFullClip')
        reloadTime = _xml.readNonNegativeFloat(ctx, section, 'reloadTime')
    ...
    @classmethod
    def getDefaultMechanicsMiscAttributes(cls):
        return {'autoreloaderSurge/maxCharges': 0, 
           'autoreloaderSurge/startCharges': 0, 
           'autoreloaderSurge/chargeTimeSRegular': 0.0, 
           'autoreloaderSurge/chargeTimeSFullClip': 0.0, 
           'autoreloaderSurge/reloadTime': 0.0}

    def _applyMechanicsAttrs(self, attr, value):
        if attr == 'autoreloaderSurge/maxCharges':
            self.maxCharges += value
        elif attr == 'autoreloaderSurge/startCharges':
            self.startCharges += value
        elif attr == 'autoreloaderSurge/chargeTimeSRegular':
            self.chargeTimeSRegular += value
        elif attr == 'autoreloaderSurge/chargeTimeSFullClip':
            self.chargeTimeSFullClip += value
        elif attr == 'autoreloaderSurge/reloadTime':
            self.reloadTime += value


class StanceDanceParams(MechanicsParams):
    __slots__ = ('timeSwitchStance', 'maxEnergy', 'gainFightEnergyPoints', 'gainTurboEnergyPoints',
                 'gainEnergyTime', 'gainTurboEnergyBonusPoints', 'gainTurboEnergySpdLimitKmh',
                 'passiveFightEnergyBonusPerHit', 'passiveTurboFwdSpdBonusKmh', 'passiveTurboBkwdSpdBonusKmh',
                 'passiveTurboEnginePowerBonus', 'passiveTurboAccuracyDebuff', 'passiveTurboAimSpeedDebuff',
                 'passiveTurboStabilizeDebuff', 'passiveTurboAfterShotDispersionDebuff',
                 'activeFightCost', 'activeFightDuration', 'activeFightAccuracyBonus',
                 'activeFightAimSpeedBonus', 'activeFightStabilizeBonus', 'activeFightAfterShotDispersionBonus',
    ...
    @classmethod
    def _readMechanicsParams(cls, ctx, section, readModifiers):
        timeSwitchStance = _xml.readNonNegativeFloat(ctx, section, 'timeSwitchStance')
        maxEnergy = _xml.readNonNegativeFloat(ctx, section, 'maxEnergy')
        gainFightEnergyPoints = _xml.readNonNegativeFloat(ctx, section, 'gainFightEnergyPoints')
        gainTurboEnergyPoints = _xml.readNonNegativeFloat(ctx, section, 'gainTurboEnergyPoints')
        gainEnergyTime = _xml.readNonNegativeFloat(ctx, section, 'gainEnergyTime')
        gainTurboEnergyBonusPoints = _xml.readNonNegativeFloat(ctx, section, 'gainTurboEnergyBonusPoints')
        gainTurboEnergySpdLimitKmh = _xml.readNonNegativeFloat(ctx, section, 'gainTurboEnergySpdLimitKmh')
        passiveFightEnergyBonusPerHit = _xml.readNonNegativeFloat(ctx, section, 'passiveFightEnergyBonusPerHit')
        passiveTurboFwdSpdBonusKmh = _xml.readNonNegativeFloat(ctx, section, 'passiveTurboFwdSpdBonusKmh')
        passiveTurboBkwdSpdBonusKmh = _xml.readNonNegativeFloat(ctx, section, 'passiveTurboBkwdSpdBonusKmh')
        passiveTurboEnginePowerBonus = _xml.readNonNegativeFloat(ctx, section, 'passiveTurboEnginePowerBonus')
        passiveTurboAccuracyDebuff = _xml.readNonNegativeFloat(ctx, section, 'passiveTurboAccuracyDebuff')
        passiveTurboAimSpeedDebuff = _xml.readNonNegativeFloat(ctx, section, 'passiveTurboAimSpeedDebuff')
        passiveTurboStabilizeDebuff = _xml.readNonNegativeFloat(ctx, section, 'passiveTurboStabilizeDebuff')
        passiveTurboAfterShotDispersionDebuff = _xml.readNonNegativeFloat(ctx, section, 'passiveTurboAfterShotDispersionDebuff')
        activeFightCost = _xml.readNonNegativeFloat(ctx, section, 'activeFightCost')
        activeFightDuration = _xml.readNonNegativeFloat(ctx, section, 'activeFightDuration')
        activeFightAccuracyBonus = _xml.readNonNegativeFloat(ctx, section, 'activeFightAccuracyBonus')
        activeFightAimSpeedBonus = _xml.readNonNegativeFloat(ctx, section, 'activeFightAimSpeedBonus')
        activeFightStabilizeBonus = _xml.readNonNegativeFloat(ctx, section, 'activeFightStabilizeBonus')
        activeFightAfterShotDispersionBonus = _xml.readNonNegativeFloat(ctx, section, 'activeFightAfterShotDispersionBonus')
        activeFightReloadSpdBonus = _xml.readNonNegativeFloat(ctx, section, 'activeFightReloadSpdBonus')
        activeTurboCost = _xml.readNonNegativeFloat(ctx, section, 'activeTurboCost')
        activeTurboDuration = _xml.readNonNegativeFloat(ctx, section, 'activeTurboDuration')
        activeTurboFwdSpdBonusKmh = _xml.readNonNegativeFloat(ctx, section, 'activeTurboFwdSpdBonusKmh')
        activeTurboBkwdSpdBonusKmh = _xml.readNonNegativeFloat(ctx, section, 'activeTurboBkwdSpdBonusKmh')
        activeTurboEnginePowerBonus = _xml.readNonNegativeFloat(ctx, section, 'activeTurboEnginePowerBonus')
        activeTurboRotationSpeedDebuff = _xml.readNonNegativeFloat(ctx, section, 'activeTurboRotationSpeedDebuff')
        activeTurboRammingDmgBonus = _xml.readNonNegativeFloat(ctx, section, 'activeTurboRammingDmgBonus')
        impulse = readImpulseData(ctx, section)
...
class OverheatGunParams(GunMechanicsParams):
    __slots__ = ('coolingPerSecFactor', 'tempOverheatOnThreshold', 'tempOverheatOffThreshold',
...
class HeatingZonesGunParams(GunMechanicsParams):
    ...
    @classmethod
    def _readMechanicsParams(cls, ctx, section, readModifiers):
        zonesValues = lmap(float, _xml.readNonEmptyString(ctx, section, 'zones').split())
...
class StagedJetBoostersParams(MechanicsParams):
    __slots__ = ('deployTime', 'reloadTime', 'reuseCount', 'duration', 'impulse', 'impulseSpeedLimits',
                 'modifiers', 'customRotationPoints')
...
PropellantGunStage = namedtuple('PropellantGunStage', [
 'maxCharge', 'modifiers', 'isOvercharge', 'damageFactorLimits'])

class PropellantGunDamageFactors(object):
    __slots__ = ('minFactor', 'maxFactor')
...
class PropellantGunParams(GunMechanicsParams):
    __slots__ = ('chargeStages', 'chargingPerSec', 'chargeSpendingAfterShot', 'forbiddenShells',
                 'shouldPauseOnReload', 'chargeDelay', 'dischargingPerSec', 'overchargeSwitchCooldown')
    MECHANICS_NAME = 'propellantAfterburnerGun'
    _MAX_STAGES = 10
    ...
    @classmethod
    def getDefaultMechanicsMiscAttributes(cls):
        return {'propellantGun/chargingPerSec': 1.0, 
           'propellantGun/dischargingPerSec': 1.0, 
           'propellantGun/chargeSpendingAfterShot': 1.0, 
           'propellantGun/damageMinFactor': 0.0, 
           'propellantGun/damageMaxFactor': 0.0, 
           'propellantGun/overchargeDamageMinFactor': 0.0, 
           'propellantGun/overchargeDamageMaxFactor': 0.0, 
           'propellantGun/chargeDelay': 0.0}

    def applyDynModifiersToMechanics(self, dynModifiers):
        if not dynModifiers:
            return
        regularFilter = self.MECHANICS_NAME + 'Regular'
        overchargeFilter = self.MECHANICS_NAME + 'Overcharge'
        regularModifiers = []
        overchargeModifiers = []
        for modifier in dynModifiers:
            modifierFilter = modifier[4]
    ...
    @classmethod
    def _readMechanicsParams(cls, ctx, section, readModifiers):
        chargingPerSec = _xml.readPositiveFloat(ctx, section, 'chargingPerSec')
        dischargingPerSec = _xml.readPositiveFloat(ctx, section, 'dischargingPerSec')
        chargeSpendingAfterShot = _xml.readPositiveFloat(ctx, section, 'chargeSpendingAfterShot')
        shouldPauseOnReload = _xml.readBool(ctx, section, 'shouldPauseOnReload', True)
        chargeDelay = _xml.readNonNegativeFloat(ctx, section, 'chargeDelay')
        overchargeSwitchCooldown = _xml.readNonNegativeFloat(ctx, section, 'overchargeSwitchCooldown')
        forbiddenShells = readShellCDsByName(ctx, section, 'forbiddenShells')
    ...
    def _readChargeStages(cls, ctx, section, readModifiers):
        if not section.has_key('chargeStages'):
            _xml.raiseWrongXml(ctx, '', ('[{}] Section <chargeStages> is missing!').format(cls.MECHANICS_NAME))
        stages = []
        overchargeStagesCount = 0
        maxCharges = set()
        for tag, subsection in section['chargeStages'].items():
            if tag != 'stage':
                _xml.raiseWrongXml(ctx, '', ('[{}] Unexpected section <{}>!').format(tag, cls.MECHANICS_NAME))
            maxCharge = _xml.readPositiveFloat(ctx, subsection, 'maxCharge')
            if maxCharge in maxCharges:
                _xml.raiseWrongXml(ctx, '', ('[{}] Max charge is duplicated: {}!').format(cls.MECHANICS_NAME, maxCharge))
            modifiers = readModifiers(ctx, _xml.getSubsection(ctx, subsection, 'modifiers'))
            isOvercharge = _xml.readBool(ctx, subsection, 'isOvercharge', False)
            damageFactorLimits = _xml.readTupleOfPositiveFloats(ctx, subsection, 'damageFactorLimits')
            if len(damageFactorLimits) != 2:
                _xml.raiseWrongXml(ctx, '', ("[{}] Expected 2 values for 'damageFactorLimits' field: {}!").format(cls.MECHANICS_NAME, damageFactorLimits))
        ...
        if not stages:
            _xml.raiseWrongXml(ctx, '', ('[{}] Section chargeStages should not be empty!').format(cls.MECHANICS_NAME))
        if len(stages) > cls._MAX_STAGES:
            _xml.raiseWrongXml(ctx, '', ('[{}] Section chargeStages should not have number of stages > {}!').format(cls.MECHANICS_NAME, cls._MAX_STAGES))
        ...
        if any(state.isOvercharge for state in stages[:len(stages) - overchargeStagesCount]):
            _xml.raiseWrongXml(ctx, '', ('[{}] Overcharge stages should be continuous: {}').format(cls.MECHANICS_NAME, stages))
        return stages

    def _applyMechanicsAttrs(self, attr, value):
        if attr == 'propellantGun/chargingPerSec':
            self.chargingPerSec *= value
        elif attr == 'propellantGun/dischargingPerSec':
            self.dischargingPerSec *= value
        elif attr == 'propellantGun/chargeSpendingAfterShot':
            self.chargeSpendingAfterShot *= value
        elif attr == 'propellantGun/damageMinFactor':
            self.__applyDamageFactor('minFactor', value, False)
        elif attr == 'propellantGun/damageMaxFactor':
            self.__applyDamageFactor('maxFactor', value, False)
        elif attr == 'propellantGun/overchargeDamageMinFactor':
            self.__applyDamageFactor('minFactor', value, True)
        elif attr == 'propellantGun/overchargeDamageMaxFactor':
            self.__applyDamageFactor('maxFactor', value, True)
        elif attr == 'propellantGun/chargeDelay':
...
class WheeledDashParams(MechanicsParams):
    __slots__ = ('deployTime', 'reloadTime', 'duration', 'speedTrigger', 'speedReloadTimeFactor',
                 'tickedImpulse')
    MECHANICS_NAME = 'wheeledDash'
    ...
    def _readMechanicsParams(cls, ctx, section, readModifiers):
        ...
        return cls(deployTime=_xml.readPositiveFloat(ctx, section, 'deployTime'), reloadTime=_xml.readPositiveFloat(ctx, section, 'reloadTime'), duration=_xml.readPositiveFloat(ctx, section, 'duration'), modifiers=modifiers, speedTrigger=speedTrigger, speedReloadTimeFactor=speedReloadTimeFactor, tickedImpulse=readTickedImpulseData(ctx, section))

    @classmethod
    def readReloadTimeBoostData(cls, ctx, section, subsection='reloadTimeBoost'):
        reloadTimeBoostCtx, reloadTimeBoostSection = _xml.getSubSectionWithContext(ctx, section, subsection)
        speedTrigger = SpeedTriggerData(enableSpeed=_xml.readNonNegativeFloat(reloadTimeBoostCtx, reloadTimeBoostSection, 'minSpeed'), hysteresisSpeed=_xml.readNonNegativeFloat(reloadTimeBoostCtx, reloadTimeBoostSection, 'hysteresisSpeed'))
        speedReloadTimeFactor = _xml.readNonNegativeFloat(reloadTimeBoostCtx, reloadTimeBoostSection, 'reloadTimeFactor')
        return (speedTrigger, speedReloadTimeFactor)

    @classmethod
    def getDefaultMechanicsMiscAttributes(cls):
        return {'wheeledDash/deployTime': 0.0, 
           'wheeledDash/reloadTime': 0.0, 
           'wheeledDash/duration': 0.0}

    def _applyMechanicsAttrs(self, attr, value):
        if attr == 'wheeledDash/deployTime':
            self.deployTime += value
        elif attr == 'wheeledDash/reloadTime':
            self.reloadTime += value
        elif attr == 'wheeledDash/duration':
            self.duration += value


class AuxiliaryRocketLauncherParams(GunMechanicsParams):
    MECHANICS_NAME = 'auxiliaryRocketLauncher'
...
class ShellSwitcherParams(GunMechanicsParams):
    __slots__ = ('modifiedShells', )
    MECHANICS_NAME = 'shellParamsSwitcher'
    ...
    @classmethod
    def _readMechanicsParams(cls, ctx, section, readModifiers):
        modifiedShells = readShellCDsByName(ctx, section, 'modifiedShells')
        if not modifiedShells:
            raise SoftException(('[{}] Empty modifiedShells!').format(cls.MECHANICS_NAME))
        return cls(modifiedShells=tuple(modifiedShells))


class ShellCalibrationBonus(object):
    __slots__ = ('dmgBonus', 'modifiers')
...
class ShellCalibrationParams(GunMechanicsParams):
    __slots__ = ('penBonuses', 'nonPenBonuses', 'forbiddenShells')
    MECHANICS_NAME = 'shellCalibration'
    ...
    def _readMechanicsParams(cls, ctx, section, readModifiers):
        params = {}
        for condition in ('penetration', 'nonPenetration'):
            condCtx, condSection = _xml.getSubSectionWithContext(ctx, section, condition)
            dmgFactor = _xml.readNonNegativeFloat(condCtx, condSection, 'damageCoefficient', 0.0)
            condMods = None
            if condSection['modifiers'] is not None:
                condMods = readModifiers(condCtx, _xml.getSubsection(condCtx, condSection, 'modifiers'))
            params[condition] = ShellCalibrationBonus(dmgFactor, condMods)

        forbiddenShells = readShellCDsByName(ctx, section, 'forbiddenShells')
        return cls(penBonuses=params['penetration'], nonPenBonuses=params['nonPenetration'], forbiddenShells=forbiddenShells)

    def applyDynModifiersToMechanics(self, dynModifiers):
        if not dynModifiers:
            return
        penFilter = self.MECHANICS_NAME + 'Penetration'
        nonPenFilter = self.MECHANICS_NAME + 'NonPenetration'
    ...
    @classmethod
    def getDefaultMechanicsMiscAttributes(cls):
        return {'shellCalibration/penetrationDamageCoefficient': 0.0, 
           'shellCalibration/nonPenetrationDamageCoefficient': 0.0}

    def _applyMechanicsAttrs(self, attr, value):
        if attr == 'shellCalibration/penetrationDamageCoefficient':
            self.penBonuses.dmgBonus += value
        elif attr == 'shellCalibration/nonPenetrationDamageCoefficient':
            self.nonPenBonuses.dmgBonus += value


class CrestMovingParams(MechanicsParams):
    MECHANICS_NAME = 'crestMoving'


class BustleFeedParams(MechanicsParams):
    __slots__ = ('activationTime', 'deactivationTime', 'modifiers', 'bustleShotReloadFactor',
                 'bustleShotDamageBonusShell0', 'bustleShotDamageBonusShell1', 'bustleShotsIndices',
                 'animationTime')
    MECHANICS_NAME = 'bustleFeed'
    ...
    def _readMechanicsParams(cls, ctx, section, readModifiers):
        modifiers = readModifiers(ctx, _xml.getSubsection(ctx, section, 'modifiers'))
        return cls(activationTime=_xml.readNonNegativeFloat(ctx, section, 'activationTime'), deactivationTime=_xml.readNonNegativeFloat(ctx, section, 'deactivationTime'), modifiers=modifiers, bustleShotReloadFactor=_xml.readPositiveFloat(ctx, section, 'bustleShotReloadFactor'), bustleShotDamageBonusShell0=_xml.readPositiveFloat(ctx, section, 'bustleShotDamageBonusShell0'), bustleShotDamageBonusShell1=_xml.readPositiveFloat(ctx, section, 'bustleShotDamageBonusShell1'))
    ...
    @classmethod
    def getDefaultMechanicsMiscAttributes(cls):
        return {'bustleFeed/bustleShotDamageBonusShell0': 0.0, 
           'bustleFeed/bustleShotDamageBonusShell1': 0.0, 
           'bustleFeed/bustleShotReloadFactor': 1.0}

    def _applyMechanicsAttrs(self, attr, value):
        if attr == 'bustleFeed/bustleShotDamageBonusShell0':
            self.bustleShotDamageBonusShell0 += value
        if attr == 'bustleFeed/bustleShotDamageBonusShell1':
            self.bustleShotDamageBonusShell1 += value
        if attr == 'bustleFeed/bustleShotReloadFactor':
            self.bustleShotReloadFactor *= value


SightPointerAbilityStage = typing.NamedTuple('SightPointerAbilityStage', (
 (
  'id', int),
 (
  'duration', float),
 (
  'angle', float),
 (
  'visionModifiers', typing.List[typing.Tuple[(str, str, str, float, str)]]),
 (
  'vehicleModifiers', typing.List[typing.Tuple[(str, str, str, float, str)]])))

class SightPointerParams(MechanicsParams):
    __slots__ = ('initialDeployTime', 'reloadTime', 'activeStages', 'selfReveal', 'selfRevealVisionTime',
                 'sightPointerStages')
    MECHANICS_NAME = 'sightPointer'
    _MAX_STAGES = 10
    _MIN_DURATION = 1
    ...
    @classmethod
    def getDefaultMechanicsMiscAttributes(cls):
        return {'sightPointer/initialDeployTime': 0.0, 
           'sightPointer/reloadTime': 0.0, 
           'sightPointer/activeStages': 0.0, 
           'sightPointer/selfRevealVisionTime': 0.0}

    def _applyMechanicsAttrs(self, attr, value):
        if attr == 'sightPointer/initialDeployTime':
            self.initialDeployTime += value
        elif attr == 'sightPointer/reloadTime':
            self.reloadTime += value
        elif attr == 'sightPointer/activeStages':
            self.activeStages += int(value)
        elif attr == 'sightPointer/selfRevealVisionTime':
            self.selfRevealVisionTime += value

    @classmethod
    def _readMechanicsParams(cls, ctx, section, readModifiers):
        initialDeployTime = _xml.readNonNegativeFloat(ctx, section, 'initialDeployTime')
        reloadTime = _xml.readPositiveFloat(ctx, section, 'reloadTime')
        activeStages = _xml.readPositiveInt(ctx, section, 'activeStages')
        selfReveal = _xml.readBool(ctx, section, 'selfReveal')
        selfRevealVisionTime = _xml.readNonNegativeFloat(ctx, section, 'selfRevealVisionTime')
        ...
        if initialDeployTime < SIGHT_POINTER_COMMON_CONSTANTS.ANIMATION_DELAY:
            _xml.raiseWrongXml(ctx, '', ('[{}] <initialDeployTime> should be more or equal than animation time').format(cls.MECHANICS_NAME))
        ...
        if reloadTime < SIGHT_POINTER_COMMON_CONSTANTS.ANIMATION_DELAY:
            _xml.raiseWrongXml(ctx, '', ('[{}] <reloadTime> should be more or equal than animation time').format(cls.MECHANICS_NAME))
        reloadTime -= SIGHT_POINTER_COMMON_CONSTANTS.ANIMATION_DELAY
        if activeStages > len(sightPointerStages):
            _xml.raiseWrongXml(ctx, '', ('[{}] Section <sightPointerStages> stages is less than current activeStages!').format(cls.MECHANICS_NAME))
    ...
    def _readStages(cls, ctx, section, readModifiers):
        if not section.has_key('sightPointerStages'):
            _xml.raiseWrongXml(ctx, '', ('[{}] Section <sightPointerStages> is missing!').format(cls.MECHANICS_NAME))
        stages = []
        stagesIDs = set()
        for tag, subsection in viewitems(section['sightPointerStages']):
            if tag != 'stage':
                _xml.raiseWrongXml(ctx, '', ('[{}] Unexpected section <{}>!').format(cls.MECHANICS_NAME, tag))
            stageIdx = _xml.readInt(ctx, subsection, 'id')
            if stageIdx in stagesIDs:
                _xml.raiseWrongXml(ctx, '', ('[{}] duplicated id:{} in <sightPointerStages>,!').format(cls.MECHANICS_NAME, stageIdx))
            stagesIDs.add(stageIdx)
            duration = _xml.readNonNegativeFloat(ctx, subsection, 'duration')
            if duration < cls._MIN_DURATION:
                _xml.raiseWrongXml(ctx, '', ('[{}] section <duration> should be >= {}!').format(cls.MECHANICS_NAME, cls._MIN_DURATION))
            angle = _xml.readNonNegativeFloat(ctx, subsection, 'angle')
            visionModifiers = readModifiers(ctx, _xml.getSubsection(ctx, subsection, 'vision_modifiers'))
            vehicleModifiers = readModifiers(ctx, _xml.getSubsection(ctx, subsection, 'vehicle_modifiers'))
        ...
        if not stages:
            _xml.raiseWrongXml(ctx, '', ('[{}] Section sightPointerStages should not be empty!').format(cls.MECHANICS_NAME))
        if len(stages) > cls._MAX_STAGES:
            _xml.raiseWrongXml(ctx, '', ('[{}] Section sightPointerStages should not have number of stages > {}!').format(cls.MECHANICS_NAME, cls._MAX_STAGES))
