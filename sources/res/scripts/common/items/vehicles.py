...
if IS_CLIENT or IS_UE_EDITOR:
    ...
    MODES_WITHOUT_CRYSTAL_EARNINGS = {
     'bob', 'fallout', 'event_battles', 'battle_royale', 'clanWarsBattles'}
...
ParagonsBranchToReset = namedtuple('ParagonsBranchToReset', [
 'branchId',
 'resetVehicles',
 'isHidden'])

class ParagonsResetBranchesCollection(object):
    __slots__ = ('__data', '__resetBranchIDsByVehicleCd', '__resetBranchIDsByNationID')
    __TOTAL_ID_LENGTH = 4
...
class VehicleDescriptor(object):
    ...
    isTemperatureGun = property(lambda self: 'temperature' in self.gun.tags)
    ...
    def prerequisites(self, newPhysic=True):
        ...
        for turretDescr, gunDescr in self.turrets:
            ...
            detachmentEff = itertools.chain((
             detachmentEff['flight'], detachmentEff['flamingOnGround']), detachmentEff['collision'].itervalues())
    ...
    def _updateAttributes(self, onAnyApp=False):
        ...
        self.miscAttrs = miscAttrs = {'repairSpeedFactor': 1.0, 
           ...
           'multShotDispersionFactor': 1.0, 
           'chassisHealthAfterHysteresisFactor': 0.0, 
           'ammoBayHealthFactor': 1.0, 
           'engineHealthFactor': 1.0, 
           'chassisHealthFactor': 1.0, 
           'fuelTankHealthFactor': 1.0, 
...
class VehicleType(object):
    ...
    __metaclass__ = ReflectionMetaclass
    __slots__ = (
     'name', 'id', 'compactDescr', 'mode', 'xmlPath', 'tags', 'level', 'hasSiegeMode',
     'hasAutoSiegeMode', 'isWheeledVehicle', 'isWheeledVehicleWithoutFeatures',
     'isFlamethrower', 'isAssaultSPG', 'isDualgunVehicleType',
     'hasCustomDefaultCamouflage', 'customizationNationID', 'baseColorID', 'speedLimits', 'repairCost',
     'crewXpFactor', 'premiumVehicleXPFactor', 'xpFactor', 'creditsFactor', 'freeXpFactor', 'healthBurnPerSec',
     'healthBurnPerSecLossFraction', 'invisibility', 'invisibilityDeltas', 'crewRoles', 'extras', 'extrasDict',
     'extrasProtection', 'devices', 'tankmen', 'damageByStaticsChances', 'i18nInfo', 'balanceByClass',
     ...
     'prefabAttachments', 'ability', 'preferential', 'camCapsuleScaleOverride', 'camGunCapsuleScaleOverride',
     '__weakref__')
...
class Cache(object):
    __slots__ = ('__vehicles', '__commonConfig', '__chassis', '__engines', '__fuelTanks',
                 ...
                 '__moduleKind', '__postProgression', '__gunSpinEffects', '__paragonsBranchesToReset')
    ...
    def initParagonsBranchesToReset(self):
        self.__paragonsBranchesToReset = _readParagonsBranchesToReset(_VEHICLE_TYPE_XML_PATH + 'common/paragons_reset_branches.xml')
    ...
    def _gunSpinEffects(self):
        if self.__gunSpinEffects is None:
            self.__gunSpinEffects = _readSpinEffectGroups(_VEHICLE_TYPE_XML_PATH + 'common/gun_spin_effects.xml')
...
def _vehicleLambda(nationID, itemName):
    fullName = nations.NAMES[nationID] + ':' + itemName
...
def getItemIdByItemName(itemTypeID, nationID, itemName):
    try:
        ...
        LOG_CURRENT_EXCEPTION()
        LOG_ERROR('(wrong item name)', itemName)
        raise
...
def _readTrackPairs(xmlCtx, section):
    if not section.has_key('trackPairParams'):
        return tuple([
         chassis_components.TrackPair(hitTesterManager=_readHitTester(xmlCtx, section, 'hitTester'), materials=_readArmor(xmlCtx, section, 'armor', index=component_constants.MAIN_TRACK_PAIR_IDX), healthParams=shared_readers.readDeviceHealthParams(xmlCtx, section))])
    else:
...
MultiGunState = namedtuple('MultiGunState', ('patterns', 'sequence'))
MultiGunPattern = namedtuple('MultiGunPattern', ('gunIndexes', 'sequence'))
...
def _readGun(xmlCtx, section, item, unlocksDescrs=None, _=None):
    ...
    if item.temperature is None:
        tags = tags.difference(('temperature', ))
    else:
        tags = tags.union(('temperature', ))
    if item.spin == component_constants.DEFAULT_SPIN_GUN:
        tags = tags.difference(('spin', ))
    else:
        tags = tags.union(('spin', ))
    if dualGun is None:
...
def _readGunLocals(xmlCtx, section, sharedItem, unlocksDescrs, turretCompactDescr):
    ...
    if IS_CLIENT or IS_UE_EDITOR:
        ...
        if not section.has_key('spinEffect'):
...
def _readGunClipBurst(xmlCtx, section, type):
    ...
    rate = _xml.readPositiveFloat(xmlCtx, section, type + '/rate', 0)
...
def _readShell(xmlCtx, section, name, nationID, shellTypeID, icons):
    ...
    shell.damage = (
     _xml.readPositiveFloat(xmlCtx, section, 'damage/armor'),
    ...
    if kind in HAS_EXPLOSION_EFFECT:
        ...
        guaranteedDamages = (
         _xml.readNonNegativeFloat(subXmlCtx, subsection, 'damage/armor', 0.0),
         _xml.readNonNegativeFloat(subXmlCtx, subsection, 'damage/devices', 0.0))
    ...
    if section.has_key('distanceFactor'):
        ...
        for factorName in subsection.keys():
            ...
            if len(factors) % 2:
                _xml.raiseWrongXml(subXmlCtx, factorName, ('Factors length must be even ({})"').format(factors))
            if len(set(factors[::2])) * 2 != len(factors):
                _xml.raiseWrongXml(subXmlCtx, factorName, ('Only unique distances are allowed({})').format(factors))
...
_g_boolMatInfoParams = (
 ...
 'collideOnceOnly', 'continueTraceIfNoHit', 'checkCaliberForRichet', 'checkCaliberForHitAngleNorm')
...
def _readSpinEffectGroups(xmlPath):
    ...
    for sname, subsection in section.items():
        ...
        sname = intern(sname)
        res[sname] = (
         sound_readers.readWWTripleSoundConfig(subsection['activationSound']),
         sound_readers.readWWTripleSoundConfig(subsection['deactivationSound']))
...
def _readParagonsBranchesToReset(xmlPath):
    ...
    section = ResMgr.openSection(xmlPath)
    if section is None or section['paragonsResetBranch'] is None:
        ...
        for sname, subsection in section.items():
            ...
            branchId = _xml.readInt(ctx, subsection, 'id', 1)
            if branchId in definedBranchIds:
                _xml.raiseWrongXml(xmlCtx, sname, ('branch with {0} id has been already defined').format(branchId))
            definedBranchIds.add(branchId)
            isHidden = _xml.readBool(ctx, subsection, 'hidden', False)
            vehicleNations = set()
            vehiclesCompDescriptorsToReset = []
            resetVehiclesNames = _xml.readTupleOfStrings(ctx, subsection, 'resetVehicles')
            for resetVehicleName in resetVehiclesNames:
                ...
                if resetVehicleCD in definedResetVehicles:
                    _xml.raiseWrongXml(xmlCtx, sname, ('vehicle with {0} has already been defined to reset in branch').format(resetVehicleName))
            ...
            if len(vehicleNations) > 1:
                _xml.raiseWrongXml(xmlCtx, sname, ('vehicles belong to more than one nation in one branch {0}').format(branchId))
        ...
        for bid, bdata in paragonsBranchesToReset.iteritems():
            for bbid, bbdata in paragonsBranchesToReset.iteritems():
                ...
                if resetVehicles.issubset(bbdata.resetVehicles) or resetVehicles.issuperset(bbdata.resetVehicles):
                    raise SoftException(('Branch {} is completely overlaps (overlapped by) branch {}').format(bid, bbid))
...
def _readShotEffects(xmlCtx, section):
    ...
    if IS_CLIENT or IS_UE_EDITOR:
        ...
        if res['shellType'] == SHELL_TYPES.FLAME:
            res['targetImpulse'] = 0.0
        else:
...
def _readThermalVisionParams(xmlCtx, section):
    if not section.has_key('thermalVision'):
        return None
    else:
        thermalCtx, thermalSection = _xml.getSubSectionWithContext(xmlCtx, section, 'thermalVision')
        return shared_components.ThermalVisionParams(initialReloadTime=_xml.readNonNegativeFloat(thermalCtx, thermalSection, 'initialReloadTime'), reloadTime=_xml.readNonNegativeFloat(thermalCtx, thermalSection, 'reloadTime'), duration=_xml.readNonNegativeInt(thermalCtx, thermalSection, 'duration'), hSectorAngle=_xml.readNonNegativeFloat(thermalCtx, thermalSection, 'hSectorAngle'), vSectorAngle=_xml.readNonNegativeFloat(thermalCtx, thermalSection, 'vSectorAngle'), distance=_xml.readNonNegativeInt(thermalCtx, thermalSection, 'distance'), timeToObserve=_xml.readNonNegativeFloat(thermalCtx, thermalSection, 'timeToObserve'), timeInObservation=_xml.readNonNegativeFloat(thermalCtx, thermalSection, 'timeInObservation'), useCount=_xml.readInt(thermalCtx, thermalSection, 'useCount'))
...
def _readAbility(section, xmlCtx):
    if not section.has_key('ability'):
        return
    else:
        abilityName = _xml.readString(xmlCtx, section, 'ability')
        abilityId = g_cache.equipmentIDs().get(abilityName)
        if abilityId is None:
            _xml.raiseWrongXml(xmlCtx, 'ability', ('Ability with name "{}" is not found').format(abilityName))
        return abilityId
    ...
    if section is None:
        ...
        for tag, subsection in section['thermalStates'].items():
            if tag == 'state':
                ...
                if subsection.has_key('heatingPerSec'):
                    stateHeatingPerSec = _xml.readPositiveInt(subXmlCtx, subsection, 'heatingPerSec')
                if subsection.has_key('coolingDelay'):
                    stateCoolingDelay = _xml.readNonNegativeFloat(subXmlCtx, subsection, 'coolingDelay')
        ...
        if not states:
            LOG_WARNING('No temperature states in thermalStates, creating state with default params')
