...
class BattleResultStats(ViewImpl, IGlobalListener, CallbackDelayer):
    ...
    def _sendPlatoonInvitation(self, args):
        if not (args and self.lsCtrl.isAvailable()):
            ...
            if not dbID:
                return
            yield self.prbDispatcher.doSelectAction(PrbAction(PREBATTLE_ACTION_NAME.LAST_STAND_SQUAD, accountsToInvite=[int(dbID)], extData={'arenaUniqueID': self.__arenaUniqueID if self.lsCtrl.isEventPrb() else None}))
            return
    ...
    def __makeTeamMemberModel(self, idx, playerVO):
        ...
        banType = (playerVO['hasPenalties'] or TeamMemberBanType).NOTBANNED if 1 else TeamMemberBanType.WARNED
        ...
        member.stats.setKeys(playerVO['progressPoints'])
