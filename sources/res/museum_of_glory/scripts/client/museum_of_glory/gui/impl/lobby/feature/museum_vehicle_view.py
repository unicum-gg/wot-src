...
class MuseumVehicleView(BaseTransitionView):
    __slots__ = ('__currentVehicleIndex', '__isAnimationFreeze', '__isAnimationPlaying',
                 '__hasDelayedBalanceUpdates', '__vehicles', '__audioEnabled', '__currentYear',
                 '__isExcursionPlaying', '__welcomeCallback', '__uiLogger', '__vehDtos',
                 '__isConfigChanged', '__isExcursionPaused', '__selectedVehID', '__worldSoundPos',
                 '__isSpaceLoaded')
    ...
    def createToolTipContent(self, event, contentID):
        ...
        if contentID == R.views.lobby.early_access.tooltips.EarlyAccessSimpleTooltipView():
            return EarlyAccessStateTooltipView(event.getArgument('state'))
    ...
    def _initialize(self, *args, **kwargs):
        ...
        g_eventBus.handleEvent(events.LobbyInterfaceEvent(events.LobbyInterfaceEvent.TOGGLE_VISIBILITY, ctx={'headerIsVisible': False, 'messengerBarVisible': False}), EVENT_BUS_SCOPE.LOBBY)
    ...
    def _finalize(self):
        ...
        self.__uiLogger.log()
        self.__unloadSpace()
        g_eventBus.handleEvent(events.LobbyInterfaceEvent(events.LobbyInterfaceEvent.TOGGLE_VISIBILITY, ctx={'headerIsVisible': True, 'messengerBarVisible': True}), EVENT_BUS_SCOPE.LOBBY)
