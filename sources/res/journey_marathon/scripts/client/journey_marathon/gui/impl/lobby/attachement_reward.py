...
class AttachmentRewardWindow(LobbyNotificationWindow):
    ...
    def __init__(self, element, isFirstEntry):
        self._blur = None
        super(AttachmentRewardWindow, self).__init__(wndFlags=WindowFlags.WINDOW | WindowFlags.WINDOW_FULLSCREEN, content=CustomizationRarityRewardScreen(element, isFirstEntry))
        self._args = (
         element, isFirstEntry)
        return
