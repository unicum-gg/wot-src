...
class FallTanksEvacuationPlugin(HintPanelPlugin, FunSubModesWatcher):
    __slots__ = ('__isActive', '__isInDisplayPeriod', '__callbackDelayer', '__isVisible',
                 '__hintContext')
    _PERIODS = (
     ARENA_PERIOD.BATTLE,)
    _HINT_TIMEOUT = 6
    ...
    def __onBattleLoading(self, event):
        ...
        if event.ctx.get('hintCtx') == self.__hintContext and not battleLoadingShown and self.__isInDisplayPeriod and not self.__sessionProvider.isReplayPlaying:
            self.__showHint()

    def __onHintShown(self, event):
        if event.ctx.get('hintCtx') == self.__hintContext:
