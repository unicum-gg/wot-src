...
_EVACUATION_LINKAGE = _LINK.DESTROY_TIMER_UI
_EVACUATION_ICON = 'destroyTimerRespawnUI'
_VEHICLE_EVACUATION_TIMER_TYPE = _TIMER_STATES.RECOVERY
_TIMERS_PRIORITY = {(_VEHICLE_EVACUATION_TIMER_TYPE, _TIMER_STATES.CRITICAL_VIEW): 1, 
...
class FallTanksTimersPanel(TimersPanel):
    ...
    def _generateMainTimersData(self):
        return [
         self._getNotificationTimerData(_VEHICLE_EVACUATION_TIMER_TYPE, _EVACUATION_ICON, _EVACUATION_LINKAGE, _COLORS.GRAY, text=backport.text(R.strings.fall_tanks.statusNotificationTimer.evacuation()), iconOffsetY=self._OFFSET),
         self._getNotificationTimerData(_TIMER_STATES.OVERTURNED, _LINK.OVERTURNED_GREEN_ICON, _LINK.DESTROY_TIMER_UI, _COLORS.GREEN, text=backport.text(R.strings.ingame_gui.destroyTimer.liftOver()), iconOffsetY=self._OFFSET)]
    ...
    def _getTimersCollectionCls(cls):
        ...
        if isReplayPlaying:
            return _FallTanksReplayStackTimersCollection
        return _FallTanksRegularStackTimersCollection

    def __updateVehicleEvacuationNotification(self, evacuationState):
        if evacuationState.isActive:
            self._showTimer(_VEHICLE_EVACUATION_TIMER_TYPE, evacuationState.totalTime, TIMER_VIEW_STATE.CRITICAL, evacuationState.endTime)
        else:
