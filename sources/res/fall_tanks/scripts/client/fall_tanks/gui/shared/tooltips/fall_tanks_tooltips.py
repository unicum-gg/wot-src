...
from fall_tanks.gui.shared.tooltips import TOOLTIP_TYPE
_IMAGE_LEFT_PADDING = 69

class FallTanksBlockToolTipData(BlocksTooltipData):

    def __init__(self, context, toolTipType=TOOLTIP_TYPE.FALL_TANKS):
...
class FallTanksShellBlockToolTipData(FallTanksBlockToolTipData):
    ...
    def _packBlocks(self, *args, **kwargs):
        ...
        shellImageBlock = formatters.packImageBlockData(img=backport.image(FALL_TANKS_SUB_MODE_IMAGES_PATH.shell.big.fall_tanks_custom_shell()), padding=formatters.packPadding(left=_IMAGE_LEFT_PADDING))
        ...
        return [
         formatters.packBuildUpBlockData([headerTextBlock, shellImageBlock]), descriptionBlock]
...
class FallTanksAbilitiesBlockToolTipData(FallTanksBlockToolTipData):
    ...
    def _packBlocks(self, *args, **kwargs):
        ...
        imageBlock = formatters.packImageBlockData(img=backport.image(FALL_TANKS_SUB_MODE_IMAGES_PATH.ability.tooltips.dyn(descriptor.icon[0])()), padding=formatters.packPadding(left=_IMAGE_LEFT_PADDING))
        descriptionBlock = packTextBlockData(text_styles.main(descriptor.description), padding=formatters.packPadding(top=-5))
        return [
         formatters.packBuildUpBlockData([abilityNameBlock, typeNameBlock, cooldownTimeBlock, imageBlock]),
         descriptionBlock]