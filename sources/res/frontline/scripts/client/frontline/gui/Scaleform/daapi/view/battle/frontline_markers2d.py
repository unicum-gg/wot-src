...
from frontline.gui.Scaleform.daapi.view.battle.frontline_plugins import FrontlineRespawnableVehicleMarkerPlugin, FrontlineVehicleMarkerPlugin
...
class FrontlineMissionsPlugin(plugins.MarkerPlugin):
    ...
    def start(self):
        super(FrontlineMissionsPlugin, self).start()
    ...
    def stop(self):
        ...
        if respawnCtrl is not None:
            respawnCtrl.onVehicleDeployed -= self._onVehicleDeployed
        super(FrontlineMissionsPlugin, self).stop()
        return
    ...
    def _onPlayerMissionUpdated(self, mission, _):
        ...
        if mission.isBaseMission():
            ...
            if markerID is not None and not self._isInFreeSpectatorMode:
                if not FrontlineMissionsPlugin._getAnyMarkerRepliedByPlayer(self._markers):
...
class FrontlineMarkersManager(MarkersManager):
    ...
    def _setupPlugins(self, arenaVisitor):
        ...
        setup = super(FrontlineMarkersManager, self)._setupPlugins(arenaVisitor)
        ...
        if arenaVisitor.hasRespawns():
            setup['vehicles'] = FrontlineRespawnableVehicleMarkerPlugin
        else:
            setup['vehicles'] = FrontlineVehicleMarkerPlugin
        return setup
    ...
    def stopPlugins(self):
        ...
        if ctrl is not None:
            ctrl.onRespawnVisibilityChanged -= self.__onRespawnScreenVisibilityChanged
        super(FrontlineMarkersManager, self).stopPlugins()
        return
