...
from server_side_replay.gui.impl.gen.view_models.views.lobby.top_replay_model import StatParams
...
class MyReplaysPage(PageSubModelPresenter):
    ...
    def __init__(self, viewModel, parentView):
        ...
        super(MyReplaysPage, self).__init__(viewModel, parentView)
        return

    @property
    def pageId(self):
        return ReplaysViews.MYREPLAYS
    ...
    def initialize(self, *args, **kwargs):
        self.__updateState()
        super(MyReplaysPage, self).initialize(*args, **kwargs)
    ...
    def __updateState(self, loadedData=None, model=None):
        ...
        if self.__loadingData:
            ...
            if self.__cachedBestReplaysData is None:
                model.setState(State.ERROR)
                return
            model.setState(State.SUCCESS)
            self.__updateReplays(self.__cachedBestReplaysData)
            return

    @replaceNoneKwargsModel
    def __updateReplays(self, allReplaysData, model=None):
    ...
    def __onDataReceived(self, dataName, data):
        if dataName not in (DataNames.MY_REPLAYS, DataNames.REPLAY_LINK):
            return
        if dataName == DataNames.MY_REPLAYS:
        ...
        with self.viewModel.transaction() as (tx):
            ...
            if self.__cachedBestReplaysData:
                self.__updateReplays(self.__cachedBestReplaysData, model=tx)
